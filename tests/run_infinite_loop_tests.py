...
class ITSMTestAutomationSuite:
        ...
        try:
            ...
            while (
                ...
                if iteration_result.errors_detected == 0:
                    ...
                    self.logger.warning(
                        f"⚠️ {iteration_result.errors_detected} errors detected and {iteration_result.errors_fixed} fixed"
                    )
    ...
    async def _execute_iteration(self) -> IterationResult:
        ...
        self.logger.info(
            f"Iteration {self.current_iteration} completed: {status} ({iteration_duration:.2f}s)"
        )
        return result
    ...
    async def _run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.playwright_suite:
                playwright_result = (
                    await self.playwright_suite.run_comprehensive_tests()
                )
    ...
    async def _execute_load_test(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (
                (successful_requests / total_reqs * 100) if total_reqs > 0 else 0
            )
            avg_response_time = (
    ...
    async def _report_progress(self):
        ...
        successful_iterations = len(
            [r for r in self.iteration_results if r.status == "success"]
        )
        success_rate = (
        ...
        self.logger.info(f"\n{'='*60}")
        self.logger.info(
            f"PROGRESS REPORT - Iteration {self.current_iteration}/{self.max_iterations}"
        )
        self.logger.info(f"{'='*60}")
        self.logger.info(f"Status: {latest_result.status}")
        self.logger.info(f"Duration: {latest_result.duration:.2f}s")
        self.logger.info(
            f"Tests: {latest_result.total_tests} total, {latest_result.passed_tests} passed, {latest_result.failed_tests} failed"
        )
        self.logger.info(
            f"Errors: {latest_result.errors_detected} detected, {latest_result.errors_fixed} fixed"
        )
        self.logger.info(
            f"Success Rate: {success_rate:.1f}% ({successful_iterations}/{total_iterations})"
        )
    ...
    async def _generate_final_report(self, total_duration: float) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_iterations = len(
                [r for r in self.iteration_results if r.status in ["failure", "error"]]
            )
            partial_success_iterations = len(
                [r for r in self.iteration_results if r.status == "partial_success"]
            )
            ...
            total_errors_detected = sum(
                r.errors_detected for r in self.iteration_results
            )
            total_errors_fixed = sum(r.errors_fixed for r in self.iteration_results)

            # 平均計算
            avg_duration = (
                sum(r.duration for r in self.iteration_results) / total_iterations
                if total_iterations > 0
                else 0
            )
            ...
            report = {
                ...
                "error_summary": {
                    ...
                    "fix_success_rate": (
                        (total_errors_fixed / total_errors_detected * 100)
            ...
            report_file = (
                ...
                / "reports"
                / f"itsm_automation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            print(
                f"Tests: {total_tests} total, {total_passed} passed, {total_failed} failed"
            )
            print(
                f"Errors: {total_errors_detected} detected, {total_errors_fixed} fixed"
            )
