...
class EnhancedRealtimeMonitoringSystem:
    ...
    async def start_enhanced_monitoring(self):
        ...
        try:
            ...
            while monitoring_count < max_monitoring_cycles:
                monitoring_count += 1
                self.logger.info(f"📊 監視サイクル {monitoring_count}/{max_monitoring_cycles} 開始")
                ...
                if not health_status["is_healthy"]:
                    ...
                    self.logger.info(f"✅ システム正常 (連続正常: {self.system_state['consecutive_healthy_checks']})")
    ...
    async def comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        if coord_health.get("error_count", 0) > 0:
            health_status["is_healthy"] = False
            health_status["issues"].append(f"Coordination errors: {coord_health['error_count']}")
                ...
                with open(coord_error_file, 'r') as f:
                    ...
                    if content:
                        ...
                        if isinstance(data, list):
                            return {"error_count": len(data), "errors": data[:5]}  # 最新5件
                        else:
    ...
    async def check_server_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            server_status["backend_response"] = response.json() if response.status_code == 200 else None
        except Exception as e:
    ...
    async def monitor_cicd_status(self):
        ...
        if github_dir.exists():
            ...
            self.logger.info(f"📋 GitHub Actions ワークフロー: {len(workflow_files)} 件確認")
    ...
    async def update_realtime_repair_state(self):
        ...
        enhanced_config = {
            ...
            "state": {
                ...
                "uptime_minutes": (datetime.now() - datetime.fromisoformat(self.system_state["monitoring_start"])).total_seconds() / 60
            }
        }
    ...
    async def update_monitoring_state(self):
        ...
        self.system_state["uptime"] = (datetime.now() - datetime.fromisoformat(self.system_state["monitoring_start"])).total_seconds()
    ...
    async def generate_monitoring_report(self):
        ...
        report = {
            ...
            "statistics": {
                "total_monitoring_cycles": self.system_state.get("monitoring_cycles", 0),
                "consecutive_healthy_checks": self.system_state["consecutive_healthy_checks"],
        ...
        try:
            ...
            self.logger.info(f"✅ 連続正常チェック: {self.system_state['consecutive_healthy_checks']} 回")
