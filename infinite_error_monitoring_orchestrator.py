...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("infinite_monitoring.log"), logging.StreamHandler()],
)
...
class InfiniteErrorMonitoringOrchestrator:
    def __init__(self):
        ...
        self.targets = [
            MonitoringTarget(
                name="WebUI", url="http://192.168.3.135:3000", type="frontend"
            ),
            MonitoringTarget(
                name="Backend API", url="http://192.168.3.135:8000", type="backend"
            ),
            MonitoringTarget(
                name="API Docs", url="http://192.168.3.135:8000/docs", type="backend"
            ),
            MonitoringTarget(
            ...
            if AIOHTTP_AVAILABLE:
                async with self.session.get(target.url) as response:
                    ...
                    if "error" in content.lower() or "exception" in content.lower():
                        errors.append(
                            f"コンテンツにエラーメッセージを検出: {target.url}"
                        )
            else:
                # requests を使用した同期処理
    ...
    async def _check_backend_errors(self, target: MonitoringTarget) -> List[str]:
        ...
        try:
            # ヘルスチェックエンドポイント
            health_url = (
                f"{target.url.rstrip('/')}/health"
                if target.url.endswith("8000")
                else target.url
            )
            ...
            if AIOHTTP_AVAILABLE:
                async with self.session.get(health_url) as response:
                    ...
                    if response_time > 5.0:  # 5秒以上は異常
                        errors.append(
                            f"レスポンス時間異常: {response_time:.2f}秒 - {health_url}"
                        )
            else:
                # requests を使用した同期処理
                start_time = time.time()
    ...
    async def _repair_frontend_errors(
        self, target: MonitoringTarget, errors: List[str]
    ) -> bool:
        """フロントエンドエラーの修復"""
        try:
            ...
            try:
                repair_url = (
                    "http://192.168.3.135:3000/api/browser-error-monitor/repair"
                )
                if AIOHTTP_AVAILABLE:
                    async with self.session.post(
                        repair_url, json={"errors": errors}
                    ) as response:
                        if response.status == 200:
                    ...
                    response = self.session.post(
                        repair_url, json={"errors": errors}, timeout=30
                    )
                    if response.status_code == 200:
                        logger.info(f"フロントエンド自動修復成功: {target.name}")
                        return True
    ...
    async def _repair_backend_errors(
        self, target: MonitoringTarget, errors: List[str]
    ) -> bool:
        """バックエンドエラーの修復"""
        try:
    ...
    async def _monitoring_cycle(self):
        ...
        logger.info(
            f"統計 - チェック:{self.stats['total_checks']} エラー:{self.stats['total_errors']} 修復:{self.stats['total_repairs']} 成功:{self.stats['successful_repairs']}"
        )
