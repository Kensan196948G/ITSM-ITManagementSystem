...
from app.services.github_actions_service import GitHubActionsService, get_failed_workflows

logger = logging.getLogger(__name__)

# ファイルパス定数
CICD_LOG_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/.claude-flow/ci-retry-log.json"
...
API_ERROR_METRICS_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/api_error_metrics.json"
COORDINATION_ERRORS_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/errors.json"
...
class CICDIntegrationService:
    ...
    async def update_repair_system_state(self, event_type: str, 
        ...
        async with self._lock:
            try:
                ...
                repair_state["cicd_integration"]["events"] = repair_state["cicd_integration"]["events"][-50:]
                repair_state["cicd_integration"]["last_activity"] = datetime.now().isoformat()
        ...
        try:
            ...
            for workflow in failed_workflows:
                analysis = {
                    ...
                    "auto_retry_eligible": await self._check_auto_retry_eligibility(workflow)
                }
        ...
        try:
            ...
            if self.github_service:
                try:
                    ...
                    logger.warning(f"Could not analyze job details for workflow {workflow['id']}: {e}")
    ...
    async def _check_auto_retry_eligibility(self, workflow: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if datetime.now().replace(tzinfo=created_at.tzinfo) - created_at > timedelta(hours=6):
                return False
    ...
    async def _get_retry_count(self, workflow_id: int) -> int:
        ...
        try:
            ...
            for entry in entries:
                if (entry.get("event_type") == "workflow_retry" and 
                    entry.get("details", {}).get("run_id") == workflow_id):
                    retry_count += 1
    ...
    async def execute_auto_repair(self, target: str = "ci_failures") -> Dict[str, Any]:
        ...
        try:
            ...
            # システム状態を更新
            await self.update_repair_system_state("auto_repair_completed", repair_results)
    ...
    async def _integrate_repair_results(self, repair_results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 総修復数を更新
            loop_state["total_errors_fixed"] = loop_state.get("total_errors_fixed", 0) + repair_results["successful_repairs"]
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            integration_status = {
                ...
                "cicd_integration": repair_state.get("cicd_integration", {
                    "enabled": False,
                    "last_activity": None,
                    "events": []
                })
...
async def monitor_and_repair_ci() -> Dict[str, Any]:
    ...
    async with CICDIntegrationService() as integration_service:
        ...
        if failures:
            ...
            # 自動修復を実行
            repair_results = await integration_service.execute_auto_repair("ci_failures")
            return repair_results
        else:
            return {
                "timestamp": datetime.now().isoformat(),
...
async def trigger_manual_integration_repair(target: str = "all", 
                                          user_email: str = "system") -> Dict[str, Any]:
    """手動統合修復をトリガー"""
    async with CICDIntegrationService() as integration_service:
