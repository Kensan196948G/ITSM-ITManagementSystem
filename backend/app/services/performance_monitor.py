...
class SystemResourceMonitor:
    ...
    def get_system_metrics(self) -> SystemMetrics:
        ...
        network_io = {
            ...
            "packets_sent": network_io_new.packets_sent
            - self.network_io_old.packets_sent,
            "packets_recv": network_io_new.packets_recv
            - self.network_io_old.packets_recv,
        }
...
class PerformanceAnalyzer:
    ...
    def _check_response_time(
        ...
        level = self._get_performance_level(
            metric.response_time, self.thresholds.response_time
        )
    ...
    def _check_resource_usage(
        ...
        if metric.cpu_usage > self.thresholds.cpu_usage[PerformanceLevel.POOR]:
            ...
            if not self._is_in_cooldown(alert_key):
                ...
                level = self._get_performance_level(
                    metric.cpu_usage, self.thresholds.cpu_usage, reverse=True
                )
        ...
        if metric.memory_usage > self.thresholds.memory_usage[PerformanceLevel.POOR]:
            ...
            if not self._is_in_cooldown(alert_key):
                ...
                level = self._get_performance_level(
                    metric.memory_usage, self.thresholds.memory_usage, reverse=True
                )
    ...
    def _get_performance_level(
        self,
        value: float,
        thresholds: Dict[PerformanceLevel, float],
        reverse: bool = False,
    ) -> PerformanceLevel:
        """パフォーマンスレベルを取得"""
        if reverse:
...
class PerformanceMonitor:
    ...
    async def _trigger_emergency_response(self, alerts: List[PerformanceAlert]):
        """緊急対応をトリガー"""
        logger.critical(
            f"Triggering emergency response for {len(alerts)} critical alerts"
        )
        ...
        alert_file = (
            Path("emergency_alerts") / f"critical_alerts_{int(time.time())}.json"
        )
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for endpoint, method in endpoints:
            ...
            endpoint_stats[endpoint_key] = self.database.get_endpoint_stats(
                endpoint, hours
            )
        ...
        return {
            ...
            "performance_level": self._get_overall_performance_level(
                response_times, error_count / len(metrics) if metrics else 0
            ),
        }

    def _get_overall_performance_level(
        self, response_times: List[float], error_rate: float
    ) -> str:
        """全体パフォーマンスレベルを取得"""
