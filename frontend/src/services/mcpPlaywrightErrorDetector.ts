...
export interface BrowserError {
  id: string;
  timestamp: Date;
  type: 'console' | 'network' | 'javascript' | 'security' | 'accessibility';
...
export interface RepairAction {
  id: string;
  errorId: string;
  type: 'css_fix' | 'javascript_fix' | 'html_fix' | 'network_fix' | 'accessibility_fix';
  description: string;
  code: string;
  applied: boolean;
  timestamp: Date;
...
export class MCPPlaywrightErrorDetector {
   ...
   * 指定されたブラウザを起動
   */
  private async launchBrowser(browserType: 'chromium' | 'firefox' | 'webkit'): Promise<void> {
    try {
      let browser: Browser;
   ...
   * ページにエラーリスナーを設定
   */
  private async setupErrorListeners(page: Page, url: string, browserType: string): Promise<void> {
    // コンソールエラーをキャッチ
   ...
   * エラーを報告し、自動修復を試行
   */
  private async reportError(error: BrowserError): Promise<void> {
   ...
   * 自動修復を試行
   */
  private async attemptAutoRepair(error: BrowserError): Promise<void> {
    try {
   ...
   * エラーに基づいて修復アクションを生成
   */
  private async generateRepairActions(error: BrowserError): Promise<RepairAction[]> {
   ...
   * 修復アクションを適用
   */
  private async applyRepair(action: RepairAction, error: BrowserError): Promise<boolean> {
    try {
   ...
   * 修復の検証
   */
  private async validateRepair(page: Page, originalError: BrowserError): Promise<boolean> {
    try {
      // ページをリロードして検証
   ...
   * エラーレポートを保存
   */
  private async saveErrorReport(error: BrowserError): Promise<void> {
    try {
  ...
  private getPageKeyFromError(error: BrowserError): string {
  ...
  private calculateRepairSuccessRate(): number {
...
// デフォルト設定
export const defaultConfig: ErrorDetectionConfig = {
  targetUrls: [
