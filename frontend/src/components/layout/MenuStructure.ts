 ...
 */
export const itsmMenuStructure: MenuSection[] = [
  // ダッシュボードセクション
  {
    id: 'dashboard',
    title: 'ダッシュボード',
 ...
 * クイックアクセス項目を取得
 */
export const getQuickAccessItems = (): MenuItem[] => {
  const quickItems: MenuItem[] = []
  
  const findQuickAccessItems = (items: MenuItem[]) => {
    items.forEach(item => {
      if (item.quickAccess) {
...
export const filterMenuByPermissions = (
  ...
  return menuItems.filter(item => {
    ...
    if (item.permission.permissions) {
      const hasPermission = item.permission.permissions.some(permission => 
...
export const getFlatMenuItems = (): MenuItem[] => {
  ...
  const flattenItems = (items: MenuItem[], parentPath = '') => {
    items.forEach(item => {
