...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/itsm_monitor.log'),
...
class ITSMRealtimeMonitor:
    def __init__(self):
        ...
        # 修復エンジンパス
        self.repair_engine = self.backend_path / "scripts" / "itsm_loop_repair_engine.py"
    ...
    def monitor_cycle(self):
        ...
        try:
            ...
            if issues:
                self.error_count += 1
                logger.warning(f"問題検出 ({len(issues)} 件): {[i['type'] for i in issues]}")
...
def main():
    ...
    # ログディレクトリ作成
    os.makedirs("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs", exist_ok=True)
