...
def create_initial_data(db: Session) -> None:
    ...
    categories_data = [
        ...
        {
            "name": "ソフトウェア",
            "description": "アプリケーション、OS、システムソフトウェア",
        },
        {
            "name": "ネットワーク",
            "description": "ネットワーク機器、接続、通信に関する問題",
        },
    ...
    for cat_data in categories_data:
        existing_cat = (
            db.query(Category)
            .filter(Category.name == cat_data["name"], Category.tenant_id == tenant_id)
            .first()
        )
    ...
    for team_data in teams_data:
        existing_team = (
            db.query(Team)
            .filter(Team.name == team_data["name"], Team.tenant_id == tenant_id)
            .first()
        )
