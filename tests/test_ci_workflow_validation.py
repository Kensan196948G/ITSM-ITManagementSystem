...
class TestCIWorkflowValidation:
    ...
    def workflow_files(self):
        """ワークフローファイルパスを取得"""
        base_path = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/.github/workflows")
        return {
            "ci": base_path / "ci.yml",
    ...
    def loop_repair_state(self):
        """無限ループ修復システムの状態を取得"""
        state_path = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/infinite_loop_state.json")
    ...
    def test_github_actions_schema_compliance(self, workflow_files):
        ...
        for name, path in workflow_files.items():
            ...
            # ジョブ内容の検証
            assert isinstance(workflow['jobs'], dict), f"{name} のjobsは辞書である必要があります"
            
            for job_name, job_config in workflow['jobs'].items():
                assert 'runs-on' in job_config, f"{name} のジョブ '{job_name}' にruns-onが必要"
                assert 'steps' in job_config, f"{name} のジョブ '{job_name}' にstepsが必要"
    ...
    def test_dependency_workflow_validation(self, workflow_files):
        ...
        assert jobs['build']['needs'] == 'lint', "buildジョブはlintに依存する必要があります"
        
        assert 'needs' in jobs['test'], "testジョブにbuildへの依存関係が必要"
        assert jobs['test']['needs'] == 'build', "testジョブはbuildに依存する必要があります"
        
        # notify ジョブの失敗時条件確認
        assert 'if' in jobs['notify'], "notifyジョブに条件が必要"
        assert '${{ failure() }}' in jobs['notify']['if'], "notifyジョブは失敗時のみ実行される必要があります"
    ...
    def test_error_handling_configuration(self, workflow_files):
        ...
        for job_name in critical_jobs:
            ...
            assert job_config.get('continue-on-error', True) == False, \
    ...
    def test_log_output_configuration(self, workflow_files):
        ...
        for step in retry_workflow['jobs']['retry-ci']['steps']:
            if 'Record retry status' in step.get('name', ''):
                found_log_step = True
                assert '.claude-flow/ci-retry-log.json' in step['run'], \
                    "ログファイルへの記録コマンドが不適切"
                break
    ...
    def test_infinite_loop_repair_integration(self, loop_repair_state):
        ...
        # (実際のタイミングテストは別途必要)
        assert loop_repair_state.get('loop_count', 0) > 0, "修復ループが実行されていません"
        assert loop_repair_state.get('total_errors_fixed', 0) >= 0, "修復エラー総数が不正"
    ...
    def test_itsm_compliance_requirements(self):
        ...
        error_state_path = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/errors.json")
    ...
    def test_performance_metrics(self):
        ...
        metrics_path = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/.claude-flow/metrics")
        ...
        if performance_file.exists():
            ...
            if total_tasks > 0:
                ...
                assert success_rate >= 0.95, f"タスク成功率が95%未満: {success_rate:.2%}"
...
def generate_test_report():
    ...
    report_path = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/tests/reports/ci_workflow_validation_report.json")
