...
class AttachmentService:
    ...
    def list_attachments(
        ...
        try:
            ...
            return [
                self._build_attachment_response(attachment)
    ...
    def get_attachment(self, attachment_id: UUID, current_user_id: UUID) -> Attachment:
        ...
        self._check_resource_access(
            attachment.resource_type, attachment.resource_id, current_user_id
        )
    ...
    def delete_attachment(
        self, attachment_id: UUID, current_user_id: UUID, permanent: bool = False
    ):
        """添付ファイルを削除する"""
        try:
            attachment = self.get_attachment(attachment_id, current_user_id)
            ...
            # 削除権限チェック
            if attachment.uploaded_by != current_user_id and not self._is_admin(
                current_user_id
            ):
                raise HTTPException(
            ...
            logger.info(
                f"Attachment {action}: {attachment_id} by user {current_user_id}"
            )
    ...
    def restore_attachment(self, attachment_id: UUID, current_user_id: UUID):
        ...
        try:
            ...
            logger.info(
                f"Attachment restored: {attachment_id} by user {current_user_id}"
            )
    ...
    async def generate_preview(
        self, attachment_id: UUID, current_user_id: UUID, size: Optional[str] = None
    ):
        """ファイルプレビューを生成する"""
        try:
            attachment = self.get_attachment(attachment_id, current_user_id)
            ...
            logger.error(
                f"Error generating preview for attachment {attachment_id}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def get_scan_result(self, attachment_id: UUID, current_user_id: UUID) -> dict:
        ...
        try:
            ...
            scan_result = (
                self.db.query(AttachmentScanResult)
                .filter(AttachmentScanResult.attachment_id == attachment_id)
                .first()
            )
            ...
            return {
                ...
                "scanned_at": (
                    scan_result.scanned_at.isoformat()
                    if scan_result.scanned_at
                    else None
                ),
            ...
            logger.error(
                f"Error getting scan result for attachment {attachment_id}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def bulk_delete_attachments(
        self, attachment_ids: List[UUID], current_user_id: UUID, permanent: bool = False
    ) -> int:
        """添付ファイルを一括削除する"""
        try:
            deleted_count = 0
            ...
            for attachment_id in attachment_ids:
                try:
                    ...
                    logger.warning(
                        f"Failed to delete attachment {attachment_id}: {e.detail}"
                    )
                    continue
    ...
    def get_storage_usage(
        self, current_user_id: UUID, resource_type: Optional[str] = None
    ) -> dict:
        """ストレージ使用量を取得する"""
        try:
            ...
            ).filter(Attachment.deleted_at.is_(None))
            ...
            type_stats = (
                ...
                .filter(Attachment.deleted_at.is_(None))
                .group_by(Attachment.resource_type)
                .all()
            )
            ...
            return {
                ...
                "storage_usage_percent": (
                    round(
                        ((result.total_size or 0) / settings.MAX_STORAGE_PER_TENANT)
                        * 100,
                        2,
                    )
                    if settings.MAX_STORAGE_PER_TENANT > 0
                    else 0
                ),
            }
    ...
    async def _validate_file(self, file: UploadFile) -> dict:
        ...
        if file_ext not in self.allowed_extensions:
            return {
                "valid": False,
                "error": f"許可されていないファイル形式です: {file_ext}",
            }
        ...
        if file_size > self.max_file_size:
            return {
                "valid": False,
                "error": f"ファイルサイズが制限（{self.max_file_size / (1024*1024):.1f}MB）を超えています",
            }
        ...
        try:
            ...
            if not self._is_allowed_mime_type(detected_type):
                return {
                    "valid": False,
                    "error": f"許可されていないファイル形式です: {detected_type}",
                }
        except Exception as e:
    ...
    async def _schedule_virus_scan(self, attachment_id: UUID):
        ...
        try:
            ...
            logger.error(
                f"Error scheduling virus scan for attachment {attachment_id}: {str(e)}"
            )
    ...
    async def _generate_preview_file(
        self, attachment: Attachment, size: Optional[str] = None
    ) -> str:
        """プレビューファイルを生成する"""
        # 画像ファイルの場合はサムネイル生成
        ...
        try:
            with Image.open(attachment.file_path) as img:
                ...
                preview_dir = os.path.join(
                    os.path.dirname(attachment.file_path), "previews"
                )
    ...
    async def _generate_text_preview(self, attachment: Attachment) -> str:
        ...
        try:
            ...
            preview_filename = (
                f"{os.path.splitext(attachment.stored_filename)[0]}_preview.txt"
            )
            preview_path = os.path.join(preview_dir, preview_filename)
    ...
    def _build_attachment_response(self, attachment: Attachment) -> AttachmentResponse:
        ...
        return AttachmentResponse(
            ...
            preview_url=(
                f"/api/v1/attachments/preview/{attachment.id}"
                if self._is_previewable(attachment.content_type)
                else None
            ),
        )
