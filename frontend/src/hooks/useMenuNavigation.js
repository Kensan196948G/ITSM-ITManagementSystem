...
import { itsmMenuStructure, filterMenuByPermissions, getFlatMenuItems } from '../components/layout/MenuStructure';
export const useMenuNavigation = () => {
    ...
    const navigationState = useMemo(() => {
        ...
        // 現在のメニューアイテムを見つける
        const findCurrentMenuItem = (items) => {
        ...
        if (parentSection && currentMenuItem) {
            ...
            // パスを分析してブレッドクラムを構築
            const buildBreadcrumbs = (items, targetPath, basePath = []) => {
                for (const item of items) {
        ...
        const siblingItems = [];
        if (parentSection && currentMenuItem) {
            const findSiblings = (items, targetItem) => {
                // 直接の兄弟を探す
                ...
                for (const item of items) {
                    if (item.children) {
                        ...
                        if (childSiblings.length > 0)
                            return item.children.filter(child => child.id !== targetItem.id);
                    }
                }
                return [];
            };
        ...
        // 権限チェック
        const userRoles = authState.user?.roles || [authState.user?.role].filter(Boolean);
    ...
    const searchMenuItems = (query) => {
        ...
        return searchableItems.filter(item => item.label.toLowerCase().includes(lowercaseQuery) ||
            item.description?.toLowerCase().includes(lowercaseQuery)).slice(0, 20);
    };
    // 関連メニューアイテムの取得
    const getRelatedMenuItems = (currentItem) => {
        if (!currentItem || !navigationState.parentSection)
            return [];
        const relatedItems = [];
        const section = navigationState.parentSection;
        // 同じセクション内の他のアイテム
        const addRelatedItems = (items, level = 0) => {
            items.forEach(item => {
