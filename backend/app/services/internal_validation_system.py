...
class InternalValidationSystem:
    ...
    async def run_comprehensive_validation(self, suite_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """包括的検証実行"""
        ...
        try:
            ...
            validation_session["execution_time"] = (end_time - start_time).total_seconds()
            
            # 結果保存
            await self._save_validation_session(validation_session)
            
            logger.info(f"✅ 包括的検証完了 (スコア: {validation_session['overall_score']:.1f}/100)")
    ...
    async def _execute_validation_suite(self, suite: ValidationSuite) -> Dict[str, Any]:
        ...
        try:
            if suite.parallel_execution:
                ...
                for test in self._sort_tests_by_dependencies(suite.tests):
                    ...
                    if all(dep in executed_tests for dep in test.dependencies):
                        ...
                        logger.warning(f"テスト {test.test_id} の依存関係が満たされていません")
            ...
            if suite_result["test_results"]:
                ...
                suite_result["summary"]["overall_score"] = total_score / len(suite_result["test_results"])
    ...
    async def _execute_validation_test(self, test: ValidationTest) -> ValidationResult:
        ...
        try:
            ...
            # 結果評価
            result.success, result.score, result.recommendations = self._evaluate_test_result(
                test, actual_result
            )
    ...
    def _evaluate_test_result(self, test: ValidationTest, actual_result: Any) -> Tuple[bool, float, List[str]]:
        """テスト結果評価"""
        recommendations = []
        ...
        try:
            if test.category == ValidationCategory.API_FUNCTIONALITY:
                ...
                return self._evaluate_database_test(test, actual_result, recommendations)
            elif test.category == ValidationCategory.SECURITY_COMPLIANCE:
                return self._evaluate_security_test(test, actual_result, recommendations)
            elif test.category == ValidationCategory.PERFORMANCE_METRICS:
                return self._evaluate_performance_test(test, actual_result, recommendations)
            else:
                # デフォルト評価
    ...
    def _evaluate_api_test(self, test: ValidationTest, actual_result: Any, recommendations: List[str]) -> Tuple[bool, float, List[str]]:
        """API テスト結果評価"""
        if test.test_id == "api_health_check":
            ...
            if isinstance(actual_result, dict):
                ...
                if not success:
                    recommendations.append(f"利用可能なエンドポイントが不足しています ({available_count}/{total_count})")
            ...
            if isinstance(actual_result, dict):
                ...
                if not success:
                    recommendations.append(f"APIレスポンスの妥当性に問題があります ({valid_responses}/{total_responses})")
                
                return success, score, recommendations
        
        return False, 0.0, recommendations
    
    def _evaluate_database_test(self, test: ValidationTest, actual_result: Any, recommendations: List[str]) -> Tuple[bool, float, List[str]]:
        """データベース テスト結果評価"""
        if test.test_id == "database_connection":
            ...
            if isinstance(actual_result, dict):
                ...
                if integrity == "ok":
                    ...
                    recommendations.append(f"データベース整合性に問題があります: {integrity}")
                    return False, 0.0, recommendations
            ...
            if isinstance(actual_result, dict):
                ...
                if not success:
                    recommendations.append(f"データ一貫性に問題があります (スコア: {consistency_score})")
                
                return success, consistency_score, recommendations
        
        return False, 0.0, recommendations
    
    def _evaluate_security_test(self, test: ValidationTest, actual_result: Any, recommendations: List[str]) -> Tuple[bool, float, List[str]]:
        """セキュリティ テスト結果評価"""
        if test.test_id == "security_headers":
            if isinstance(actual_result, dict):
                ...
                if not success:
                    ...
                    recommendations.append(f"必要なセキュリティヘッダーが不足: {list(missing)}")
            ...
            if isinstance(actual_result, dict):
                ...
                if not success:
                    recommendations.append(f"認証セキュリティに改善が必要 (スコア: {auth_score})")
                
                return success, auth_score, recommendations
        
        return False, 0.0, recommendations
    
    def _evaluate_performance_test(self, test: ValidationTest, actual_result: Any, recommendations: List[str]) -> Tuple[bool, float, List[str]]:
        """パフォーマンス テスト結果評価"""
        if test.test_id == "response_time_performance":
            if isinstance(actual_result, dict):
                ...
                if avg_response_time <= threshold:
                    ...
                    score = 50.0
                    success = False
                    recommendations.append(f"レスポンス時間が遅い: {avg_response_time:.2f}秒")
                else:
                    score = 0.0
                    success = False
                    recommendations.append(f"レスポンス時間が非常に遅い: {avg_response_time:.2f}秒")
            ...
            if isinstance(actual_result, dict):
                ...
                if query_time <= threshold:
                    ...
                    score = 50.0
                    success = False
                    recommendations.append(f"データベースクエリが遅い: {query_time:.2f}秒")
                else:
                    score = 0.0
                    success = False
                    recommendations.append(f"データベースクエリが非常に遅い: {query_time:.2f}秒")
    ...
    async def _test_endpoints_availability(self, test: ValidationTest) -> Dict[str, Any]:
        """エンドポイント可用性テスト"""
        endpoints = [
            "/health",
            "/docs",
                ...
                try:
                    async with session.get(f"{self.base_url}{endpoint}") as response:
                        if response.status < 500:  # 500番台以外は利用可能とみなす
                            available_count += 1
                            endpoint_results[endpoint] = {"status": "available", "code": response.status}
                        else:
                            endpoint_results[endpoint] = {"status": "error", "code": response.status}
                except Exception as e:
    ...
    async def _test_api_response_validation(self, test: ValidationTest) -> Dict[str, Any]:
        """APIレスポンス検証テスト"""
        test_endpoints = [
            {"endpoint": "/health", "method": "GET", "expected_fields": ["status"]},
            {"endpoint": "/api/v1/incidents", "method": "GET", "expected_fields": ["items", "total"]},
            {"endpoint": "/api/v1/users", "method": "GET", "expected_fields": ["items", "total"]}
        ]
            ...
            for test_ep in test_endpoints:
                try:
                    async with session.request(
                        test_ep["method"],
                        f"{self.base_url}{test_ep['endpoint']}"
                    ) as response:
                        if response.status < 400:
                            try:
                                ...
                                has_expected_fields = all(
                                    field in data for field in test_ep["expected_fields"]
                                )
                                
                                if has_expected_fields:
                                    valid_responses += 1
                                    response_results[test_ep["endpoint"]] = {"status": "valid"}
                                else:
                                    response_results[test_ep["endpoint"]] = {
                                        "status": "invalid",
                                        "reason": "missing_expected_fields",
                                        "missing": [f for f in test_ep["expected_fields"] if f not in data]
                                    }
    ...
    async def _test_data_consistency(self, test: ValidationTest) -> Dict[str, Any]:
        ...
        try:
            ...
            if "incidents" in existing_tables and "users" in existing_tables:
                try:
                    ...
                    if orphaned_assignments > 0:
                        ref_integrity_issues.append(f"Orphaned incident assignments: {orphaned_assignments}")
            ...
            # 平均スコア計算
            avg_consistency_score = total_score / len(consistency_checks) if consistency_checks else 0
        ...
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20)) as session:
                async with session.get(f"{self.base_url}/health") as response:
                    ...
                    return {
                        "headers_present": len(present_headers) == len(required_headers),
                        "present_headers": present_headers,
                        "missing_headers": list(set(required_headers) - set(present_headers)),
                        "all_headers": headers,
                        "compliance_rate": (len(present_headers) / len(required_headers)) * 100
                    }
            ...
            with socket.create_connection((hostname, port), timeout=10) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    return {
                        ...
                        "secure": days_until_expiry > 30 and protocol in ['TLSv1.2', 'TLSv1.3']
                    }
                    
        except Exception as e:
            return {"ssl_valid": False, "error": str(e)}
    
    async def _test_authentication_security(self, test: ValidationTest) -> Dict[str, Any]:
        """認証セキュリティテスト"""
        try:
            auth_score = 100.0
            security_issues = []
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
                # ログインエンドポイントテスト
                try:
                    async with session.post(f"{self.base_url}/api/v1/auth/login") as response:
                        if response.status in [400, 422]:  # バリデーションエラーは正常
                            pass
                        elif response.status == 404:
                            security_issues.append("ログインエンドポイントが見つかりません")
                            auth_score -= 30
                        elif response.status >= 500:
                            security_issues.append("ログインエンドポイントでサーバーエラー")
                            auth_score -= 20
                except Exception as e:
                    security_issues.append(f"ログインエンドポイントテストエラー: {str(e)}")
                    auth_score -= 25
    ...
    async def _test_response_time_performance(self, test: ValidationTest) -> Dict[str, Any]:
        """レスポンス時間パフォーマンステスト"""
        try:
            ...
            return {
                ...
                "threshold": self.performance_thresholds["response_time_threshold"],
                "performance_rating": "good" if avg_response_time <= self.performance_thresholds["response_time_threshold"] else "poor"
            }
            
        except Exception as e:
            return {"avg_response_time": float('inf'), "error": str(e)}
    
    async def _test_system_resource_performance(self, test: ValidationTest) -> Dict[str, Any]:
        """システムリソースパフォーマンステスト"""
        try:
            # 複数回測定して平均を取る
            cpu_measurements = []
            ...
            return {
                ...
                "status": {
                    "cpu": "normal" if cpu_usage < self.performance_thresholds["cpu_threshold"] else "high",
                    "memory": "normal" if memory_usage < self.performance_thresholds["memory_threshold"] else "high"
                }
            }
    ...
    async def _test_database_performance(self, test: ValidationTest) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_query_time = statistics.mean(query_times) if query_times else float('inf')
            ...
            return {
                ...
                "threshold": self.performance_thresholds["database_query_threshold"],
                "performance_rating": "good" if avg_query_time <= self.performance_thresholds["database_query_threshold"] else "poor"
            }
            ...
            for suite_name, suite_result in results.items():
                ...
                if "summary" in suite_result:
                    ...
                    if summary.get("overall_score", 0) < 70:
                        recommendations.append(f"{suite_name} スイートのスコアが低い ({summary['overall_score']:.1f}/100)")
                    
                    if summary.get("failed", 0) > 0:
                        recommendations.append(f"{suite_name} で {summary['failed']} 件のテストが失敗")
    ...
    async def _save_validation_session(self, session: Dict[str, Any]):
        ...
        try:
            ...
            session_file = self.coordination_path / f"validation_session_{timestamp}.json"
    ...
    def get_validation_status(self) -> Dict[str, Any]:
        """検証状況取得"""
        try:
            latest_session = self.validation_history[-1] if self.validation_history else None
            
            return {
                "total_sessions": len(self.validation_history),
                "total_validations": len(self.validation_results),
                "latest_session": {
                    "validation_id": latest_session.get("validation_id") if latest_session else None,
                    "timestamp": latest_session.get("start_time") if latest_session else None,
                    "overall_score": latest_session.get("overall_score") if latest_session else 0,
                    "success_rate": latest_session.get("success_rate") if latest_session else 0
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/internal_validation.log"),
