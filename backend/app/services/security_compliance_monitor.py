...
class SecurityComplianceMonitor:
    ...
    async def _run_compliance_monitoring(self):
        ...
        while True:
            try:
                ...
                await asyncio.sleep(
                    self.config["compliance_check_interval_hours"] * 3600
                )
    ...
    async def _run_vulnerability_scanning(self):
        ...
        while True:
            try:
                ...
                # 脆弱性レポート生成
                await self._generate_vulnerability_report(
                    vulnerabilities, config_issues, dependency_issues
                )
    ...
    async def _analyze_log_entry(self, log_entry: str, source_file: str):
        ...
        try:
            ...
            if self.security_rules["sql_injection_detection"]["enabled"]:
                for pattern in self.security_rules["sql_injection_detection"][
                    "patterns"
                ]:
            ...
            # 認証失敗検出
            if (
                "authentication failed" in log_entry.lower()
                or "login failed" in log_entry.lower()
            ):
        ...
        try:
            for check_category, check_info in self.itsm_compliance_checks.items():
                ...
                if check_category == "incident_management":
                    compliant, evidence, remediation_steps = (
                        await self._check_incident_management()
                    )
                elif check_category == "change_management":
                    compliant, evidence, remediation_steps = (
                        await self._check_change_management()
                    )
                    ...
                    compliant, evidence, remediation_steps = (
                        await self._check_monitoring_logging()
                    )
    ...
    async def _check_incident_management(
        self,
        ...
        # インシデント記録システムの確認
        incidents_api_exists = (
            self.backend_path / "app" / "api" / "v1" / "incidents.py"
        ).exists()
        ...
        if db_file.exists():
            try:
                ...
                incidents_table_exists = any(
                    "incidents" in table[0] for table in tables
                )
                conn.close()
            except Exception:
                pass
    ...
    async def _check_change_management(self) -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        # 変更管理APIの確認
        changes_api_exists = (
            self.backend_path / "app" / "api" / "v1" / "changes.py"
        ).exists()
    ...
    async def _check_data_protection(self) -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        if not backup_dir_exists:
            remediation_steps.append(
                "バックアップディレクトリの作成とバックアップ戦略の実装"
            )

        # ログ暗号化・保護の確認
        logs_dir = self.backend_path / "logs"
        logs_protected = (
            logs_dir.exists() and oct(logs_dir.stat().st_mode)[-3:] <= "755"
        )
    ...
    async def _check_monitoring_logging(self) -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        # 監視システムの確認
        monitoring_service_exists = (
            self.backend_path / "app" / "services" / "api_error_monitor.py"
        ).exists()
        ...
        if missing_logs:
            remediation_steps.append(
                f"不足ログファイルの作成: {', '.join(missing_logs)}"
            )

        # アラート機能の確認
        alert_system_exists = (
            self.backend_path / "app" / "services" / "alert_manager.py"
        ).exists()
        ...
        compliant = (
            monitoring_service_exists and not missing_logs and alert_system_exists
        )
    ...
    async def _create_security_event(
        ...
        try:
            event_id = hashlib.md5(
                f"{threat_type.value}{source_ip}{datetime.now()}".encode()
            ).hexdigest()[:8]
            ...
            if self.config["auto_blocking"] and source_ip not in self.whitelist_ips:
                if severity in [SecurityLevel.HIGH, SecurityLevel.CRITICAL]:
                    ...
                    # ブロック解除のスケジュール
                    await self._schedule_ip_unblock(
                        source_ip, self.config["block_duration_hours"]
                    )
                    ...
                    self.blocked_ips.add(source_ip)
                    response_action = (
                        f"IP {source_ip} をブルートフォース攻撃によりブロック"
                    )
            ...
            logger.warning(
                f"セキュリティイベント作成: {threat_type.value} from {source_ip} - {description}"
            )
    ...
    async def _track_failed_authentication(self, ip: str):
        ...
        if (
            ...
            >= self.security_rules["brute_force_detection"]["max_failed_attempts"]
        ):
            await self._create_security_event(
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_events_24h": len(
                [e for e in recent_events if e.severity == SecurityLevel.CRITICAL]
            ),
            "blocked_ips_count": len(self.blocked_ips),
            "suspicious_ips_count": len(self.suspicious_ips),
            "auto_blocking_enabled": self.config["auto_blocking"],
            "last_threat_detected": max(
                [e.timestamp for e in recent_events], default=None
            ),
    ...
    def get_compliance_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_rate": (
                (len(compliant_checks) / len(recent_checks) * 100)
                if recent_checks
                else 0
            ),
            "frameworks_checked": list(set([c.framework.value for c in recent_checks])),
            "remediation_required": len(
                [c for c in recent_checks if c.remediation_required]
            ),
            "last_check_time": max(
                [c.last_checked for c in recent_checks], default=None
            ),
        }
    ...
    async def _generate_vulnerability_report(
        self, vulnerabilities, config_issues, dependency_issues
    ):
        """脆弱性レポート生成（簡易実装）"""
        pass
