...
class TestComprehensiveLoad:
    ...
    def test_spike_load_testing(self, test_config, api_headers):
        ...
        normal_success_rate = len([r for r in normal_results if r["status_code"] == 200]) / len(normal_results) * 100 if normal_results else 0
        spike_success_rate = len([r for r in spike_results if r["status_code"] == 200]) / len(spike_results) * 100 if spike_results else 0
        ...
        # Assertions
        assert normal_success_rate >= 90.0  # Normal users should maintain good success rate
        assert spike_success_rate >= 70.0   # Spike load can have lower success rate but not fail completely
    ...
    def test_endurance_testing(self, test_config, api_headers):
            ...
            while time.time() - start_time < test_duration:
                try:
                    ...
                    api_calls = [
                        ...
                        ("/incidents", {"data": [{"id": f"INC{request_count:06d}"}], "meta": {"total": 1}}),
                        (f"/incidents/INC{request_count:06d}", {"id": f"INC{request_count:06d}", "status": "new"})
                    ]
        ...
        if all_results:
            ...
            for result in all_results:
                window = int((result["timestamp"] - all_results[0]["timestamp"]) // 30)  # 30-second windows
                if window not in time_windows:
                    time_windows[window] = []
                if "response_time" in result:
                    time_windows[window].append(result["response_time"])
            
            window_avg_times = {w: statistics.mean(times) for w, times in time_windows.items() if times}
            ...
            if len(window_avg_times) > 1:
                ...
                degradation_ratio = last_window_avg / first_window_avg if first_window_avg > 0 else 1
                ...
                "data": [
                    {
                        ...
                        "description": f"This is a test incident for volume testing. Record number {i}. " * 5
    ...
    def test_scalability_testing(self, test_config, api_headers):
        ...
        for num_users in user_loads:
            ...
            if load_results:
                successful_requests = [r for r in load_results if r["status_code"] == 200]
                response_times = [r["response_time"] for r in load_results if "response_time" in r]
                
                success_rate = len(successful_requests) / len(load_results) * 100 if load_results else 0
                avg_response_time = statistics.mean(response_times) if response_times else 0
        ...
        for i, result in enumerate(scalability_results):
            print(f"- {result['users']} users: {result['success_rate']:.1f}% success, "
        ...
        if len(scalability_results) > 1:
            ...
            response_time_increase = last_response_time / first_response_time if first_response_time > 0 else 1
    ...
    def test_resource_consumption_monitoring(self, test_config, api_headers):
        ...
        if resource_measurements:
            ...
            max_memory_increase = max(m["memory_increase_mb"] for m in resource_measurements)
            ...
            assert max_memory_increase < 500  # Memory increase should be less than 500MB
    ...
    def test_recovery_after_load(self, test_config, api_headers):
        ...
        high_load_success_rate = len([r for r in high_load_results if r["status"] == "success"]) / len(high_load_results) * 100
        ...
        recovery_success_rate = len([r for r in recovery_results if r["status"] == "success"]) / len(recovery_results) * 100
        recovery_avg_response_time = statistics.mean([r["response_time"] for r in recovery_results if "response_time" in r])
