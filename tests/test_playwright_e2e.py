...
class PlaywrightTestSuite:
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Comprehensive tests completed in {total_duration:.2f} seconds"
        )
        return results
    ...
    async def test_navigation(self):
        ...
        try:
            ...
            # 主要ナビゲーション要素の確認
            nav_selectors = ["nav", "header", ".navbar", ".menu", "a[href]", "button"]
        ...
        try:
            ...
            try:
                response = await page.goto(
                    f"{self.base_url}/nonexistent-page", timeout=self.test_timeout
                )
                if response:
        ...
        try:
            ...
            for viewport in viewports:
                try:
                    ...
                    screenshot_path = (
                        self.screenshots_dir
                        / f"{test_name}_{viewport['name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                    )
                    await page.screenshot(path=screenshot_path)
                    screenshots.append(str(screenshot_path))
        ...
        try:
            ...
            accessibility_checks = [
                ...
                },
                {
                    "selector": "a",
                    "attribute": "href",
                    "description": "Links should have href",
                },
                {
                    "selector": "button",
                    "attribute": "type",
                    "description": "Buttons should have type",
                },
                {
                    "selector": "input",
                    "attribute": "label",
                    "description": "Inputs should have labels",
                },
            ]
                ...
                try:
                    ...
                    for element in elements:
                        if check["attribute"] == "label":
                            ...
                            if not attr_value:
                                errors.append(
                                    f"{check['description']}: missing {check['attribute']}"
                                )
                except Exception as e:
                    errors.append(
                        f"Accessibility check {check['selector']} failed: {e}"
                    )
    ...
    async def execute_load_test(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"Starting load test: {test_name} ({concurrent_users} users, {duration}s)"
        )
        ...
        self.logger.info(
            f"Load test {test_name} completed: {successful_requests}/{total_requests} successful, {error_rate:.1f}% error rate"
        )
        ...
        try:
            ...
            result = SecurityTestResult(
                ...
                details=(
                    ...
                    if vulnerability_found
                    else "No XSS vulnerability found"
                ),
                recommendation=(
                    "Implement proper input sanitization"
                    if vulnerability_found
                    else "XSS protection appears adequate"
                ),
                timestamp=datetime.now().isoformat(),
            )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                for payload in sql_payloads:
                    try:
                        ...
                        async with session.get(
                            f"{self.api_base_url}/search", params=params, timeout=5
                        ) as response:
            ...
            result = SecurityTestResult(
                ...
                details=(
                    ...
                    if vulnerability_found
                    else "No SQL injection vulnerability found"
                ),
                recommendation=(
                    "Use parameterized queries"
                    if vulnerability_found
                    else "SQL injection protection appears adequate"
                ),
                timestamp=datetime.now().isoformat(),
            )
    ...
    async def test_csrf_protection(self):
        ...
        try:
            ...
            # CSRFトークンの存在確認
            csrf_tokens = await page.query_selector_all(
                'input[name*="csrf"], input[name*="token"], meta[name*="csrf"]'
            )
            ...
            result = SecurityTestResult(
                ...
                details=(
                    ...
                    else "CSRF protection detected"
                ),
                recommendation=(
                    "Implement CSRF tokens for forms"
                    if not has_csrf_protection
                    else "CSRF protection appears adequate"
                ),
                timestamp=datetime.now().isoformat(),
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.base_url, timeout=10) as response:
                    ...
                    vulnerability_level = (
                        ...
                        else "medium" if missing_headers else "none"
                    )
                    ...
                    result = SecurityTestResult(
                        ...
                        recommendation=(
                            ...
                            if missing_headers
                            else "Security headers are properly configured"
                        ),
                        timestamp=datetime.now().isoformat(),
                    )
    ...
    async def test_input_validation(self):
        ...
        try:
            ...
            result = SecurityTestResult(
                ...
                status="vulnerable" if vulnerability_found else "secure",
                details=(
                    "Inadequate input validation detected"
                    if vulnerability_found
                    else "Input validation appears adequate"
                ),
                recommendation=(
                    "Implement proper input validation and sanitization"
                    if vulnerability_found
                    else "Input validation is properly implemented"
                ),
                timestamp=datetime.now().isoformat(),
            )
    ...
    async def generate_test_report(self, total_duration: float) -> Dict[str, Any]:
        ...
        # セキュリティテスト集計
        security_secure = len(
            [r for r in self.security_results if r.status == "secure"]
        )
        security_vulnerable = len(
            [r for r in self.security_results if r.status == "vulnerable"]
        )
        ...
        report = {
            ...
            "status": "completed",
            "summary": {
                "total_tests": len(self.e2e_results)
                + len(self.load_test_results)
        ...
        report_file = (
            self.reports_dir
            / f"playwright_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    print(
        f"Success Rate: {results['summary']['passed_tests'] / max(results['summary']['total_tests'], 1) * 100:.1f}%"
    )
    print("=" * 80)
