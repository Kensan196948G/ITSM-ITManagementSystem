        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if isinstance(value, str):
                    ...
                    sanitized[key] = self.sanitize_data(value) if isinstance(value, (dict, list)) else value
            return sanitized
...
class SecureExceptionHandler:
    ...
    def classify_exception(self, exc: Exception, context: SecurityContext) -> tuple[ErrorClassification, ExceptionSeverity]:
        """例外を分類"""
        exc_type = type(exc).__name__
        exc_message = str(exc).lower()
        
        # セキュリティエラーの検出
        if any(keyword in exc_message for keyword in ['unauthorized', 'forbidden', 'authentication', 'permission']):
            return ErrorClassification.SECURITY_ERROR, ExceptionSeverity.HIGH
        
        # システムエラーの検出
        if any(keyword in exc_message for keyword in ['database', 'connection', 'timeout', 'memory']):
            severity = ExceptionSeverity.CRITICAL if 'critical' in exc_message else ExceptionSeverity.HIGH
            return ErrorClassification.SYSTEM_ERROR, severity
        
        # ユーザーエラーの検出
        if any(keyword in exc_message for keyword in ['validation', 'invalid', 'bad request']):
            return ErrorClassification.USER_ERROR, ExceptionSeverity.MEDIUM
        
        # ビジネスエラーの検出
        if any(keyword in exc_message for keyword in ['business', 'rule', 'constraint']):
    ...
    def create_secure_exception(self, exc: Exception, context: SecurityContext) -> SecureException:
        """セキュアな例外オブジェクトを作成"""
        # メッセージをサニタイズ
        sanitized_message, sensitive_detected = self.sanitizer.sanitize_message(str(exc))
    ...
    def _generate_remediation_steps(self, exc: Exception, classification: ErrorClassification) -> List[str]:
        """修復手順を生成"""
        steps_map = {
        ...
        return steps_map.get(classification, ["Review error details", "Check logs", "Contact support"])
    
    def _identify_affected_resources(self, exc: Exception, context: SecurityContext) -> List[str]:
        """影響を受けるリソースを特定"""
        resources = []
    ...
    async def handle_exception(self, exc: Exception, context: SecurityContext) -> SecureException:
        """例外を安全に処理"""
        ...
        if secure_exc.severity in [ExceptionSeverity.CRITICAL, ExceptionSeverity.EMERGENCY]:
...
def rate_limit(max_requests: int = 100, window_seconds: int = 60):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            identifier = kwargs.get('ip_address') or kwargs.get('user_id') or 'anonymous'
            
            access_control = AccessControlManager()
            if not access_control.check_rate_limit(identifier, max_requests, window_seconds):
...
async def secure_api_endpoint(user_id: str, ip_address: str, permissions: List[str], 
