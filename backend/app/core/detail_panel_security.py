...
class DetailPanelSecurityManager:
    ...
    def check_entity_access(
        ...
        try:
            # キャッシュキーを生成
            cache_key = (
                f"entity_access:{user_id}:{entity_type}:{entity_id}:{access_type}"
            )
    ...
    def _check_entity_permission(
        self, user_id: UUID, entity_type: str, entity_id: UUID, access_type: str
    ) -> bool:
        """具体的な権限チェック実装"""
    ...
    def _check_incident_access(
        self, user: Any, incident_id: UUID, access_type: str
    ) -> bool:
        """インシデントアクセス権限チェック"""
        from app.models.incident import Incident
    ...
    def _check_user_access(
        self, user: Any, target_user_id: UUID, access_type: str
    ) -> bool:
        """ユーザーアクセス権限チェック"""
        # 自分自身の情報は常にアクセス可能
...
class RateLimiter:
    ...
    def check_rate_limit(self, user_id: UUID, endpoint: str) -> bool:
        ...
        # リクエスト数をインクリメント
        cache_manager.set(cache_key, current_requests + 1, expire=self.time_window)
...
class DataSanitizer:
    ...
    def _remove_sensitive_fields(
        data: Union[Dict, List, Any], sensitive_fields: set
    ) -> Union[Dict, List, Any]:
        """機密フィールドの除去"""
