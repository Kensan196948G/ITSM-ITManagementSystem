...
class QualityGateChecker:
    ...
    def analyze_test_reports(self) -> Dict[str, Any]:
        ...
        for report_file in self.reports_dir.glob("*-report.json"):
            if report_file.exists():
                try:
                    ...
                    # 全体メトリクスに加算
                    analysis["overall_metrics"]["total_tests"] += suite_metrics.get("total", 0)
                    analysis["overall_metrics"]["passed_tests"] += suite_metrics.get("passed", 0)
                    analysis["overall_metrics"]["failed_tests"] += suite_metrics.get("failed", 0)
                    analysis["overall_metrics"]["skipped_tests"] += suite_metrics.get("skipped", 0)
        ...
        if total_tests > 0:
            ...
            analysis["overall_metrics"]["success_rate"] = (passed_tests / total_tests) * 100
    ...
    def _extract_test_metrics(self, data: Dict[str, Any], suite_name: str) -> Dict[str, Any]:
        """テストメトリクスを抽出"""
        metrics = {
    ...
    def _evaluate_quality_gates(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # テスト成功率チェック
        success_rate_passed = success_rate >= self.quality_criteria["min_test_success_rate"]
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if failed_tests > 0:
            recommendations.append(f"失敗したテスト {failed_tests} 件を修正してください")
        
        if success_rate < self.quality_criteria["min_test_success_rate"]:
            recommendations.append(f"テスト成功率 {success_rate:.1f}% を {self.quality_criteria['min_test_success_rate']:.1f}% 以上に改善してください")
        
        # パフォーマンス問題の検出
        for suite_name, suite_data in analysis["test_suites"].items():
            if suite_data.get("duration", 0) > 60:  # 60秒超過
                recommendations.append(f"{suite_name} テストスイートの実行時間 ({suite_data['duration']:.1f}s) を短縮してください")
    ...
    def print_summary(self, analysis: Dict[str, Any]):
        ...
        for suite_name, suite_data in analysis["test_suites"].items():
            ...
            print(f"  {suite_name}: {suite_data['passed']}/{suite_data['total']} ({success_rate:.1f}%)")
