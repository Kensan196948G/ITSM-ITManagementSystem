...
class CICDSecurityManager:
    ...
    def verify_github_webhook(self, payload: bytes, signature: str) -> bool:
        ...
        expected_signature = (
            "sha256="
            + hashlib.sha256(GITHUB_WEBHOOK_SECRET.encode() + payload).hexdigest()
        )
    ...
    def check_rate_limit(
        ...
        if identifier not in RATE_LIMIT_STORAGE:
            RATE_LIMIT_STORAGE[identifier] = {"requests": [], "blocked_until": None}
...
async def verify_api_authentication(
    ...
    try:
        ...
        raise ITSMException(
            status_code=500, error_code="AUTHENTICATION_ERROR", message="認証処理エラー"
        )


async def check_ci_permissions(
    auth_data: Dict[str, Any] = Depends(verify_api_authentication),
) -> Dict[str, Any]:
    """CI/CD操作権限をチェック"""
...
async def check_rate_limit_middleware(request: Request) -> None:
    ...
    if security_manager.check_ip_blocked(client_ip):
        security_manager.log_security_event(
            "blocked_ip_request", {"ip": client_ip, "path": str(request.url.path)}
        )
    ...
    if not security_manager.check_rate_limit(client_ip):
        security_manager.log_security_event(
            "rate_limit_exceeded", {"ip": client_ip, "path": str(request.url.path)}
        )
...
async def validate_github_webhook(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        if not security_manager.verify_github_webhook(body, signature):
            security_manager.log_security_event(
                "webhook_verification_failed",
                {"ip": request.client.host, "signature_prefix": signature[:20]},
            )
