...
class GitHubActionsErrorResolver:
    ...
    def get_all_failed_runs(self) -> List[Dict]:
        ...
        try:
            result = subprocess.run(['gh', 'run', 'list', '--status', 'failure', '--limit', '50'], 
    ...
    def get_error_details(self, run_id: str) -> Tuple[str, str]:
        ...
        try:
            # 失敗したログのみ取得
            result = subprocess.run(['gh', 'run', 'view', run_id, '--log-failed'], 
            ...
            if result.returncode == 0:
                ...
                # ログ取得失敗の場合、基本情報を取得
                result = subprocess.run(['gh', 'run', 'view', run_id], 
                                      capture_output=True, text=True, timeout=30)
                return "ログ取得失敗", result.stdout if result.returncode == 0 else "詳細取得失敗"
    ...
    def _extract_error_summary(self, error_log: str) -> str:
        ...
        error_indicators = ['error', 'failed', 'Error', 'FAILED', '×', '❌', 'Exception']
    ...
    def generate_fix_prompt(self, run_info: Dict, error_summary: str, error_log: str) -> str:
        """修復用プロンプトを生成"""
...
"""
        ...
        logger.info(f"最終統計: {self.loop_count}ループ, {self.total_errors_resolved}エラー解決")
