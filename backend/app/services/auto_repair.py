    ...
    def _extract_stack_trace(
        ...
        for i in range(start_idx, end_idx):
            ...
            if any(
                ...
                for keyword in ["traceback", "file ", "line ", "error"]
            ):
...
class AutoFixer:
    ...
    def _apply_sqlalchemy_fixes(self, content: str, error: DetectedError) -> str:
        ...
        patterns = [
            ...
            # 外部キー制約の追加
            (
                r"(\w+_id) = Column\(Integer\)",
                r'\1 = Column(Integer, ForeignKey("\1.id"))',
            ),
            # インポートの追加
            (
    ...
    def _apply_fastapi_fixes(self, content: str, error: DetectedError) -> str:
        ...
        fixes = [
            ...
            (
                ...
                r'@router.\1("\2", response_model=dict)',
            ),
            # 例外処理の追加
            (
                "raise HTTPException",
    ...
    async def _fix_pydantic_validation(
    ...
    async def _fix_database_connection(
        self, error: DetectedError
    ) -> Optional[FixAction]:
        """データベース接続エラーの修復"""
        return FixAction(
            ...
            validation_tests=[
                "python -c 'from app.db.base import SessionLocal; SessionLocal()'"
            ],
            status=FixStatus.PENDING,
            applied_at=None,
            result=None,
    ...
    async def _fix_cors_configuration(
        self, error: DetectedError
    ) -> Optional[FixAction]:
        """CORS設定エラーの修復"""
...
"""
    ...
    def _fix_syntax_line(self, line: str, error: DetectedError) -> str:
        ...
        fixes = [
            ...
            # コロンの追加
            (
                r"(if|elif|else|try|except|finally|for|while|def|class|with)\s+([^:]+)$",
                r"\1 \2:",
            ),
            # 括弧の閉じ忘れ
            (r"\([^)]*$", line + ")"),
            ...
            (
                r"^(\s*)(.+)",
                lambda m: "    " + m.group(2) if len(m.group(1)) % 4 != 0 else line,
            ),
        ]
...
class AutoRepairSystem:
    ...
    async def run_once(self) -> Dict[str, Any]:
        ...
        for error in detected_errors:
            if error.severity in ["critical", "high"]:
                ...
                if fix_action:
                    ...
                    if success:
                        self.logger.info(
                            f"修復成功: {error.error_type.value} - {error.message[:50]}..."
                        )
                    else:
                        self.logger.error(
                            f"修復失敗: {error.error_type.value} - {error.message[:50]}..."
                        )
        ...
        result = {
            ...
            "fixes_successful": len(
                [f for f in fixes_applied if f.status == FixStatus.COMPLETED]
            ),
        ...
        self.logger.info(
            f"修復サイクル完了: {result['fixes_successful']}/{result['fixes_attempted']} 修復成功"
        )
        return result
    ...
    async def _save_fixes(self, fixes: List[FixAction]):
        ...
        fix_data = {
            ...
            "success_rate": (
                len(successful_fixes) / len(all_fixes) if all_fixes else 0.0
            ),
