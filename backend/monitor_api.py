...
def monitor(interval: int, duration: int):
    ...
    async def run_monitoring():
        if duration > 0:
            ...
            monitoring_task = asyncio.create_task(api_monitor.start_monitoring(interval))
    ...
    async def run_log_analysis():
        ...
        if new_errors > 0:
            ...
            for error in api_monitor.errors[-min(5, new_errors):]:
                ...
                click.echo(f"  {severity_emoji[error.severity]} {error.error_type} - {error.endpoint}")
...
def fix():
    ...
    async def run_fixes():
        ...
        # 結果表示
        successful_fixes = [e for e in api_monitor.errors if e.fix_attempted and e.fix_successful]
        failed_fixes = [e for e in api_monitor.errors if e.fix_attempted and not e.fix_successful]
...
@cli.command()
@click.option('--category', type=click.Choice([cat.value for cat in ErrorCategory]), help='エラーカテゴリでフィルタ')
@click.option('--severity', type=click.Choice([sev.value for sev in ErrorSeverity]), help='重要度でフィルタ')
...
def errors(category: str, severity: str, limit: int):
    ...
    for i, error in enumerate(errors, 1):
        ...
        fix_status = "✅" if error.fix_successful else "❌" if error.fix_attempted else "⏳"
