...
async def get_dashboard_metrics(
    ...
    try:
        ...
        # 基本メトリクス取得
        total_tickets = db.query(func.count(Incident.id)).filter(
            Incident.deleted_at.is_(None)
        ).scalar() or 0
        
        open_tickets = db.query(func.count(Incident.id)).filter(
            and_(
                Incident.deleted_at.is_(None),
        ...
        resolved_tickets = db.query(func.count(Incident.id)).filter(
            and_(
                Incident.deleted_at.is_(None),
        ...
        # 期限超過チケット数
        overdue_tickets = db.query(func.count(Incident.id)).filter(
            and_(
                Incident.deleted_at.is_(None),
                Incident.status.in_([IncidentStatus.NEW, IncidentStatus.ASSIGNED, IncidentStatus.IN_PROGRESS]),
        ...
        # SLA遵守率計算
        sla_compliant = db.query(func.count(Incident.id)).filter(
            and_(
                Incident.deleted_at.is_(None),
                Incident.status == IncidentStatus.RESOLVED,
                Incident.resolved_at >= start_date,
        ...
        sla_compliance_rate = (sla_compliant / total_resolved * 100) if total_resolved > 0 else 100
        ...
        # 最近のチケット取得（最新10件）
        recent_tickets_query = db.query(Incident).filter(
            Incident.deleted_at.is_(None)
        ...
        for i in range(min(days, 30)):
            ...
            created_count = db.query(func.count(Incident.id)).filter(
                and_(
        ...
        category_stats = db.query(
            func.coalesce(Incident.category_id, text("'uncategorized'")).label('category'),
        ...
        ).group_by(text('category')).all()
        
        category_data = [{"category": str(stat.category), "count": stat.count} for stat in category_stats]
        
        # 担当者別パフォーマンス（上位10名）
        assignee_performance = db.query(
            User.full_name,
            func.count(Incident.id).label('assigned_count'),
            func.count(case([(Incident.status == IncidentStatus.RESOLVED, 1)])).label('resolved_count'),
            func.avg(
                case([
                    (Incident.resolved_at.is_not(None),
                     func.extract('epoch', Incident.resolved_at - Incident.created_at) / 3600)
            ...
            Incident, User.id == Incident.assignee_id
        ).filter(
            and_(
                Incident.deleted_at.is_(None),
                Incident.created_at >= start_date
...
async def get_trend_chart_data(
    ...
    metric: str = Query("tickets", pattern="^(tickets|resolution_time|sla)$", description="メトリクス種別"),
    db: Session = Depends(get_db),
    current_user_id: UUID = Depends(get_current_user_id)
) -> Dict[str, Any]:
    """トレンドチャートデータを取得する"""
    ...
    try:
        ...
        if metric == "tickets":
            ...
            for row in query.all():
                compliance_rate = (row.compliant / row.total * 100) if row.total > 0 else 100
                trend_data.append({
                    "period": row.period.isoformat(),
...
async def get_distribution_chart_data(
    chart_type: str = Query("priority", pattern="^(priority|status|category|assignee)$", description="分布種別"),
    ...
    try:
        ...
        if chart_type == "priority":
            ...
            query = base_query.join(
                Incident.category, isouter=True
            ).with_entities(
                func.coalesce(Incident.category_id, text("'uncategorized'")).label('category'),
                func.count(Incident.id).label('count')
            ...
            for row in query.all():
                category_label = "未分類" if str(row.category) == "uncategorized" else str(row.category)
                distribution_data.append({
            ...
            query = base_query.join(
                User, Incident.assignee_id == User.id, isouter=True
            ).with_entities(
                func.coalesce(User.full_name, text("'未割当'")).label('assignee'),
                func.count(Incident.id).label('count')
            ).group_by(text('assignee')).order_by(func.count(Incident.id).desc()).limit(10)
