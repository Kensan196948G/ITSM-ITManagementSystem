...
<body>
    ...
    async def generate_report(
        ...
        try:
            ...
            collected_data = await self._collect_data(
                start_time, end_time, api_monitor_data, enhanced_monitor_data, repair_engine_data
            )
            
            # レポートタイプに応じた処理
            if config.report_type == ReportType.SUMMARY:
                report_data = await self._generate_summary_report(collected_data, config)
            elif config.report_type == ReportType.DETAILED:
                report_data = await self._generate_detailed_report(collected_data, config)
            elif config.report_type == ReportType.PERFORMANCE:
                report_data = await self._generate_performance_report(collected_data, config)
            elif config.report_type == ReportType.SECURITY:
                report_data = await self._generate_security_report(collected_data, config)
            elif config.report_type == ReportType.COMPLIANCE:
                report_data = await self._generate_compliance_report(collected_data, config)
            elif config.report_type == ReportType.TREND_ANALYSIS:
                report_data = await self._generate_trend_analysis_report(collected_data, config)
            elif config.report_type == ReportType.REPAIR_ANALYSIS:
                report_data = await self._generate_repair_analysis_report(collected_data, config)
            else:
            ...
            if config.include_recommendations:
                recommendations = await self._generate_recommendations(collected_data, config.report_type)
    ...
    async def _collect_error_logs(self, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        """エラーログを収集"""
        try:
            error_logs = []
            log_files = [
    ...
    async def _generate_summary_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """概要レポートを生成"""
        ...
        detailed_data = {
            ...
            "エラー統計": {
                ...
                "Critical エラー": len([e for e in data.get("error_logs", []) if "critical" in e.get("message", "").lower()]),
                "エラー率": "計算中..."
            }
        }
    ...
    async def _generate_detailed_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """詳細レポートを生成"""
        start_time, end_time = data["time_range"]
        ...
        summary = {
            ...
            "API接続エラー": len([e for e in api_errors if "connection" in str(e).lower()]),
        ...
        detailed_data = {
            ...
            "パフォーマンス分析": self._analyze_performance(data.get("performance_data", {})),
    ...
    async def _generate_performance_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """パフォーマンスレポートを生成"""
        start_time, end_time = data["time_range"]
        ...
        summary = {
            ...
            "システム稼働日数": self._calculate_uptime_days(system_metrics.get('boot_time')),
            "API応答時間": "計算中..."
        }
        ...
        detailed_data = {
            ...
            "最適化推奨事項": self._generate_performance_recommendations(system_metrics)
        }
    ...
    async def _generate_security_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """セキュリティレポートを生成"""
        start_time, end_time = data["time_range"]
        error_logs = data.get("error_logs", [])
        ...
        summary = {
            ...
            "認証失敗数": len([e for e in security_errors if "auth" in e.get("message", "").lower()]),
            "不正アクセス試行": len([e for e in security_errors if "unauthorized" in e.get("message", "").lower()]),
            "SQL注入検出": len([e for e in security_errors if "sql" in e.get("message", "").lower()]),
            "XSS攻撃検出": len([e for e in security_errors if "xss" in e.get("message", "").lower()]),
            "セキュリティスコア": self._calculate_security_score(security_errors)
        }
        
        detailed_data = {
            "セキュリティイベント": [self._format_security_event(e) for e in security_errors],
            "脅威分析": self._analyze_threats(security_errors),
            "セキュリティポリシー準拠": self._check_security_compliance(),
            "推奨セキュリティ対策": self._generate_security_recommendations(security_errors)
        }
    ...
    async def _generate_compliance_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """コンプライアンスレポートを生成"""
        start_time, end_time = data["time_range"]
        ...
        summary = {
            ...
            "監査ログ整合性": "良好" if compliance_checks.get('audit_integrity', True) else "要確認",
            "データ保護準拠": "準拠" if compliance_checks.get('data_protection', True) else "非準拠",
            "バックアップ整合性": "良好" if compliance_checks.get('backup_integrity', True) else "要確認"
        }
    ...
    async def _generate_trend_analysis_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """トレンド分析レポートを生成"""
        start_time, end_time = data["time_range"]
        api_errors = data.get("api_monitor", {}).get("errors", [])
    ...
    async def _generate_repair_analysis_report(self, data: Dict[str, Any], config: ReportConfig) -> ReportData:
        """修復分析レポートを生成"""
        start_time, end_time = data["time_range"]
    ...
    async def _generate_charts(self, data: Dict[str, Any], report_type: ReportType) -> List[Dict[str, Any]]:
        """チャートを生成"""
        charts = []
    ...
    async def _create_summary_charts(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """概要チャートを作成"""
        charts = []
    ...
    async def _create_performance_charts(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """パフォーマンスチャートを作成"""
        charts = []
    ...
    async def _create_trend_charts(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        while current_time < end_time:
            ...
            error_counts.append(len([e for e in data.get("api_monitor", {}).get("errors", []) 
                                   if hasattr(e, 'timestamp') and e.timestamp.hour == current_time.hour]))
        ...
        if time_points:
            ...
            fig.update_layout(title="エラー発生トレンド", xaxis_title="時刻", yaxis_title="エラー数")
    ...
    async def export_report(self, report_data: ReportData, format: ReportFormat, output_path: Path = None) -> str:
        """レポートをエクスポート"""
        try:
            if not output_path:
    ...
    def _classify_errors(self, errors: List) -> Dict[str, int]:
        ...
        for error in errors:
            ...
            elif any(keyword in error_str for keyword in ['performance', 'slow', 'memory']):
                classification["Performance"] += 1
            elif any(keyword in error_str for keyword in ['security', 'attack', 'unauthorized']):
    ...
    def _analyze_time_series(self, errors: List, start_time: datetime, end_time: datetime) -> Dict[str, Any]:
        """時系列分析"""
        # 簡易的な時系列分析
        hourly_counts = {}
        ...
        for error in errors:
            if hasattr(error, 'timestamp'):
                hourly_counts[error.timestamp.hour] = hourly_counts.get(error.timestamp.hour, 0) + 1
    ...
    def _is_security_related(self, error: Dict[str, Any]) -> bool:
        ...
        security_keywords = ['auth', 'unauthorized', 'forbidden', 'attack', 'injection', 'xss', 'csrf']
    ...
    def _format_security_event(self, error: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity": "HIGH" if "critical" in error.get("message", "").lower() else "MEDIUM",
    ...
    def _analyze_trends(self, errors: List, start_time: datetime, end_time: datetime) -> Dict[str, Any]:
        return {
    ...
    def _analyze_repair_effectiveness(self, repair_data: Dict[str, Any]) -> Dict[str, Any]:
        return {
    ...
    def _generate_performance_recommendations(self, system_metrics: Dict[str, Any]) -> List[str]:
        recommendations = []
        if system_metrics.get('cpu_percent', 0) > 80:
            recommendations.append("CPU使用率が高いため、プロセス最適化を検討してください")
        if system_metrics.get('memory_percent', 0) > 85:
            recommendations.append("メモリ使用率が高いため、メモリリークの確認を推奨します")
