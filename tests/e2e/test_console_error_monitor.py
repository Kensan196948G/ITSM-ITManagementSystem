...
class ConsoleErrorClassifier:
    ...
    def __init__(self):
        self.error_patterns = {
            "javascript_error": [
                ...
                "Uncaught", "Cannot read property", "Cannot read properties",
                "is not a function", "is not defined", "null is not an object"
            ],
            "react_error": [
                "React", "Warning: ", "Error: ", "Unhandled error", 
                "Component", "Hook", "render", "useState", "useEffect",
                "Minified React error", "ReactDOM"
            ],
            "network_error": [
                "Failed to fetch", "ERR_NETWORK", "ERR_INTERNET_DISCONNECTED",
                "ERR_CONNECTION_REFUSED", "ERR_NAME_NOT_RESOLVED",
                "404", "500", "502", "503", "504", "CORS", "XMLHttpRequest"
            ],
            "security_error": [
    ...
    def classify_error(self, message: str, url: str = "") -> Dict[str, Any]:
        ...
        if any(indicator in message_lower for indicator in [
            "api", "server", "backend", "database", "401", "403", "500", "502", "503", "504"
        ]):
            source = "backend"
            assign_to = "ITSM-DevAPI"
        elif any(indicator in url.lower() for indicator in [
            "api", "/v1/", "/auth", "/incidents", "/problems", "/changes"
        ]):
            source = "backend"
            assign_to = "ITSM-DevAPI"
        
        # 重要度判定
        if any(critical in message_lower for critical in [
            "uncaught", "fatal", "critical", "cannot read", "is not a function",
            "500", "502", "503", "504", "network error"
        ]):
            severity = "high"
        elif any(warning in message_lower for warning in [
            "warning", "deprecated", "404", "401", "403"
        ]):
            severity = "low"
...
class ConsoleErrorMonitor:
    ...
    def _create_error_info(self, message: str, error_type: str, 
                          file: str = "", line: int = 0, 
                          stack_trace: str = "",
...
class TestConsoleErrorMonitor:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                try:
                    ...
                    # アクセス失敗もエラーとして記録
                    monitor.errors.append(monitor._create_error_info(
    ...
    async def _perform_ui_interactions(self, page: Page, monitor: ConsoleErrorMonitor) -> None:
        """UI操作を実行してエラーを誘発
