...
class DatabaseErrorRepairSystem:
    ...
    def __init__(self):
        ...
        self.backup_dir = (
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/backups"
        )
    ...
    async def start_infinite_monitoring(self):
        ...
        while self.monitoring_active:
            try:
                monitoring_cycle += 1
                self.logger.info(
                    f"Starting database monitoring cycle #{monitoring_cycle}"
                )
                ...
                if self._has_critical_database_issues(
                ...
                self.logger.error(
                    f"Database monitoring cycle #{monitoring_cycle} failed: {str(e)}"
                )
                ...
                if consecutive_failures >= max_consecutive_failures:
                    self.logger.critical(
                        f"Too many consecutive monitoring failures: {consecutive_failures}"
                    )
    ...
    async def check_connection_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status.update(
                {
                    ...
                    "details": {
                        "connection_time_status": self._evaluate_connection_time(
                            connection_time
                        ),
            ...
            if connection_time > self.performance_thresholds["connection_time_warning"]:
                status["details"][
                    "connection_warning"
                ] = f"Slow connection time: {connection_time:.2f}s"

            if test_query_time > self.performance_thresholds["query_time_warning"]:
                status["details"][
                    "query_warning"
                ] = f"Slow query execution: {test_query_time:.2f}s"

            self.logger.info(
                f"Database connection health check passed: {connection_time:.3f}s connection, {test_query_time:.3f}s query"
            )
    ...
    async def check_database_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                if not integrity_ok:
                    integrity_status["issues_found"].append(
                ...
                integrity_status["checks"]["foreign_key_check"] = {
                    ...
                    "violations": [
                        dict(row._mapping) for row in fk_violations[:10]
                    ],  # Limit to 10
                }
            ...
            self.logger.info(
                f"Database integrity check completed: {integrity_status['overall_status']}"
            )
            ...
            integrity_status.update(
                {
                    ...
                    "error": str(e),
                    "checks": {
                        "integrity_check": {"status": "failed", "error": str(e)}
                    },
                }
            )
    ...
    async def monitor_database_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                for query_name, query in test_queries:
                    try:
                        ...
                        if (
                            execution_time
                            > self.performance_thresholds["query_time_warning"]
                        ):
                ...
                if os.path.exists(self.db_path):
                    ...
                    if (
                        db_size_mb
                        > self.performance_thresholds["database_size_warning"]
                    ):
                        performance_metrics["optimization_suggestions"].append(
                ...
                try:
                    ...
                    performance_metrics["database_stats"]["total_indexes"] = len(
                        indexes
                    )
                ...
                slow_queries = sum(
                    1
                    for q in performance_metrics["query_performance"].values()
                ...
                if failed_queries > 0:
                    ...
                    slow_queries > 2
                    or len(performance_metrics["optimization_suggestions"]) > 3
                ):
            ...
            self.logger.info(
                f"Database performance monitoring completed: {performance_metrics['overall_performance']}"
            )

        except Exception as e:
            performance_metrics.update(
                {"overall_performance": "error", "error": str(e)}
            )
    ...
    async def monitor_connection_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(5):
                try:
                    ...
                    connection_tests.append(
                        {"test": i + 1, "time": connection_time, "status": "success"}
                    )
                except Exception as e:
                    connection_tests.append(
                        {"test": i + 1, "status": "failed", "error": str(e)}
                    )

            successful_connections = sum(
                1 for test in connection_tests if test["status"] == "success"
            )
            ...
            if successful_connections == len(connection_tests):
                ...
                pool_status["issues"].append(
                    f"Some connections failed: {len(connection_tests) - successful_connections}"
                )
            else:
            ...
            if (
                avg_connection_time
                > self.performance_thresholds["connection_time_warning"]
            ):
                pool_status["issues"].append(
                    f"Slow average connection time: {avg_connection_time:.2f}s"
                )

            self.logger.info(
                f"Connection pool monitoring completed: {pool_status['status']}"
            )

        except Exception as e:
            pool_status.update({"status": "error", "error": str(e)})
        ...
        try:
            ...
            if os.path.exists(self.db_path):
                ...
                try:
                    ...
                    if "database is locked" in str(e).lower():
                        ...
                        lock_status["lock_details"].append(
                            {"type": "database_lock", "message": str(e)}
                        )
                ...
                if existing_lock_files:
                    lock_status["lock_details"].append(
                        {"type": "lock_files", "files": existing_lock_files}
                    )

            self.logger.info(
                f"Transaction lock check completed: {lock_status['status']}"
            )

        except Exception as e:
            lock_status.update({"status": "error", "error": str(e)})
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                try:
                    ...
                    if not has_stats:
                        optimization_status["recommendations"].append(
                            "Database statistics missing - run ANALYZE"
                        )

                except Exception:
                    optimization_status["recommendations"].append(
                        "Unable to check database statistics"
                    )
                ...
                if os.path.exists(self.db_path):
                    ...
                    if file_size > 100 * 1024 * 1024:  # 100MB
                        optimization_status["recommendations"].append(
                            "Large database - consider regular VACUUM"
                        )
            ...
            self.logger.info(
                f"Database optimization check completed: {optimization_status['status']}"
            )

        except Exception as e:
            optimization_status.update({"status": "error", "error": str(e)})
    ...
    async def check_backup_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(self.backup_dir):
                ...
                if backup_files:
                    ...
                    if backup_age_hours < 24:
                        ...
                        backup_status["recommendations"].append(
                            "Backup is getting old - consider creating new backup"
                        )
                    else:
                        backup_status["status"] = "stale"
                        backup_status["recommendations"].append(
                            "Backup is stale - immediate backup recommended"
                        )

                else:
                    backup_status["status"] = "no_backups"
                    backup_status["recommendations"].append(
                        "No backups found - create backup immediately"
                    )
            ...
            self.logger.info(
                f"Backup status check completed: {backup_status['status']}"
            )

        except Exception as e:
            backup_status.update({"status": "error", "error": str(e)})

            self.logger.error(f"Backup status check failed: {str(e)}")

        return backup_status

    def _has_critical_database_issues(
        self, connection_status, integrity_status, performance_metrics
    ) -> bool:
        """クリティカルなデータベース問題の検出"""
        # Connection failures
    ...
    async def execute_auto_repair(self):
        ...
        try:
            ...
            repair_success = all(
                action.get("success", False) for action in repair_actions
            )
            ...
            self.logger.info(
                f"Database auto repair completed: {'success' if repair_success else 'partial failure'}"
            )
    ...
    async def _create_emergency_backup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if backup_size == source_size:
                ...
                result["details"][
                    "error"
                ] = f"Backup size mismatch: {backup_size} vs {source_size}"
    ...
    async def _repair_database_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                integrity_issues = [
                    row[0] for row in integrity_result.fetchall() if row[0] != "ok"
                ]
            ...
            self.logger.info(
                f"Database integrity repair: {'success' if result['success'] else 'partial'}"
            )
    ...
    async def _reset_connection_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result.update({"success": False, "details": {"error": str(e)}})
    ...
    async def _optimize_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                self.logger.info(
                    f"Database optimization completed: {size_reduction / (1024 * 1024):.2f} MB saved"
                )
            ...
            for lock_file in lock_files:
                if os.path.exists(lock_file):
                    try:
                        ...
                        self.logger.warning(
                            f"Could not remove lock file {lock_file}: {str(e)}"
                        )
        ...
        try:
            ...
            if connection_test.get("connection_status") == "healthy":
                ...
                if integrity_test.get("overall_status") in ["healthy", "warning"]:
                    result.update(
                        {
                            "success": True,
                            "details": {
                                "connection_status": connection_test.get(
                                    "connection_status"
                                ),
                                "integrity_status": integrity_test.get(
                                    "overall_status"
                                ),
                                "connection_time": connection_test.get(
                                    "connection_time"
                                ),
                    ...
                    result["details"][
                        "error"
                    ] = f"Integrity test failed: {integrity_test.get('overall_status')}"
            else:
                result["details"][
                    "error"
                ] = f"Connection test failed: {connection_test.get('connection_status')}"

            self.logger.info(
                f"Post-repair database test: {'passed' if result['success'] else 'failed'}"
            )
    ...
    async def emergency_database_recovery(self):
        ...
        try:
            ...
            if os.path.exists(self.db_path):
                try:
                    ...
                    recovery_actions.append(
                        f"Emergency backup: {emergency_backup.get('success', False)}"
                    )
                except Exception:
            ...
            if os.path.exists(self.backup_dir):
                ...
                if backup_files:
                    ...
                    try:
                        ...
                        recovery_actions.append(
                            f"Database restored from: {os.path.basename(latest_backup)}"
                        )
                    except Exception as e:
            ...
            recovery_actions.append(
                f"Post-recovery test: {test_result.get('connection_status', 'unknown')}"
            )

            self.logger.info(
                f"Emergency database recovery completed: {', '.join(recovery_actions)}"
            )
    ...
    async def generate_database_metrics_report(
        self,
        connection_status,
        integrity_status,
        performance_metrics,
        ...
        try:
            ...
            self.logger.info(
                f"Database metrics report generated: {metrics_report['overall_database_health']}"
            )

        except Exception as e:
            self.logger.error(f"Failed to generate database metrics report: {str(e)}")

    def _calculate_overall_database_health(
        self, connection_status, integrity_status, performance_metrics, backup_status
    ) -> str:
        """総合データベース健康状態計算"""
        # Critical issues
        if connection_status.get("connection_status") == "error":
        ...
        # Warning issues
        warning_conditions = [
            connection_status.get("connection_time", 0)
            > self.performance_thresholds["connection_time_warning"],
        ...
        try:
                    ...
                    if filename.endswith(".db"):
                        ...
                        if file_mtime < cutoff_date:
                            try:
                                ...
                                self.logger.warning(
                                    f"Could not remove old backup {filename}: {str(e)}"
                                )
            ...
            if os.path.exists(log_dir):
                ...
                for filename in os.listdir(log_dir):
                    if filename.startswith("database_report_") and filename.endswith(
                        ".json"
                    ):
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/database_monitor.log"
            ),
