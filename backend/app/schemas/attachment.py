...
class AttachmentBase(BaseModel):
    ...
    tags: Optional[str] = Field(
        None, max_length=500, description="タグ（カンマ区切り）"
    )
    is_public: bool = Field(False, description="公開設定")
    access_level: str = Field(
        "private", pattern="^(private|internal|public)$", description="アクセスレベル"
    )
...
class AttachmentUploadResponse(BaseModel):
    ...
    uploaded_files: List[AttachmentResponse] = Field(
...
class AttachmentUpdateRequest(BaseModel):
    ...
    access_level: Optional[str] = Field(
        None, pattern="^(private|internal|public)$", description="アクセスレベル"
    )
...
class AttachmentSearchRequest(BaseModel):
    ...
    uploaded_from: Optional[datetime] = Field(
        None, description="アップロード日時（開始）"
    )
    uploaded_to: Optional[datetime] = Field(
        None, description="アップロード日時（終了）"
    )
    search_query: Optional[str] = Field(None, description="ファイル名・説明文検索")
    tags: Optional[List[str]] = Field(None, description="タグフィルター")
    uploaded_by: Optional[List[UUID]] = Field(
        None, description="アップロード者フィルター"
    )
    scan_status: Optional[List[str]] = Field(
        None, description="スキャンステータスフィルター"
    )
...
class AttachmentBulkActionRequest(BaseModel):
    ...
    attachment_ids: List[UUID] = Field(
        ..., min_items=1, description="対象添付ファイルID一覧"
    )
    action: str = Field(
        ...,
        pattern="^(delete|restore|update_tags|change_access)$",
        description="実行アクション",
    )
    parameters: Optional[Dict[str, Any]] = Field(
        None, description="アクション固有パラメータ"
    )
...
class AttachmentStatisticsResponse(BaseModel):
    ...
    by_resource_type: Dict[str, Dict[str, Any]] = Field(
        ..., description="リソースタイプ別統計"
    )
    by_content_type: Dict[str, Dict[str, Any]] = Field(
        ..., description="コンテンツタイプ別統計"
    )
...
class AttachmentQuotaResponse(BaseModel):
    ...
    next_cleanup_date: Optional[datetime] = Field(
        None, description="次回クリーンアップ予定日"
    )
...
class AttachmentCleanupRequest(BaseModel):
    ...
    cleanup_type: str = Field(
        ...,
        pattern="^(orphaned|deleted|old_versions|large_files)$",
        description="クリーンアップ種別",
    )
    dry_run: bool = Field(True, description="テスト実行フラグ")
    parameters: Optional[Dict[str, Any]] = Field(
        None, description="クリーンアップパラメータ"
    )
    ...
    def validate_parameters(cls, v, values):
        ...
        if cleanup_type == "orphaned":
            ...
            if v and "min_size_mb" in v:
                if (
                    not isinstance(v["min_size_mb"], (int, float))
                    or v["min_size_mb"] <= 0
                ):
