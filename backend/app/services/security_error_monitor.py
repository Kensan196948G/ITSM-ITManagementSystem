...
class SecurityErrorMonitor:
    ...
    async def start_infinite_security_monitoring(self):
        ...
        while self.monitoring_active:
            try:
                monitoring_cycle += 1
                self.logger.info(f"Starting security monitoring cycle #{monitoring_cycle}")
                ...
                await self.generate_security_report(
                    auth_status, injection_status, rate_limit_status,
                    suspicious_status, data_exposure_status, security_config_status,
                    threat_intel_status, mitigation_result
                )
                ...
                self.logger.error(f"Security monitoring cycle #{monitoring_cycle} failed: {str(e)}")
                self.logger.error(traceback.format_exc())
                
                if consecutive_failures >= max_consecutive_failures:
                    self.logger.critical(f"Too many consecutive security monitoring failures: {consecutive_failures}")
    ...
    async def monitor_authentication_security(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Authentication security check: {auth_status['overall_status']}")
            ...
            async with httpx.AsyncClient() as client:
                ...
                for creds in test_credentials:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            test_result["issues"].append(f"Weak credentials accepted: {creds['username']}")
                ...
                for payload in injection_payloads:
                    try:
                        ...
                        if response.status_code == 200:
                            test_result["secure"] = False
                            test_result["issues"].append("SQL injection vulnerability detected")
    ...
    async def _test_authorization_endpoint(self, endpoint: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    test_result["secure"] = False
                    test_result["issues"].append("Endpoint accessible without authentication")
                elif response.status_code == 401:
                    test_result["tests_performed"].append("Authentication requirement verified")
                ...
                for headers in privilege_escalation_headers:
                    ...
                    if response.status_code == 200:
                        test_result["secure"] = False
                        test_result["issues"].append("Privilege escalation possible via headers")
                ...
                if "alg" in test_case:
                    ...
                    try:
                        async with httpx.AsyncClient() as client:
                            ...
                            response = await client.get(f"{self.base_url}/api/v1/users", headers=headers, timeout=5)
                            
                            if response.status_code == 200:
                                jwt_test["secure"] = False
                                jwt_test["issues"].append("None algorithm attack successful")
                ...
                if response.cookies:
                    for cookie_name, cookie_value in response.cookies.items():
                        ...
                        if 'secure' not in str(cookie_value).lower():
                            ...
                            session_test["issues"].append(f"Cookie {cookie_name} missing Secure flag")
                        
                        # HttpOnlyフラグのチェック
                        if 'httponly' not in str(cookie_value).lower():
                            session_test["secure"] = False
                            session_test["issues"].append(f"Cookie {cookie_name} missing HttpOnly flag")
        ...
        try:
                    ...
                    try:
                        ...
                        async with httpx.AsyncClient() as client:
                            ...
                            error_indicators = [
                                ...
                                "syntax error", "table", "column", "constraint"
                            ]
            ...
            for payload in path_payloads:
                try:
                    async with httpx.AsyncClient() as client:
                        response = await client.get(
                            f"{self.base_url}/api/v1/attachments/{payload}",
                            timeout=5
                        )
                        
                        # システムファイルの内容検出
                        system_file_indicators = [
                            "root:x:", "daemon:x:", "[boot loader]", "windows registry"
                        ]
            ...
            self.logger.info(f"Injection attack detection completed: {len(injection_status['detected_attacks'])} attacks detected")
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                for i in range(request_count):
                    try:
                        response = await client.get(f"{self.base_url}{test_endpoint}", timeout=2)
            ...
            for _ in range(concurrent_requests):
                task = asyncio.create_task(self._send_concurrent_requests(test_endpoint, 5))
                tasks.append(task)
            
            ddos_results = await asyncio.gather(*tasks, return_exceptions=True)
            ddos_elapsed_time = time.time() - ddos_start_time
            
            ddos_successful = sum(1 for result in ddos_results if isinstance(result, int) and result > 0)
            ...
            window_start = current_time - timedelta(minutes=self.security_thresholds["rate_limit_window_minutes"])
            
            for ip, timestamps in self.rate_limit_tracker.items():
                # 古いタイムスタンプを削除
                while timestamps and datetime.fromisoformat(timestamps[0]) < window_start:
                    timestamps.popleft()
            ...
            self.logger.info(f"Rate limiting monitoring completed: {rate_limit_status['rate_violations']} violations")
        ...
        try:
            async with httpx.AsyncClient() as client:
                for _ in range(count):
                    try:
                        response = await client.get(f"{self.base_url}{endpoint}", timeout=1)
    ...
    async def detect_suspicious_activities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for user_agent in bot_user_agents:
                try:
                    async with httpx.AsyncClient() as client:
                        ...
                        response = await client.get(f"{self.base_url}/api/v1/health", headers=headers, timeout=2)
                        
                        # ボットブロッキングの確認
                        if response.status_code == 200 and user_agent not in self.whitelisted_user_agents:
                            suspicious_status["bot_detection"][user_agent or "empty"] = {
                                "blocked": False,
                                "status_code": response.status_code
                            }
                        else:
            ...
            for test in anomaly_tests:
                try:
                    async with httpx.AsyncClient() as client:
                        response = await client.get(f"{self.base_url}{test['path']}", timeout=2)
            ...
            self.logger.info(f"Suspicious activity detection completed: {len(suspicious_status['suspicious_patterns'])} patterns found")
    ...
    async def detect_data_exposure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for endpoint in info_endpoints:
                try:
                    async with httpx.AsyncClient() as client:
                        response = await client.get(f"{self.base_url}{endpoint}", timeout=2)
                        ...
                        if response.status_code == 200:
                            ...
                            if exposed_info:
                                data_exposure_status["information_disclosure"][endpoint] = {
                                    "exposed": True,
                                    "sensitive_data": exposed_info
                                }
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        ...
                        if response.status_code >= 400:
                            ...
                            if disclosed_info:
                                data_exposure_status["sensitive_data_exposure"][endpoint] = {
            ...
            self.logger.info(f"Data exposure detection completed: {len(data_exposure_status['exposure_risks'])} risks found")
    ...
    async def check_security_configuration(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                for header, expected_value in required_headers.items():
                    ...
                    if header_value is None:
                        ...
                        config_status["configuration_issues"].append(f"Missing security header: {header}")
                    else:
                        is_secure = True
                        if expected_value:
            ...
            self.logger.info(f"Security configuration check completed: {len(config_status['configuration_issues'])} issues found")
    ...
    async def analyze_threat_intelligence(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_threats = [
                ...
                if datetime.fromisoformat(threat.timestamp) > datetime.now() - timedelta(hours=24)
            ]
            ...
            if recent_threats:
                ...
                # リスクアセスメント
                critical_threats = sum(1 for t in recent_threats if t.severity == "critical")
                ...
                # 推奨事項生成
                if critical_threats > 0:
                    threat_intel_status["recommendations"].append("Immediate security review required for critical threats")
                
                if "SQL_INJECTION" in threat_types:
                    threat_intel_status["recommendations"].append("Implement parameterized queries and input validation")
                
                if "XSS" in threat_types:
                    threat_intel_status["recommendations"].append("Implement output encoding and CSP headers")
                
                if "RATE_LIMIT_VIOLATION" in threat_types:
                    threat_intel_status["recommendations"].append("Review and strengthen rate limiting policies")
                
                # 攻撃パターンの特定
                attack_patterns = []
                for ip, count in source_ips.items():
                    if count > 5:  # 同一IPから5回以上の脅威
                        attack_patterns.append(f"Repeated attacks from {ip}: {count} incidents")
                
                threat_intel_status["pattern_analysis"]["attack_patterns"] = attack_patterns
            ...
            self.logger.info(f"Threat intelligence analysis completed: Risk level {threat_intel_status.get('risk_assessment', {}).get('risk_level', 'unknown')}")
    ...
    async def execute_security_mitigation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_threats = [
                threat for threat in self.security_threats
                if datetime.fromisoformat(threat.timestamp) > datetime.now() - timedelta(hours=1)
            ]
            ...
            for ip, count in source_ip_counts.items():
                if count >= self.security_thresholds["auto_block_threshold"]:
                    if ip not in self.blocked_ips:
                        ...
                        mitigation_result["mitigations_applied"].append(f"Blocked IP {ip} (threat count: {count})")
            ...
            if critical_threats:
                # クリティカル脅威に対する緊急対応
                mitigation_result["mitigations_applied"].append("Emergency security mode activated for critical threats")
                
                # 監視間隔を短縮
                original_interval = self.monitoring_interval
                self.monitoring_interval = max(10, original_interval // 2)
                mitigation_result["mitigations_applied"].append(f"Monitoring interval reduced: {original_interval}s -> {self.monitoring_interval}s")
            ...
            for ip in high_auth_failure_ips:
                if ip not in self.whitelisted_ips and ip not in self.blocked_ips:
                    self.blocked_ips.add(ip)
                    mitigation_result["blocked_ips_added"] += 1
                    mitigation_result["mitigations_applied"].append(f"Blocked IP {ip} for authentication failures")
            ...
            if len(recent_threats) > 50:  # 高い脅威レベル
                mitigation_result["mitigations_applied"].append("Increased security monitoring due to high threat activity")
            
            mitigation_result["success"] = True
            self.logger.info(f"Security mitigation completed: {len(mitigation_result['mitigations_applied'])} actions applied")
            
        except Exception as e:
            mitigation_result.update({
                "success": False,
                "error": str(e)
            })
    ...
    async def emergency_security_response(self):
        ...
        try:
            ...
            response_actions.append(f"System state: {threat_count} threats, {blocked_count} blocked IPs")
            
            # 2. 緊急ブロックリストの拡張
            critical_threats = [
                t for t in self.security_threats[-100:]  # 最新100件
                if t.severity == "critical" and t.source_ip
            ]
            
            emergency_blocked = 0
            for threat in critical_threats:
                if threat.source_ip not in self.whitelisted_ips and threat.source_ip not in self.blocked_ips:
                    self.blocked_ips.add(threat.source_ip)
                    emergency_blocked += 1
            
            response_actions.append(f"Emergency blocked {emergency_blocked} critical threat IPs")
            
            # 3. 監視強化
            original_interval = self.monitoring_interval
            self.monitoring_interval = 10  # 10秒間隔に強化
            response_actions.append(f"Enhanced monitoring: {original_interval}s -> {self.monitoring_interval}s")
            
            # 4. 脅威履歴の分析とクリーンアップ
            high_severity_threats = len([t for t in self.security_threats if t.severity in ["critical", "high"]])
            ...
            self.security_threats = [
                ...
                if datetime.fromisoformat(t.timestamp) > cutoff_time or t.severity in ["critical", "high"]
            ]
            cleaned_count = original_count - len(self.security_threats)
            
            response_actions.append(f"Threat data cleanup: {cleaned_count} old threats removed")
            response_actions.append(f"High severity threats in system: {high_severity_threats}")
            ...
            self.logger.info(f"Emergency security response completed: {', '.join(response_actions)}")
            
        except Exception as e:
            response_actions.append(f"Emergency response failed: {str(e)}")
            self.logger.error(f"Emergency security response failed: {str(e)}")
    
    async def generate_security_report(self, auth_status, injection_status, rate_limit_status,
                                     suspicious_status, data_exposure_status, security_config_status,
                                     threat_intel_status, mitigation_result):
        """セキュリティレポート生成"""
        try:
            ...
            mitigation_success_rate = mitigated_threats / total_threats if total_threats > 0 else 0
            ...
            security_metrics = SecurityMetrics(
                ...
                blocked_ips=len(self.blocked_ips),
                suspicious_activities=len(suspicious_status.get("suspicious_patterns", [])),
                authentication_failures=sum(self.failed_auth_tracker.values()),
                authorization_violations=len(auth_status.get("vulnerabilities_found", [])),
                data_exposure_attempts=len(data_exposure_status.get("exposure_risks", [])),
            ...
            security_report = {
                ...
                "security_recommendations": self._generate_security_recommendations(
                    auth_status, injection_status, data_exposure_status, security_config_status
                )
            }
            ...
            self.logger.info(f"Security report generated: {security_report['overall_security_status']} status")
            
        except Exception as e:
            self.logger.error(f"Failed to generate security report: {str(e)}")
    
    def _calculate_overall_security_status(self, auth_status, injection_status, rate_limit_status,
                                         suspicious_status, data_exposure_status, security_config_status) -> str:
        """総合セキュリティ状態計算"""
        # クリティカル問題の検出
    ...
    def _generate_security_recommendations(self, auth_status, injection_status, 
                                         data_exposure_status, security_config_status) -> List[str]:
        """セキュリティ推奨事項生成"""
        recommendations = []
        ...
        try:
            ...
            if exposure_risks:
                recommendations.append("Review and secure information disclosure endpoints")
                recommendations.append("Implement proper error handling to prevent information leakage")
            ...
            if len(self.blocked_ips) > 20:
                recommendations.append("Review IP blocking policies and whitelist management")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/security_monitor.log'),
