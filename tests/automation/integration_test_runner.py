...
class IntegrationTestRunner:
        ...
        try:
            ...
            if pytest_results_file.exists():
                ...
                suite_results = {
                    ...
                    "success_rate": pytest_data.get("summary", {}).get("passed", 0) / max(pytest_data.get("summary", {}).get("total", 1), 1),
    ...
    async def run_api_health_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_checks.append({
                ...
                "details": github_result.stdout if github_result.returncode == 0 else github_result.stderr
            })
        except Exception as e:
            health_checks.append({
                "name": "GitHub API Connection",
    ...
    async def run_load_performance_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 全体的な判定
            passed_tests = len([r for r in performance_results if r["status"] == "PASS"])
        ...
        for suite_name, suite_results in self.test_suites.items():
            if isinstance(suite_results, dict):
                ...
                if "total_tests" in suite_results:
                    ...
                    self.overall_results["passed_tests"] += suite_results.get("passed", 0)
                    self.overall_results["failed_tests"] += suite_results.get("failed", 0)
                    self.overall_results["error_tests"] += suite_results.get("errors", 0)
                    self.overall_results["skipped_tests"] += suite_results.get("skipped", 0)
                elif "total_checks" in suite_results:
                    self.overall_results["total_tests"] += suite_results["total_checks"]
                    self.overall_results["passed_tests"] += suite_results.get("passed_checks", 0)
    ...
    def check_quality_gates(self) -> Dict[str, Any]:
        ...
        success_rate_check = {
            ...
            "status": "PASS" if success_rate >= self.quality_gates["min_success_rate"] else "FAIL"
        }
        ...
        duration_check = {
            ...
            "status": "PASS" if duration <= self.quality_gates["max_duration_minutes"] else "FAIL"
        }
        ...
        critical_check = {
            ...
            "status": "PASS" if critical_failures <= self.quality_gates["max_critical_failures"] else "FAIL"
        }
        ...
        for check in quality_checks:
            ...
            print(f"{status_emoji} {check['gate']}: {check['actual']} (Required: {check['threshold']})")
    ...
    async def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        comprehensive_report = {
            ...
            "executive_summary": {
                ...
                "success_rate": self.overall_results["passed_tests"] / max(self.overall_results["total_tests"], 1)
            },
        ...
        # レポートファイル保存
        report_file = self.base_path / "reports" / f"integration_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(report_file, 'w') as f:
            json.dump(comprehensive_report, f, indent=2)
        
        # Markdownレポート生成
        markdown_report = self.generate_markdown_report(comprehensive_report)
        markdown_file = self.base_path / "reports" / f"integration_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if success_rate < 0.9:
            recommendations.append("テスト成功率が90%を下回っています。失敗したテストケースの詳細調査を推奨します。")
        
        if self.overall_results["error_tests"] > 0:
            recommendations.append("エラーが発生したテストがあります。環境設定や依存関係を確認してください。")
        
        if "playwright_e2e" not in self.test_suites or self.test_suites["playwright_e2e"].get("status") == "SKIPPED":
            recommendations.append("E2Eテストが実行されていません。Playwrightのインストールを確認してください。")
...
async def main():
    ...
    try:
        ...
        # 最終判定
        overall_status = final_report.get("quality_gates", {}).get("overall_status", "UNKNOWN")
