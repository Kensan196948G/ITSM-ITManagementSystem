...
async def get_security_context(request: Request) -> SecurityContext:
    ...
    return SecurityContext(
        ...
        permissions=(
            request.headers.get("X-Permissions", "").split(",")
            if request.headers.get("X-Permissions")
            else []
        ),
...
async def analyze_error(
    ...
    try:
        ...
        if analyzed_error:
            # 修復提案を生成
            suggestions = log_analyzer.suggestion_engine.generate_suggestions(
                analyzed_error
            )
            ...
            analysis_result = {
                ...
                "estimated_resolution_time": (
                    "15-30 minutes" if suggestions else "unknown"
                ),
            }
        else:
            analysis_result = {
...
async def create_repair_task(
    ...
    try:
        ...
        if success:
            logger.info(
                f"Repair task created by user {security_context.user_id}: {repair_task.id}"
            )
...
async def get_repair_history(
    limit: int = 50, security_context: SecurityContext = Depends(get_security_context)
) -> Dict[str, Any]:
    """修復履歴を取得"""
    global monitoring_system
    ...
    try:
        ...
        return {
            ...
            "success_rate": (
                len([t for t in repair_history if t.success]) / len(repair_history)
                if repair_history
                else 0
            ),
        }
...
async def get_monitoring_metrics(
    hours: int = 24, security_context: SecurityContext = Depends(get_security_context)
) -> Dict[str, Any]:
    """監視メトリクスを取得"""
    try:
        # 各監視システムからメトリクスを収集
...
async def emergency_stop(
    reason: str, security_context: SecurityContext = Depends(get_security_context)
) -> Dict[str, Any]:
    """緊急停止"""
    global monitoring_system, monitoring_active
    ...
    try:
        logger.critical(
            f"Emergency stop requested by user {security_context.user_id}: {reason}"
        )
