...
export class AutoRepairEngine {
    constructor() {
        Object.defineProperty(this, "repairRules", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "repairHistory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
     ...
     * 修復後の検証
     */
    async validateRepair(page, originalError, fix) {
        const results = [];
        try {
            // JavaScript検証
    ...
    getRepairStatistics() {
        ...
        const ruleUsage = this.repairRules.map(rule => {
            const usageCount = this.repairHistory.filter(r => r.appliedActions.some(action => action.description.includes(rule.name))).length;
            return {
                ruleName: rule.name,
                usageCount,
                successCount: this.repairHistory.filter(r => r.success && r.appliedActions.some(action => action.description.includes(rule.name))).length
            };
        });
        return {
            totalRepairs: total,
