...
class IncidentCreatePage(AuthenticatedPage):
    ...
    def is_loaded(self) -> bool:
        ...
        try:
            self.page.wait_for_selector(
                '[data-testid="incident-create-form"]', timeout=5000
            )
            return True
        except:
            return False
    ...
    def fill_incident_form(self, incident_data: Dict[str, Any]):
        ...
        # Fill description
        self.page.fill(
            '[data-testid="incident-description"]', incident_data["description"]
        )

        # Select priority
        self.page.select_option(
            '[data-testid="incident-priority"]', incident_data["priority"]
        )

        # Select category
        self.page.select_option(
            '[data-testid="incident-category"]', incident_data["category"]
        )

        # Fill additional fields if provided
        if "impact" in incident_data:
            self.page.select_option(
                '[data-testid="incident-impact"]', incident_data["impact"]
            )

        if "urgency" in incident_data:
            self.page.select_option(
                '[data-testid="incident-urgency"]', incident_data["urgency"]
            )

        if "assignee" in incident_data:
            self.page.select_option(
                '[data-testid="incident-assignee"]', incident_data["assignee"]
            )
...
class IncidentDetailPage(AuthenticatedPage):
    ...
    def is_loaded(self) -> bool:
        ...
        try:
            self.page.wait_for_selector(
                '[data-testid="incident-details"]', timeout=5000
            )
            return True
        except:
            return False
    ...
    def update_status(self, new_status: str):
        ...
        try:
            confirm_button = self.page.wait_for_selector(
                '[data-testid="confirm-status-change"]', timeout=2000
            )
...
class IncidentHistoryPage(AuthenticatedPage):
    ...
    def is_loaded(self) -> bool:
        ...
        try:
            self.page.wait_for_selector(
                '[data-testid="incident-history"]', timeout=5000
            )
            return True
        except:
            return False
...
class IncidentEditPage(AuthenticatedPage):
    ...
    def is_loaded(self) -> bool:
        ...
        try:
            self.page.wait_for_selector(
                '[data-testid="incident-edit-form"]', timeout=5000
            )
            return True
        except:
            return False
