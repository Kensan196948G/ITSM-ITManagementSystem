...
class Attachment(Base):
    ...
    resource_type = Column(String(50), nullable=False)  # incidents, problems, changes, users
    ...
    scan_results = relationship("AttachmentScanResult", back_populates="attachment", cascade="all, delete-orphan")
    access_logs = relationship("AttachmentAccessLog", back_populates="attachment", cascade="all, delete-orphan")
...
class AttachmentScanResult(Base):
    ...
    # スキャン情報
    scan_status = Column(String(20), nullable=False)  # clean, infected, suspicious, error
