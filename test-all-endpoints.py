...
class EndpointTester:
    ...
    async def test_endpoint(self, service: str, base_url: str, path: str, timeout: int = 10) -> Dict:
        """個別エンドポイントのテスト"""
        url = f"{base_url}{path}"
        test_result = {
            'service': service,
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
                async with session.get(url) as response:
                    ...
                    if response.status < 400:
                        ...
                        logger.info(f"✅ {url} - {response.status} ({response_time:.3f}s)")
                    elif response.status < 500:
                        test_result['status'] = 'warning'
                        test_result['error_message'] = f"Client error: {response.status}"
                        self.results['failed'] += 1
                        logger.warning(f"⚠️ {url} - {response.status} ({response_time:.3f}s)")
                    else:
                        test_result['status'] = 'error'
                        test_result['error_message'] = f"Server error: {response.status}"
                        self.results['failed'] += 1
                        logger.error(f"❌ {url} - {response.status} ({response_time:.3f}s)")
                    ...
                    elif path == '/' and service == 'frontend' and response.status == 200:
        ...
        # エラーの場合は詳細をエラーリストに追加
        if test_result['status'] in ['error', 'timeout', 'connection_error', 'unknown_error']:
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if len([e for e in self.results['errors'] if 'connection_error' in e.get('status', '')]) > 0:
            recommendations.append("🔌 接続エラーが発生しています。ネットワーク設定を確認してください。")
...
"""
        ...
        for service_name, service_results in report['detailed_results'].items():
            for result in service_results:
                status_class = 'status-success' if result.get('status') == 'success' else 'status-warning' if result.get('status') == 'warning' else 'status-error'
                html_content += f"""
                <tr>
        ...
        <p style="text-align: center; color: #7f8c8d; margin-top: 40px;">
            レポート生成時刻: """ + report['summary']['timestamp'] + """
        </p>
    </div>
</body>
</html>
...
async def main():
    ...
    parser.add_argument('--ip', default='192.168.3.135', help='テスト対象IPアドレス (デフォルト: 192.168.3.135)')
