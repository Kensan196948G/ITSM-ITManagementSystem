...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/itsm_repair.log'),
...
class ITSMLoopRepairEngine:
    def __init__(self):
        ...
        self.realtime_repair_state = self.coordination_path / "realtime_repair_state.json"
    ...
    def repair_api_health_status(self):
        ...
        try:
            ...
            if self._check_pytest_status():
                ...
                logger.warning("Pytest が失敗しているため、health status は unhealthy のまま")
                return False
    ...
    def update_repair_metrics(self, cycle, issues_fixed):
        ...
        try:
            ...
            state["total_errors_fixed"] = state.get("total_errors_fixed", 0) + len(issues_fixed)
            ...
            logger.info(f"修復メトリクス更新: サイクル{cycle}, 修復数{len(issues_fixed)}")
    ...
    def run_repair_cycle(self, cycle):
        ...
        logger.info(f"修復サイクル {cycle} 完了 - 成功: {success}, 修復数: {len(issues_fixed)}")
