...
class UserService:
    ...
    def create_user(self, user_data: UserCreate, current_user_id: UUID) -> UserResponse:
        ...
        try:
            ...
            existing_user = (
                self.db.query(User)
                .filter(and_(User.email == user_data.email, User.deleted_at.is_(None)))
                .first()
            )
            ...
            if user_data.password_expiry_days:
                db_user.password_expiry = datetime.utcnow() + timedelta(
                    days=user_data.password_expiry_days
                )
    ...
    def update_user(
        self, user_id: UUID, user_data: UserUpdate, current_user_id: UUID
    ) -> UserResponse:
        """ユーザー情報を更新する"""
        try:
            user = self._get_user_by_id(user_id, current_user_id)
            ...
            if user_data.password:
                ...
                if user_data.password_expiry_days:
                    user.password_expiry = datetime.utcnow() + timedelta(
                        days=user_data.password_expiry_days
                    )
    ...
    def update_user_role(
        self, user_id: UUID, role_data: UserRoleUpdate, current_user_id: UUID
    ) -> UserResponse:
        """ユーザーのロールを変更する"""
        try:
            user = self._get_user_by_id(user_id, current_user_id)
            ...
            logger.info(
                f"User role updated: {user.email} to {role_data.role} by user {current_user_id}"
            )
    ...
    def reset_user_password(self, user_id: UUID, current_user_id: UUID) -> str:
        ...
        try:
            ...
            logger.info(
                f"Password reset for user: {user.email} by user {current_user_id}"
            )
    ...
    def get_user_performance(
        self, user_id: UUID, current_user_id: UUID, days: int
    ) -> Dict[str, Any]:
        """ユーザーのパフォーマンス統計を取得する"""
        try:
            user = self._get_user_by_id(user_id, current_user_id)
            ...
            sla_compliance_rate = (
                (sla_compliant / resolved_count * 100) if resolved_count > 0 else 100
            )
            ...
            return {
                ...
                "incident_stats": {
                    ...
                    "resolution_rate": (
                        (resolved_count / assigned_count * 100)
                        if assigned_count > 0
                        else 0
                    ),
    ...
    def _get_user_basic_stats(self, user_id: UUID) -> Dict[str, Any]:
        ...
        assigned_tickets = (
            self.db.query(func.count(Incident.id))
            .filter(
                and_(Incident.assignee_id == user_id, Incident.deleted_at.is_(None))
            )
            .scalar()
            or 0
        )
