        ...
        try:
            ...
            if not str(path).startswith(str(project_root)):
                self.log_security_event(
                    "blocked_file_access", f"Outside project: {path}"
                )
                return False
            ...
            if any(path.match(pattern) for pattern in restricted_files):
                self.log_security_event(
                    "blocked_restricted_file", f"Restricted: {path}"
                )
                return False
...
class LoopRepairEngine:
    ...
    def __init__(self, project_root: str = None):
        ...
        self.repo_name = os.getenv(
            "GITHUB_REPOSITORY", "Kensan196948G/ITSM-ITManagementSystem"
        )
    ...
    async def run_infinite_loop(self):
        """無限ループ修復エンジンメイン実行"""
        self.logger.info(
            "Starting Loop Repair Engine - 5-second interval infinite loop"
        )
        self.running = True
        ...
        try:
            while self.running and self.current_iteration < self.max_iterations:
                ...
                self.logger.info(
                    f"=== Loop Iteration {self.current_iteration}/{self.max_iterations} ==="
                )
                ...
                # 5秒間隔待機
                self.logger.info(
                    f"Waiting {self.loop_interval} seconds for next iteration..."
                )
    ...
    async def detect_errors(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.append(
                {"type": "detection_error", "message": str(e), "severity": "high"}
            )
    ...
    async def check_github_actions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.append(
                {"type": "github_check_error", "message": str(e), "severity": "medium"}
            )
        ...
        try:
            ...
            if not self.security_manager.validate_command(cmd):
                return [
                    {
                        "type": "security_block",
                        "message": "pytest command blocked",
                        "severity": "high",
                    }
                ]
            ...
            # テスト結果記録
            await self.record_test_metrics(
                "pytest", process.returncode == 0, stdout.decode()
            )

        except Exception as e:
            errors.append(
                {"type": "pytest_error", "message": str(e), "severity": "medium"}
            )
        ...
        try:
            ...
            if not self.security_manager.validate_command(cmd):
                return [
                    {
                        "type": "security_block",
                        "message": "playwright command blocked",
                        "severity": "high",
                    }
                ]
            ...
            # テスト結果記録
            await self.record_test_metrics(
                "playwright", process.returncode == 0, stdout.decode()
            )

        except Exception as e:
            errors.append(
                {"type": "playwright_error", "message": str(e), "severity": "medium"}
            )
    ...
    async def run_api_tests(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.append(
                {"type": "api_test_error", "message": str(e), "severity": "medium"}
            )
        ...
        try:
            ...
            if not self.security_manager.validate_command(cmd):
                return [
                    {
                        "type": "security_block",
                        "message": "npm build command blocked",
                        "severity": "high",
                    }
                ]
    ...
    async def check_backend_build(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.append(
                {"type": "backend_build_error", "message": str(e), "severity": "medium"}
            )
    ...
    async def check_system_health(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.append(
                {"type": "system_health_error", "message": str(e), "severity": "low"}
            )
    ...
    async def repair_github_actions_failure(self, error: Dict[str, Any]) -> bool:
        """GitHub Actions 失敗修復"""
        self.logger.info(
            f"Attempting to repair GitHub Actions failure: {error.get('workflow')}"
        )
...
"""
    ...
    async def repair_frontend_build_failure(self, error: Dict[str, Any]) -> bool:
        ...
        if package_json.exists() and self.security_manager.validate_file_access(
            str(package_json)
        ):
            try:
    ...
    async def repair_python_syntax_error(self, error: Dict[str, Any]) -> bool:
        """Python構文エラー修復"""
        self.logger.info(
            f"Attempting to repair Python syntax error in {error.get('file')}"
        )
...
'''
    ...
    async def verify_repairs(self) -> bool:
        ...
        try:
            ...
            # 重要エラーが残っているかチェック
            critical_errors = [
                e for e in verification_errors if e.get("severity") == "high"
            ]

            if critical_errors:
                self.logger.warning(
                    f"Verification failed: {len(critical_errors)} critical errors remain"
                )
                return False
            else:
        ...
        try:
            report = {
                ...
                "summary": {
                    ...
                    "success_rate": self.repair_count
            ...
            report_file = (
                ...
                / "reports"
                / f"loop_repair_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
