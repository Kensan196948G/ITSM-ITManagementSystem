...
class InfiniteErrorMonitor:
    ...
    async def check_url_health(self, url: str, timeout: int = 10) -> Tuple[bool, str, Optional[Dict]]:
        """URLの健全性チェック"""
        try:
    ...
    async def run_playwright_check(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                'bash', '/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/frontend/run-comprehensive-webui-monitor.sh', '--once',
    ...
    async def auto_fix_errors(self, frontend_errors: List[str], backend_errors: List[str]) -> bool:
        """エラーの自動修復"""
        fix_success = True
        ...
        if frontend_errors:
            ...
            try:
                process = await asyncio.create_subprocess_exec(
                    'bash', '/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/frontend/run-comprehensive-webui-monitor.sh', '--fix',
                    cwd='/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/frontend',
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
        ...
        if backend_errors:
            ...
            try:
                process = await asyncio.create_subprocess_exec(
                    'python3', 'monitor_api.py', 'fix-errors',
                    cwd='/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend',
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
        ...
        if self.consecutive_errors >= self.config['restart_threshold']:
            ...
            try:
                ...
                restart_process = await asyncio.create_subprocess_exec(
                    'bash', '-c', 'cd /media/kensan/LinuxHDD/ITSM-ITmanagementSystem/frontend && npm start',
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                ...
                restart_process = await asyncio.create_subprocess_exec(
                    'bash', '-c', 'cd /media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend && python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload',
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
    ...
    def generate_status_report(self) -> Dict:
        ...
        error_rate = (self.stats['total_errors_found'] / max(self.stats['total_checks'], 1)) * 100
        fix_rate = (self.stats['total_errors_fixed'] / max(self.stats['total_errors_found'], 1)) * 100
        ...
        return {
            ...
            'system_health': 'good' if self.consecutive_errors < 3 else 'warning' if self.consecutive_errors < 7 else 'critical'
        }
    ...
    def generate_final_report(self):
        ...
        logger.info(f"📈 修復成功率: {(self.stats['total_errors_fixed'] / max(self.stats['total_errors_found'], 1)) * 100:.1f}%")
        logger.info("=" * 60)
