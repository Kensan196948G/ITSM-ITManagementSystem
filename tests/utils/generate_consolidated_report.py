...
class TestReportGenerator:
        ...
        for json_file in self.input_dir.glob("*-report.json"):
            try:
                with open(json_file, "r") as f:
                    ...
                    report_data["suites"][suite_name] = self._process_pytest_report(
                        data
                    )
        ...
        if benchmark_files:
            try:
                with open(benchmark_files[0], "r") as f:
                    ...
                    report_data["performance"]["benchmarks"] = (
    ...
    def _process_benchmark_data(self, data: Dict) -> Dict[str, Any]:
        ...
        return {"total_benchmarks": len(benchmarks), "benchmarks": benchmarks}
    ...
    <div class="container">
        ...
        for suite_name, suite_data in data["suites"].items():
            ...
            status_class = (
                "status-passed"
                if success_rate >= 95
                else "status-failed" if success_rate < 80 else ""
            )
...
"""
        ...
        for suite_name, suite_data in data["suites"].items():
            success_icon = (
                "✅"
                if suite_data.get("success_rate", 0) >= 95
                else "⚠️" if suite_data.get("success_rate", 0) >= 80 else "❌"
            )
        ...
        if data.get("coverage"):
            ...
            md_content += f"\n## 📈 コードカバレッジ\n\n"
            md_content += (
                f"- **カバレッジ率:** {coverage.get('percent_covered', 0):.1f}%\n"
            )
            md_content += (
                f"- **総ステートメント数:** {coverage.get('num_statements', 0)}\n"
            )
    ...
    def generate_json_summary(self, data: Dict[str, Any]) -> str:
        ...
        summary = {
            "test_execution_summary": {
                ...
                "overall_status": (
                    "PASS" if data["summary"]["success_rate"] >= 95 else "FAIL"
                ),
                "quality_gate_result": {
    ...
    def _generate_recommendations(self, data: Dict[str, Any]) -> List[str]:
        ...
        if success_rate < 95:
            if success_rate < 50:
                recommendations.append(
                    "🚨 CRITICAL: テスト成功率が50%を下回っています。即座に修正が必要です。"
                )
            elif success_rate < 80:
                recommendations.append(
                    "⚠️ テスト成功率が80%を下回っています。品質ゲート不合格です。"
                )
            else:
                recommendations.append(
                    "📈 テスト成功率が95%を下回っています。安定性向上のため修正推奨です。"
                )
        ...
        if failed_count > 0:
            if failed_count > 10:
                recommendations.append(
                    f"🔥 {failed_count}件の大量のテスト失敗が発生しています。優先度高で対応してください。"
                )
            else:
                recommendations.append(
                    f"🐛 {failed_count}件のテストが失敗しています。詳細なログを確認してください。"
                )

        # Performance recommendations
        if total_duration > 300:  # 5 minutes
            recommendations.append(
                "⏰ テスト実行時間が5分を超えています。テストの並列化や最適化を検討してください。"
            )
        elif total_duration > 600:  # 10 minutes
            recommendations.append(
                "🐌 テスト実行時間が10分を超えています。CI/CDパイプラインの効率化が必要です。"
            )
        ...
        for suite_name, suite_data in data["suites"].items():
            ...
            if suite_success_rate < 80:
                if suite_name in critical_suites:
                    recommendations.append(
                        f"🚫 {suite_name}スイート（クリティカル）の成功率が{suite_success_rate:.1f}%です。即座に修正してください。"
                    )
                else:
                    recommendations.append(
                        f"⚠️ {suite_name}スイートの成功率が{suite_success_rate:.1f}%です。優先的に修正してください。"
                    )

            # Suite performance
            if suite_name == "unit" and suite_duration > 60:
                recommendations.append(
                    f"⚡ ユニットテストの実行時間が{suite_duration:.1f}秒です。1分以内を目標に最適化してください。"
                )
            elif suite_name == "e2e" and suite_duration > 300:
                recommendations.append(
                    f"🎭 E2Eテストの実行時間が{suite_duration:.1f}秒です。並列実行や選択的実行を検討してください。"
                )
        ...
        if data.get("coverage"):
            ...
            if coverage_percent < 60:
                recommendations.append(
                    f"📊 コードカバレッジが{coverage_percent:.1f}%です。最低60%を目標にテストを追加してください。"
                )
            elif coverage_percent < 80:
                recommendations.append(
                    f"📈 コードカバレッジが{coverage_percent:.1f}%です。80%を目標にカバレッジを向上させてください。"
                )
        ...
        if "benchmarks" in data.get("performance", {}):
            ...
            if slow_benchmarks:
                recommendations.append(
                    f"🐌 実行時間の長いベンチマーク: {', '.join(slow_benchmarks[:3])}{'...' if len(slow_benchmarks) > 3 else ''}"
                )
        ...
        if not recommendations:
            if success_rate == 100:
                recommendations.append(
                    "🎉 完璧！全てのテストが成功し、品質基準を満たしています。"
                )
            elif success_rate >= 98:
                recommendations.append(
                    "✨ 優秀！高品質を維持しています。現在の水準を継続してください。"
                )
            else:
        ...
        if success_rate >= 95:
            recommendations.append(
                "💡 提案: テスト自動化の拡張やパフォーマンステストの追加を検討してください。"
            )
...
def main():
    ...
    parser.add_argument(
        "--input-dir", required=True, help="Directory containing test report files"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Directory to save generated reports"
    )
