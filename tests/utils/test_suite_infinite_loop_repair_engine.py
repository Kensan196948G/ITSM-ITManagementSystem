...
class TestSuiteInfiniteLoopRepairEngine:
    ...
    def __init__(self):
        ...
        self.test_configs = {
            ...
            "load_test_params": {"users": 10, "spawn_rate": 2, "run_time": "60s"},
        }
    ...
    async def start_infinite_repair_loop(self):
        ...
        try:
            ...
            for loop_num in range(1, self.repair_state["max_loops"] + 1):
                ...
                self.logger.info(
                    f"🔄 修復ループ {loop_num}/{self.repair_state['max_loops']} 開始"
                )
    ...
    async def check_initial_system_status(self):
        ...
        self.logger.info(
            f"📋 初期状態: {json.dumps(status, indent=2, ensure_ascii=False)}"
        )
        return status
    ...
    async def check_system_health(self) -> Dict[str, Any]:
                ...
                with open(coord_error_file, "r") as f:
                    ...
                    if content:
                        ...
                        health["components"]["coordination"] = {
                            "errors": (
                                len(coord_data) if isinstance(coord_data, list) else 0
                            )
                        }
                    else:
        ...
        # 全体ステータス判定
        api_healthy = (
            health["components"].get("api", {}).get("health_status") == "healthy"
        )
        coord_healthy = (
            health["components"].get("coordination", {}).get("errors", 1) == 0
        )

        if api_healthy and coord_healthy:
            health["overall_status"] = "healthy"
        elif (
            health["components"].get("api", {}).get("total_errors", 1) == 0
            and coord_healthy
        ):
    ...
    async def run_comprehensive_test_suite(self) -> Dict[str, Any]:
        ...
        if all_successful:
            ...
            self.logger.info(
                f"✅ テストスイート成功 (連続成功: {self.repair_state['consecutive_clean_runs']})"
            )
        else:
    ...
    async def run_playwright_tests(self) -> Dict[str, Any]:
        ...
        try:
            cmd = (
                ["npx", "playwright", "test"]
                + self.test_configs["playwright_args"]
                + ["--reporter=html,json", "--output-dir=test-results"]
            )
    ...
    async def check_success_conditions(self) -> bool:
        ...
        # 連続クリーン実行回数チェック
        if (
            self.repair_state["consecutive_clean_runs"]
            >= self.repair_state["required_clean_runs"]
        ):
            return True
    ...
    async def record_repair_results(
        self, loop_num: int, errors_found: Dict, test_results: Dict
    ):
        """修復結果記録"""
        repair_record = {
            "loop": loop_num,
    ...
    async def generate_final_report(self):
        ...
        final_report = {
            ...
            "success": self.repair_state["consecutive_clean_runs"]
            >= self.repair_state["required_clean_runs"],
        }

        # レポートファイル保存
        report_file = (
            self.tests_dir / "reports" / "test_suite_infinite_repair_final_report.json"
        )
        ...
        try:
            ...
            self.logger.info("=" * 50)
            self.logger.info(
                f"✅ 修復ループ実行: {final_report['summary']['total_loops']} 回"
            )
            self.logger.info(
                f"✅ エラー修復数: {final_report['summary']['total_errors_fixed']} 件"
            )
            self.logger.info(
                f"✅ 連続成功実行: {final_report['summary']['consecutive_clean_runs']} 回"
            )
            self.logger.info(
                f"✅ 最終ヘルス状態: {final_report['summary']['final_health_status']['overall_status']}"
            )
            self.logger.info(
                f"✅ 修復成功: {'はい' if final_report['success'] else 'いいえ'}"
            )
            self.logger.info("=" * 50)
    ...
    async def run_command(
        self, cmd: List[str], cwd: Path = None, timeout: int = 120
    ) -> Dict[str, Any]:
        """コマンド実行"""
