...
class ErrorDetectionRequest(BaseModel):
    ...
    scan_type: str = Field(
        ..., description="スキャンタイプ (full, quick, security, performance)"
    )
    target_endpoints: Optional[List[str]] = Field(
        None, description="対象エンドポイント"
    )
    ...
    try:
        ...
        if mcp_monitor.error_history:
            ...
            error_stats = {
                ...
                "critical_errors_24h": sum(
                    1 for e in recent_errors if e.severity == "critical"
                ),
                "error_types": {},
        ...
        if loop_controller.repair_history:
            ...
            repair_stats = {
                ...
                "success_rate": (
                    successful_repairs / len(recent_repairs) if recent_repairs else 0
                ),
                "avg_repair_time": (
                    sum(r.duration for r in recent_repairs) / len(recent_repairs)
                    if recent_repairs
                    else 0
                ),
            }
        ...
        return {
            ...
            "system_metrics": {
                ...
                "health_score": _calculate_health_score(
                    loop_status, error_stats, repair_stats
                ),
...
async def get_detailed_metrics(
    ...
    try:
        ...
        metrics_files = [
            (
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/infinite_loop_metrics.json",
                "loop_metrics",
            ),
            (
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/mcp_playwright_metrics.json",
                "mcp_metrics",
            ),
            (
    ...
    try:
        ...
        verification_results = {
            ...
            "performance_health": {
                ...
                "avg_response_time": (
                    sum(perf_metrics.get("api_response_times", {}).values())
                    / len(perf_metrics.get("api_response_times", {}))
                    if perf_metrics.get("api_response_times")
                    else 0
                ),
            },
...
def _generate_next_steps(
    verification_results: Dict[str, Any], repair_type: str
) -> List[str]:
    """次のステップ推奨生成"""
    next_steps = []
    ...
    if verification_results.get("overall_success", False):
        ...
        if not verification_results.get("api_health", {}).get("passed", False):
            next_steps.append(
                "API health issues persist - consider manual intervention"
            )
        ...
        if repair_type != "emergency":
            next_steps.append(
                "Consider escalating to emergency repair if issues persist"
            )
...
def _calculate_health_score(
    loop_status: Dict, error_stats: Dict, repair_stats: Dict
) -> float:
    """システムヘルススコア計算"""
    try:
        score = 100.0
