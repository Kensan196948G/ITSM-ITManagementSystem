...
from app.services.api_error_monitor import (
    api_monitor,
    ErrorSeverity,
    ErrorCategory,
    SecurityAlert,
...
from app.services.comprehensive_report_generator import (
    report_generator,
    ReportConfig,
    ReportType,
    ReportFormat,
)
from app.services.security_compliance_monitor import (
...
async def get_monitor_status():
    ...
    return MonitorStatusResponse(
        ...
        last_health_check=(
            datetime.fromisoformat(status["last_health_check"])
...
async def get_health_history(
    ...
    cutoff_time = datetime.now() - timedelta(hours=hours)
    health_history = [
        h for h in api_monitor.health_history if h.timestamp > cutoff_time
    ]

    # 最新順でソート・制限
    health_history = sorted(health_history, key=lambda x: x.timestamp, reverse=True)[
        :limit
    ]
...
async def get_error_summary(
    ...
    medium_errors = len(
        [e for e in recent_errors if e.severity == ErrorSeverity.MEDIUM]
    )
    ...
    fix_success_rate = (
        (len(successful_fixes) / len(attempted_fixes) * 100) if attempted_fixes else 0
    )
        ...
        recent_error_trend.append(
            {
                ...
                "critical": len(
                    [e for e in hour_errors if e.severity == ErrorSeverity.CRITICAL]
                ),
            }
        )
...
async def get_comprehensive_status():
    ...
    recent_alerts = [
        a
        for a in api_monitor.security_alerts
    ...
    recent_perf = [
        m
        for m in api_monitor.performance_metrics
...
async def get_database_health(
    ...
    cutoff_time = datetime.now() - timedelta(hours=hours)
    health_history = [
        h for h in api_monitor.database_health_history if h.timestamp > cutoff_time
    ]
...
async def get_comprehensive_metrics():
    ...
    try:
        ...
        metrics_path = Path(
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/comprehensive_api_metrics.json"
        )
        if not metrics_path.exists():
        ...
        raise HTTPException(
            status_code=500, detail=f"包括的メトリクス取得エラー: {str(e)}"
        )
...
async def block_ip_address(
    ...
    if ip_address in api_monitor.blocked_ips:
        raise HTTPException(
            status_code=400, detail="IPアドレスは既にブロックされています"
        )
...
async def unblock_ip_address(
    ...
    if ip_address not in api_monitor.blocked_ips:
        raise HTTPException(
            status_code=400, detail="IPアドレスはブロックされていません"
        )
...
async def get_infinite_loop_status():
    ...
    try:
        ...
        return InfiniteLoopMonitorResponse(
            monitoring=(
                continuous_monitor.running
                if hasattr(continuous_monitor, "running")
                else False
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"無限ループ監視ステータス取得エラー: {str(e)}"
        )
...
async def start_infinite_loop_monitoring(
    ...
    try:
        ...
        if hasattr(continuous_monitor, "running") and continuous_monitor.running:
            raise HTTPException(
                status_code=400, detail="無限ループ監視は既に実行中です"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"無限ループ監視開始エラー: {str(e)}"
        )
...
async def stop_infinite_loop_monitoring():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"無限ループ監視停止エラー: {str(e)}"
        )


@router.post("/auto-repair", response_model=AutoRepairResponse)
async def trigger_auto_repair(
    repair_request: AutoRepairRequest, background_tasks: BackgroundTasks
):
    """自動修復エンジンを実行"""
    try:
        ...
        if repair_request.target_errors:
            ...
            target_errors = [
                e
                for e in api_monitor.errors
                if e.error_type in repair_request.target_errors and not e.fix_attempted
            ]
        else:
            # 未修復の全エラー
        ...
        # 修復戦略の決定
        repair_strategy = _determine_repair_strategy(
            repair_request.repair_mode, target_errors
        )
        ...
        return AutoRepairResponse(
            ...
            estimated_completion_time=_calculate_estimated_completion(
                len(target_errors), repair_strategy
            ),
                    ...
                    if "all_fixes" in data:
                        for fix in data["all_fixes"]:
                            ...
                            if fix_time > cutoff_time:
                                if (
                                    not session_id
                                    or fix.get("session_id") == session_id
                                ):
        ...
        return {
            ...
            "system_health": {
                ...
                "critical_errors_24h": len(
                    [e for e in recent_errors if e.severity.value == "critical"]
                ),
                ...
                "manual_intervention_required": len(
                    [
                        ...
                        if not e.fix_attempted
                        and e.severity.value in ["critical", "high"]
                    ]
                ),
            },
            "performance_impact": {
...
async def trigger_emergency_repair(
    ...
    try:
        ...
        critical_errors = [
            e
            for e in api_monitor.errors
            if e.severity.value == "critical" and not e.fix_successful
        ]
        high_errors = [
            e
            for e in api_monitor.errors
            if e.severity.value == "high" and not e.fix_successful
        ]
...
async def get_repair_recommendations():
    ...
    try:
        ...
        if recent_perf:
            avg_response_time = sum(m.response_time for m in recent_perf) / len(
                recent_perf
            )
...
def _calculate_repair_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        success_rate = (
            (len(successful_repairs) / len(attempted_repairs) * 100)
        ...
        return {
            ...
            "last_detection": max(
                [e.timestamp for e in api_monitor.errors], default=None
            ),
...
async def _run_infinite_loop_monitoring(
    monitor: ContinuousBackendMonitor, interval: int, auto_repair: bool
):
    """無限ループ監視を実行"""
    try:
...
async def _run_auto_repair_engine(
    session_id: str, errors: List, strategy: str, max_attempts: int
):
    """自動修復エンジンを実行"""
    try:
...
async def _run_emergency_repair(
    session_id: str, errors: List, force_restart: bool, bypass_safety: bool
):
    """緊急修復を実行"""
    try:
...
async def get_integrated_monitoring_status():
    ...
    try:
        ...
        return {
            "integrated_monitoring": {
                ...
                "enhanced_monitor": {
                    ...
                    "performance_metrics": enhanced_status.get(
                        "performance_metrics", {}
                    ),
                },
        ...
        raise HTTPException(
            status_code=500, detail=f"統合ステータス取得エラー: {str(e)}"
        )
...
async def get_comprehensive_monitoring_dashboard():
    ...
    try:
        ...
        dashboard = {
            "overview": {
                ...
                "overall_health_score": _calculate_overall_health_score(
                    api_status, enhanced_status, system_resources
                ),
            },
            "monitoring_systems": {
                "api_monitor": {
                ...
                "enhanced_monitor": {
                    "status": (
                        "active"
                        if enhanced_status.get("monitoring_active", False)
                        else "inactive"
                    ),
                    "total_detections": enhanced_status.get("total_detections", 0),
                    "total_repairs": enhanced_status.get("total_repairs", 0),
                    "success_rate": enhanced_status.get("performance_metrics", {}).get(
            ...
            "recommendations": _generate_comprehensive_recommendations(
                api_status, enhanced_status, repair_stats, system_resources
            ),
            "generated_at": datetime.now().isoformat(),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"ダッシュボード情報取得エラー: {str(e)}"
        )
...
def _calculate_overall_health_score(
    api_status: Dict, enhanced_status: Dict, system_resources: Dict
) -> float:
    """総合ヘルススコアを計算"""
    score = 100.0
...
def _analyze_error_trends(errors: List) -> Dict[str, Any]:
    ...
    return {
        ...
        "peak_error_time": (
            max(time_buckets.items(), key=lambda x: x[1])[0] if time_buckets else "不明"
        ),
...
def _generate_comprehensive_recommendations(
    api_status: Dict, enhanced_status: Dict, repair_stats: Dict, system_resources: Dict
) -> List[str]:
    """包括的推奨事項を生成"""
    recommendations = []
    ...
    if system_resources.get("cpu_percent", 0) > 80:
        recommendations.append(
            "⚠️ CPU使用率が高いです。プロセスの最適化を検討してください"
        )
    if system_resources.get("memory", {}).get("percent", 0) > 85:
        recommendations.append(
            "⚠️ メモリ使用率が高いです。メモリリークの確認を推奨します"
        )
...
async def list_generated_reports():
    ...
    try:
        ...
        reports_path = Path(
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/reports"
        )
...
async def download_report(filename: str):
    ...
    try:
        ...
        if not file_path.exists():
            raise HTTPException(
                status_code=404, detail="レポートファイルが見つかりません"
            )

        if not file_path.is_file():
            raise HTTPException(
                status_code=400, detail="指定されたパスはファイルではありません"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"レポートダウンロードエラー: {str(e)}"
        )
...
async def cleanup_old_reports(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"レポートクリーンアップエラー: {str(e)}"
        )
    ...
    try:
        ...
        dashboard_data = {
            "real_time_metrics": {
                ...
                "monitoring_active": api_status["monitoring"]
                or enhanced_status.get("monitoring_active", False),
                "total_errors_24h": len(recent_errors),
                "critical_errors_24h": len(
                    [e for e in recent_errors if e.severity.value == "critical"]
                ),
                ...
                "success_rate": repair_stats.get("success_rate", 0),
                "system_health_score": _calculate_overall_health_score(
                    api_status, enhanced_status, system_resources
                ),
            },
            "monitoring_status": {
            ...
            "alerts": _generate_dashboard_alerts(
                api_status, enhanced_status, system_resources
            ),
            "quick_actions": [
                {
                    "id": "start_monitoring",
                    "label": "監視開始",
                    "enabled": not api_status["monitoring"],
                },
                {"id": "generate_report", "label": "レポート生成", "enabled": True},
                {
                    "id": "emergency_repair",
                    "label": "緊急修復",
                    "enabled": len(recent_errors) > 0,
                },
        ...
        raise HTTPException(
            status_code=500, detail=f"ダッシュボードデータ取得エラー: {str(e)}"
        )
...
async def get_real_time_statistics():
    ...
    try:
        ...
        for i in range(24):
            ...
            hour_errors = [
                e for e in api_monitor.errors if hour_start <= e.timestamp < hour_end
            ]
            ...
            hourly_stats.append(
                {
                    ...
                    "critical_count": len(
                        [e for e in hour_errors if e.severity.value == "critical"]
                    ),
        ...
        latest_stats = {
            ...
            "total_systems_monitored": sum(
                [
                    ...
                    (
                        1
                        if enhanced_monitor.get_monitoring_status().get(
            ...
            "avg_repair_time": advanced_repair_engine.get_repair_statistics().get(
        ...
        raise HTTPException(
            status_code=500, detail=f"リアルタイム統計取得エラー: {str(e)}"
        )
...
async def start_security_monitoring(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"セキュリティ監視開始エラー: {str(e)}"
        )
...
async def get_security_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"セキュリティステータス取得エラー: {str(e)}"
        )
...
async def get_security_events(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"セキュリティイベント取得エラー: {str(e)}"
        )
...
async def unblock_ip(
    ...
    try:
        if ip_address in security_compliance_monitor.blocked_ips:
            ...
            raise HTTPException(
                status_code=404, detail=f"IP {ip_address} はブロックされていません"
            )
...
async def get_compliance_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"コンプライアンスステータス取得エラー: {str(e)}"
        )
...
async def get_compliance_checks(
    framework: Optional[str] = Query(
        None, description="コンプライアンスフレームワークでフィルタ"
    ),
    ...
    try:
        ...
        return {
            ...
            "filters": {"framework": framework, "compliant_only": compliant_only},
            "available_frameworks": [f.value for f in ComplianceFramework],
            "retrieved_at": datetime.now().isoformat(),
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"コンプライアンスチェック取得エラー: {str(e)}"
        )
...
async def run_compliance_check(
    background_tasks: BackgroundTasks,
    framework: str = Query(
        "itsm_v4", description="実行するコンプライアンスフレームワーク"
    ),
):
    """コンプライアンスチェックを実行"""
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"無効なフレームワーク: {framework}"
            )

        # バックグラウンドでコンプライアンスチェック実行
        if framework == "itsm_v4":
            background_tasks.add_task(
                security_compliance_monitor._check_itsm_compliance
            )
        else:
            background_tasks.add_task(
                security_compliance_monitor._check_security_compliance
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"コンプライアンスチェック実行エラー: {str(e)}"
        )
    ...
    try:
        ...
        return {
            "threat_intelligence": {
                ...
                "blocked_attacks": len(
                    [e for e in recent_events if e.get("blocked", False)]
                ),
                "top_threat_sources": security_compliance_monitor.suspicious_ips,
                "threat_level": (
                    "LOW"
                    if len(recent_events) < 5
                    else "MEDIUM" if len(recent_events) < 20 else "HIGH"
                ),
            },
            "recommendations": [
                "定期的なセキュリティパッチの適用",
        ...
        raise HTTPException(
            status_code=500, detail=f"脅威インテリジェンス取得エラー: {str(e)}"
        )
...
async def get_comprehensive_integrated_status():
    ...
    try:
        ...
        overall_health_score = (
            sum(score for _, score in health_components) / len(health_components) * 100
        )
            ...
            "overall_system_health": {
                ...
                "status": (
                    ...
                    if overall_health_score >= 90
                        ...
                        else "WARNING" if overall_health_score >= 50 else "CRITICAL"
                    )
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"包括的統合ステータス取得エラー: {str(e)}"
        )
...
def _generate_comprehensive_alerts_and_recommendations(
    ...
    if security_status["critical_events_24h"] > 0:
        alerts.append(
            {
                "level": "critical",
                "message": f"{security_status['critical_events_24h']}件のクリティカルセキュリティイベントが発生",
                "component": "security",
            }
        )
    ...
    if compliance_status["compliance_rate"] < 80:
        alerts.append(
            {
                "level": "warning",
                "message": f"コンプライアンス率が低下しています ({compliance_status['compliance_rate']:.1f}%)",
