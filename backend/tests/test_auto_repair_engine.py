...
class ITSMTestAutoRepairEngine:
    ...
    async def repair_cycle(self):
        ...
        logger.info(f"🔄 Repair Cycle #{self.repair_cycles} - Starting test environment scan")
        ...
        await self.update_metrics(health_status, test_errors, fixes_applied, cycle_duration, github_actions_status)
        
        # 6. ログ出力
        if fixes_applied > 0:
            logger.info(f"✅ Cycle #{self.repair_cycles} completed: {fixes_applied} fixes applied in {cycle_duration:.2f}s")
        else:
            logger.info(f"✅ Cycle #{self.repair_cycles} completed: System healthy ({cycle_duration:.2f}s)")
    ...
    async def check_test_health(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                ["python3", "-m", "pytest", "backend/tests/test_basic.py::TestBasicSetup::test_basic_assertion", "-v", "--tb=no"],
                cwd=self.base_path,
                capture_output=True,
                text=True,
                timeout=30
    ...
    async def detect_test_errors(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                # エラーパターンマッチング
                if "TypeError: Client.__init__() got an unexpected keyword argument 'app'" in error_output:
                    errors.append({
                ...
                if "ValidationError" in error_output and "Extra inputs are not permitted" in error_output:
                    errors.append({
                ...
                if "no such table" in error_output or "database" in error_output.lower():
                    errors.append({
                        "type": "database_error",
                        "severity": "high",
    ...
    async def check_github_actions_status(self) -> str:
        ...
        try:
            ...
            workflow_files = [f for f in os.listdir(workflows_path) if f.endswith('.yml') or f.endswith('.yaml')]
    ...
    async def update_metrics(self, health_status: str, errors: List[Dict], fixes_applied: int, cycle_duration: float, github_status: str):
        """修復メトリクスを更新"""
        try:
            metrics = {
                ...
                "repair_effectiveness": {
                    "success_rate": (self.total_fixes / max(self.repair_cycles, 1)) * 100,
