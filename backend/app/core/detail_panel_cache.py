...
class DetailPanelCacheManager:
    ...
    def generate_cache_key(
        self, cache_type: str, entity_id: UUID, user_id: UUID, **params
    ) -> str:
        """キャッシュキーを生成"""
    ...
    def invalidate_user_cache(self, user_id: UUID):
        """ユーザー関連のキャッシュを無効化"""
        patterns = [f"*:{user_id}:*", f"user_detail:{user_id}:*"]
...
class CacheableQueryBuilder:
    ...
    async def get_user_with_performance(
        ...
        def fetch_user_data():
            ...
            if include_activities:
                # 最近の活動を取得
                result["recent_activities"] = self._get_user_activities(user_id, days)
    ...
    def _calculate_user_performance(self, user_id: UUID, days: int) -> Dict[str, Any]:
        """ユーザーパフォーマンス指標を計算"""
        from app.models.incident import Incident
        from sqlalchemy import func, and_
    ...
    def _get_user_activities(self, user_id: UUID, days: int) -> List[Dict[str, Any]]:
        """ユーザーの最近の活動を取得"""
        # 活動ログテーブルがある場合の実装
        # プレースホルダーとして空のリストを返す
        return []
...
def cache_result(
    cache_type: str, expire: int = 300, key_generator: Optional[Callable] = None
):
    """キャッシュ結果デコレータ"""
