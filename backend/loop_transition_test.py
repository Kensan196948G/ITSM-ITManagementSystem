...
class LoopTransitionTest:
            ...
            with open("../coordination/errors.json", "r") as f:
                ...
                if "errors" in coord_data:
                    ...
                    error_details["coordination"] = {
                        ...
                        "errors": (
                            coord_data["errors"][:3] if coord_data["errors"] else []
                        ),  # First 3 errors
                    }
    ...
    def track_loop_progress(self, duration_minutes=5):
        ...
        print(
            f"Initial Errors Fixed: {initial_loop_state.get('total_errors_fixed', 'N/A')}"
        )
        ...
        while datetime.now() < end_time:
            ...
            print(
                f"Check #{check_count}: Loop={current_loop_count}, Errors={current_total_errors}, Fixed={current_errors_fixed}"
            )

            # Loop変化を検出
            if current_loop_count != initial_loop_state.get("loop_count", 0):
                print(
                    f"  → Loop transition detected: {initial_loop_state.get('loop_count', 0)} → {current_loop_count}"
                )
    ...
    def analyze_transition_patterns(self):
        ...
        return {"total_transitions": len(transitions), "transitions": transitions}
    ...
    def analyze_error_resolution(self):
        ...
        if len(error_counts) > 1:
            ...
            print(
                f"Error Decreasing Instances: {decreasing_trend}/{len(error_counts)-1}"
            )
    ...
    def test_continuous_loop_until_zero_errors(self, max_wait_minutes=10):
        ...
        while datetime.now() < end_time:
            ...
            print(
                f"Continuous Check #{check_count}: Loop={loop_count}, Errors={error_count}"
            )
        ...
        if zero_error_achieved:
            ...
            print(
                f"✗ TIMEOUT: Zero errors not achieved within {max_wait_minutes} minutes"
            )
    ...
    def run_comprehensive_test(self):
        ...
        # 4. ゼロエラーまでの継続テスト
        continuous_test = self.test_continuous_loop_until_zero_errors(
            max_wait_minutes=7
        )
        ...
        print(f"{'='*60}")
        print(
            f"Test Duration: {(datetime.now() - self.test_start_time).total_seconds() / 60:.1f} minutes"
        )
        print(f"Loop Transitions: {transition_analysis.get('total_transitions', 0)}")
        print(
            f"Error Range: {error_analysis.get('min_errors', 'N/A')} - {error_analysis.get('max_errors', 'N/A')}"
        )
        print(
            f"Zero Error Achieved: {'YES' if continuous_test['zero_error_achieved'] else 'NO'}"
        )
