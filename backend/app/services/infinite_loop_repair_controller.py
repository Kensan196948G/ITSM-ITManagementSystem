...
class InfiniteLoopRepairController:
    ...
    async def _execute_repair_cycle(self) -> RepairCycleResult:
        ...
        try:
            ...
            if errors_detected > 0:
                self.logger.info(
                    f"Detected {errors_detected} errors, initiating repair..."
                )
                ...
                if repair_success:
                    ...
                    if verification_result:
                        ...
                        self.loop_state["last_successful_repair"] = (
                            datetime.now().isoformat()
                        )
                        self.logger.info(
                            f"Repair cycle {cycle_id} completed successfully"
                        )
                    else:
                        self.logger.warning(
                            f"Repair verification failed for cycle {cycle_id}"
                        )
            ...
            self.logger.error(
                f"Repair cycle {cycle_id} failed with exception: {str(e)}"
            )
            self.loop_state["consecutive_failures"] += 1
        ...
        try:
            ...
            if error_count > 0:
                ...
                self.error_patterns[error_pattern] = (
                    self.error_patterns.get(error_pattern, 0) + 1
                )
    ...
    async def _perform_additional_repairs(self) -> bool:
        ...
        try:
            ...
            successful_actions = sum(
                1 for action in repair_actions if action.get("status") == "success"
            )
            success_rate = (
                successful_actions / len(repair_actions) if repair_actions else 0
            )
    ...
    async def _repair_system_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing_files:
                ...
                result["details"] = (
                    f"Missing files detected: {', '.join(missing_files)}"
                )
            else:
    ...
    async def _repair_configuration_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if repaired_files:
                result["details"] = (
                    f"Repaired configuration files: {', '.join(repaired_files)}"
                )
            else:
    ...
    async def _cleanup_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["details"] = (
                f"GC collected: {collected} objects, Temp files cleaned: {cleaned_files}"
            )
    ...
    async def _update_metrics(self):
        ...
        try:
            ...
            # 平均修復時間
            avg_repair_time = sum(r.duration for r in self.repair_history) / len(
                self.repair_history
            )
            ...
            recent_success_rate = sum(
                1 for r in recent_cycles if r.repair_success
            ) / len(recent_cycles)
            ...
            metrics = LoopControlMetrics(
                ...
                last_error_detection=(
                    self.repair_history[-1].start_time
                    if self.repair_history[-1].errors_detected > 0
                    else None
                ),
    ...
    async def _check_emergency_stop_condition(self) -> bool:
        ...
        # 連続失敗回数チェック
        if (
            self.loop_state["consecutive_failures"]
            >= self.config["emergency_stop_threshold"]
        ):
            self.logger.critical(
                f"Emergency stop triggered: {self.loop_state['consecutive_failures']} consecutive failures"
            )
        ...
        if self.loop_state["current_cycle"] >= self.config["max_repair_cycles"]:
            self.logger.info(
                f"Maximum repair cycles reached: {self.config['max_repair_cycles']}"
            )
            return True

        # 連続クリーンサイクル達成チェック
        if (
            self.loop_state["error_free_cycles"]
            >= self.config["consecutive_clean_required"]
        ):
            self.logger.info(
                f"System stabilized: {self.loop_state['error_free_cycles']} consecutive clean cycles"
            )
            return True
    ...
    def _calculate_next_cycle_delay(self, cycle_result: RepairCycleResult) -> float:
        ...
        backoff_multiplier = min(
            2 ** self.loop_state["consecutive_failures"],
            self.config["max_cycle_interval"] / base_delay,
        )
    ...
    async def _cleanup_and_finalize(self):
        ...
        try:
            ...
            final_state = {
                ...
                "final_status": (
                    "completed"
                    if not self.loop_state["emergency_stop"]
                    else "emergency_stop"
                ),
            }
            ...
            self.logger.info(
                f"Infinite repair loop finalized: {final_state['final_status']}"
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_repairs": len(self.repair_history),
            "last_repair": (
                self.repair_history[-1].cycle_id if self.repair_history else None
            ),
        }
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/infinite_repair_controller.log"
            ),
