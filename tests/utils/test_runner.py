...
class ITSMTestRunner:
    ...
    def load_config(self, config_path: str = None) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "reporting": {"html": True, "json": True, "junit": True, "coverage": True},
            "quality_gates": {
                "min_coverage": 80,
                "max_failed_tests": 0,
    ...
    def run_test_suite(
        ...
        if self.config["reporting"]["html"]:
            cmd.extend(
                [
                    "--html",
                    f"tests/reports/{suite_name}-report.html",
                    "--self-contained-html",
                ]
            )
    ...
    def run_all_suites(
        self, suites: List[str] = None, parallel: bool = False
    ) -> Dict[str, Any]:
    ...
    def generate_report(
        self, output_path: str = "tests/reports/execution-summary.json"
    ):
...
def main():
    ...
    parser.add_argument(
        "--parallel", action="store_true", help="Run compatible suites in parallel"
    )
