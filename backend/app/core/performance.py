    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slow_request_rate": round(
                slow_requests / len(self.request_times) * 100, 2
            ),
...
class QueryOptimizer:
    ...
    def optimize_user_query(db: Session, base_query):
        """ユーザークエリの最適化"""
        return base_query.options(
            selectinload("reported_incidents"), selectinload("assigned_incidents")
        )
...
class MemoryOptimizer:
    ...
    @staticmethod
    def selective_fields(
        data: Dict[str, Any], fields: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """指定されたフィールドのみを返す"""
        if fields is None:
            return data
