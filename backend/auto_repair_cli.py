...
class AutoRepairCLI:
    ...
    async def test_api(self, args):
        ...
        try:
            ...
            print("\n📊 テスト結果:")
            print(f"  ヘルスチェック: {'✅' if test_results['health_check']['success'] else '❌'}")
            
            for endpoint_result in test_results['api_endpoints']:
                status = '✅' if endpoint_result['success'] else '❌'
                print(f"  {endpoint_result['method']} {endpoint_result['endpoint']}: {status}")
            
            print(f"  負荷テスト: {'✅' if test_results['load_test']['success'] else '❌'}")
            print(f"  バリデーションテスト: {'✅' if test_results['validation_test']['success'] else '❌'}")
    ...
    async def generate_report(self, args):
        ...
        try:
            ...
            if args.dashboard:
                ...
                dashboard_path = Path(self.project_root) / "tests" / "reports" / "dashboard.json"
