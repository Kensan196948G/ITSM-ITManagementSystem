...
class InfiniteErrorRepairSystem:
    ...
    def __init__(self):
        ...
        self.system_metrics_file = (
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/system_metrics.json"
        )
    ...
    async def start_system(self):
        ...
        try:
            self.logger.info(
                f"Starting {self.config['system_name']} v{self.config['version']}"
            )
            ...
            if not initial_health["healthy"]:
                self.logger.warning(
                    f"Initial health check failed: {initial_health['issues']}"
                )

                if (
                    self.config["emergency_stop_on_critical"]
                    and initial_health["critical"]
                ):
                    self.logger.critical(
                        "Critical issues detected during startup - aborting"
                    )
                    return False
    ...
    async def _restore_previous_state(self):
        ...
        try:
            ...
            if previous_state.get("status") == "running":
                self.logger.warning(
                    "Previous session ended abnormally - performing recovery"
                )
            ...
            if "metrics" in previous_state:
                ...
                self.metrics["restart_count"] = (
                    saved_metrics.get("restart_count", 0) + 1
                )
                self.metrics["total_uptime"] += saved_metrics.get("total_uptime", 0)
                self.metrics["errors_handled"] += saved_metrics.get("errors_handled", 0)
                self.metrics["repairs_executed"] += saved_metrics.get(
                    "repairs_executed", 0
                )
                self.metrics["emergency_stops"] += saved_metrics.get(
                    "emergency_stops", 0
                )
    ...
    async def _perform_initial_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                test_file = Path(
                    "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/health_check.tmp"
                )
                ...
                health_result["components"]["filesystem"] = {
                    "status": "failed",
                    "error": str(e),
                }

            self.logger.info(
                f"Initial health check completed: {'PASS' if health_result['healthy'] else 'FAIL'}"
            )
    ...
    async def _perform_crash_recovery(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Database integrity check failed during recovery: {str(e)}"
                )
        ...
        try:
            ...
            while self.system_running and not self.shutdown_requested:
                try:
                    ...
                    restart_attempts += 1
                    self.logger.error(
                        f"MCP monitoring failed (attempt {restart_attempts}): {str(e)}"
                    )

                    if restart_attempts >= self.config["max_restart_attempts"]:
                        self.logger.critical(
                            "Max restart attempts reached for MCP monitoring"
                        )
        ...
        try:
            ...
            while self.system_running and not self.shutdown_requested:
                try:
                    ...
                    restart_attempts += 1
                    self.logger.error(
                        f"Loop controller failed (attempt {restart_attempts}): {str(e)}"
                    )

                    if restart_attempts >= self.config["max_restart_attempts"]:
                        self.logger.critical(
                            "Max restart attempts reached for loop controller"
                        )
                        if self.config["emergency_stop_on_critical"]:
                            self.metrics["emergency_stops"] += 1
                            self.shutdown_requested = True
            ...
            metrics_data = {
                ...
                "component_status": {
                    ...
                    "loop_controller": {
                        ...
                        "current_cycle": self.loop_controller.loop_state[
                            "current_cycle"
                        ],
                        "total_cycles": self.loop_controller.loop_state["total_cycles"],
                        "error_free_cycles": self.loop_controller.loop_state[
                            "error_free_cycles"
                        ],
                    },
                },
    ...
    async def _cleanup_system(self):
        ...
        try:
            ...
            final_state = {
                ...
                "status": "stopped",
                "shutdown_reason": (
                    "graceful" if not self.shutdown_requested else "signal"
                ),
