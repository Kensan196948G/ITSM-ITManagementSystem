...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("master-infinite-loop.log"), logging.StreamHandler()],
)
...
class MasterInfiniteLoopController:
    ...
    def __init__(self):
        ...
        self.state = {
            ...
            "api_status": "unknown",
            "processes": {},
            "error_counts": {"webui": 0, "api": 0, "critical": 0},
        }
    ...
    async def run_webui_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # WebUI基本健全性チェック
            webui_endpoints = [self.config["webui_url"], self.config["admin_url"]]
                    ...
                    try:
                        async with session.get(
                            ...
                            if response.status == 200:
                                ...
                                logger.warning(
                                    f"WebUI エンドポイント {endpoint}: 異常 (ステータス: {response.status})"
                                )
                    except Exception as e:
                        logger.warning(
                            f"WebUI エンドポイント {endpoint}: エラー - {str(e)}"
                        )
            ...
            if healthy_endpoints > 0:
                ...
                logger.info(
                    f"WebUI監視完了: {healthy_endpoints}/{total_endpoints} エンドポイント正常 ({success_rate:.1f}%)"
                )
                return {
                    "status": "success",
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                    ...
                    try:
                        ...
                        async with session.get(
                            ...
                            if response.status in [
                                ...
                                healthy_endpoints += 1
                                logger.info(
                                    f"API エンドポイント {endpoint}: 正常 (ステータス: {response.status})"
                                )
                            else:
                                logger.warning(
                                    f"API エンドポイント {endpoint}: 異常 (ステータス: {response.status})"
                                )
                    except Exception as e:
                        logger.warning(
                            f"API エンドポイント {endpoint}: エラー - {str(e)}"
                        )
                ...
                if healthy_endpoints > 0:
                    success_rate = (healthy_endpoints / total_endpoints) * 100
                    logger.info(
                        f"API監視完了: {healthy_endpoints}/{total_endpoints} エンドポイント正常 ({success_rate:.1f}%)"
                    )
                    return {
                        "status": "success",
                        "healthy_endpoints": healthy_endpoints,
    ...
    async def perform_emergency_repair(self) -> Dict[str, Any]:
        ...
        try:
            ...
            repair_results.append(
                {"component": "webui", "status": "error", "error": str(e)}
            )
        ...
        try:
            ...
            repair_results.append(
                {"component": "api", "status": "error", "error": str(e)}
            )
    ...
    async def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report_file = (
                self.base_dir
                / f"master-comprehensive-report-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    async def monitoring_cycle(self):
        ...
        try:
            ...
            # 健全性チェック
            webui_health = await self.check_service_health(
                self.config["webui_url"], "webui"
            )
            ...
            if tasks:
                ...
                if success_count > 0:
                    ...
                    logger.info(
                        f"監視サイクル完了: {success_count}/{len(results)} 成功"
                    )
                else:
                    self.state["consecutive_failures"] += 1
                    logger.warning(
                        f"監視サイクル失敗: 連続失敗回数 {self.state['consecutive_failures']}"
                    )

            # 緊急修復判定
            if (
                self.state["consecutive_failures"]
                >= self.config["emergency_repair_threshold"]
            ):
