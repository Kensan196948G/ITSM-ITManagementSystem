...
class UserCreate(UserBase):
    ...
    password_expiry_days: Optional[int] = Field(
        None, gt=0, le=365, description="パスワード有効期限（日数）"
    )
...
class UserUpdate(BaseModel):
    ...
    first_name: Optional[str] = Field(
        None, min_length=1, max_length=50, description="名"
    )
    last_name: Optional[str] = Field(
        None, min_length=1, max_length=50, description="姓"
    )
    ...
    password: Optional[str] = Field(
        None, min_length=8, max_length=128, description="パスワード"
    )
    password_expiry_days: Optional[int] = Field(
        None, gt=0, le=365, description="パスワード有効期限（日数）"
    )
...
class UserLoginRequest(BaseModel):
    ...
    two_factor_code: Optional[str] = Field(
        None, min_length=6, max_length=6, description="二要素認証コード"
    )
...
class PasswordChangeRequest(BaseModel):
    ...
    confirm_password: str = Field(
        ..., min_length=8, max_length=128, description="新しいパスワード（確認）"
    )
...
class UserPreferencesUpdate(BaseModel):
    ...
    theme: Optional[str] = Field(
        None, pattern="^(light|dark|auto)$", description="テーマ"
    )
    ...
    date_format: Optional[str] = Field(
        None, max_length=20, description="日付フォーマット"
    )
    items_per_page: Optional[int] = Field(
        None, ge=10, le=100, description="1ページあたりの表示件数"
    )
...
class UserPerformanceMetrics(BaseModel):
    ...
    avg_resolution_time: Optional[float] = Field(
        None, description="平均解決時間（時間）"
    )
    sla_compliance_rate: Optional[float] = Field(None, description="SLA遵守率（%）")
    tickets_resolved_last_30_days: int = Field(
        0, description="過去30日の解決チケット数"
    )
    customer_satisfaction_score: Optional[float] = Field(
        None, description="顧客満足度スコア"
    )
    first_contact_resolution_rate: Optional[float] = Field(
        None, description="初回解決率（%）"
    )
...
class UserDetailResponse(UserResponse):
    ...
    performance_metrics: UserPerformanceMetrics = Field(
        default_factory=UserPerformanceMetrics
    )
