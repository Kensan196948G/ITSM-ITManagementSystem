...
class APITestClient:
    ...
    async def authenticate(self, email: str = "test@example.com", password: str = "password123") -> bool:
        """認証実行"""
...
class TestComprehensiveAPIFlow:
    ...
    async def test_complete_incident_lifecycle(self, authenticated_client):
        ...
        # 2. インシデント詳細取得
        detail_response = await authenticated_client.get(f"/api/v1/incidents/{incident_id}")
        ...
        # 4. インシデント削除
        delete_response = await authenticated_client.delete(f"/api/v1/incidents/{incident_id}")
