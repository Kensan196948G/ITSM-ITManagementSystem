...
class TestAPILoadPerformance:
        ...
        def api_call():
            with patch("requests.get") as mock_get:
                mock_get.return_value = self.mock_api_response(
                    response_data=mock_response_data, delay=0.2
                )
    ...
    def test_incident_detail_performance(self, benchmark, test_config, api_headers):
        ...
        def api_call():
            with patch("requests.get") as mock_get:
                mock_get.return_value = self.mock_api_response(
                    response_data=mock_response_data, delay=0.15
                )
                response = requests.get(
    ...
    def test_incident_creation_performance(
        self, benchmark, test_config, api_headers, sample_incident_data
    ):
        ...
        def api_call():
            with patch("requests.post") as mock_post:
                mock_post.return_value = self.mock_api_response(
                    status_code=201, response_data=mock_response_data, delay=0.3
                )
        ...
        def api_call():
            with patch("requests.patch") as mock_patch:
                mock_patch.return_value = self.mock_api_response(
                    response_data=mock_response_data, delay=0.25
                )
    ...
    def test_concurrent_incident_creation(
        self, test_config, api_headers, load_test_data, performance_config
    ):
            ...
            try:
                ...
                with patch("requests.post") as mock_post:
                    mock_post.return_value = self.mock_api_response(
                        ...
                        delay=0.2
                        + (time.time() % 0.3),  # Simulate variable response time
                    )
        ...
        with ThreadPoolExecutor(
            ...
            for i in range(performance_config["requests_per_user"]):
                incident_data = load_test_data["incidents"][
                    i % len(load_test_data["incidents"])
                ]
                ...
                if i < performance_config["ramp_up_time"]:
                    time.sleep(
                        performance_config["ramp_up_time"]
        ...
        assert (
            avg_response_time <= performance_config["acceptable_response_time"]
        ...
        assert (
            p95_response_time <= performance_config["acceptable_response_time"] * 2
    ...
    def test_read_heavy_load(self, test_config, api_headers, performance_config):
        ...
        ), f"Read response time {avg_response_time:.3f}s too slow"
    ...
    def test_sustained_load(self, test_config, api_headers, performance_config):
        ...
        while time.time() - start_test_time < test_duration:
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for _ in range(5):  # 5 requests per interval
                    future = executor.submit(
                        self._single_api_request, test_config, api_headers
                    )
                    futures.append(future)
            ...
            success_rate = (
                len(successful) / len(interval_results) if interval_results else 0
            )
            avg_response_time = (
        ...
        overall_success_rate = statistics.mean(
            [r["success_rate"] for r in results_over_time]
        )
        overall_avg_response_time = statistics.mean(
            [r["avg_response_time"] for r in results_over_time]
        )
        ...
        ), f"Sustained load success rate {overall_success_rate:.3f} too low"
        assert (
            performance_degradation <= 0.5
        ), f"Performance degradation {performance_degradation:.3f} too high"
    ...
    def _single_api_request(self, test_config, api_headers):
        ...
        try:
            ...
            with patch("requests.get") as mock_get:
                mock_get.return_value = self.mock_api_response(
                    response_data=mock_response_data, delay=0.1 + (time.time() % 0.2)
                )
    ...
    def test_memory_usage_during_load(self, test_config, api_headers):
        ...
        ), f"Total memory increase {total_memory_increase:.2f}MB too high"
