...
class RepairCoordinator:
    ...
    async def execute_repairs(self) -> List[RepairTask]:
        ...
        # 同時実行数制限
        while (len(self.running_tasks) < self.max_concurrent_repairs and 
               self.repair_queue):
...
class InfiniteAutoRepairSystem:
    ...
    def _get_priority_from_severity(self, severity) -> int:
        ...
        return priority_map.get(severity.value if hasattr(severity, 'value') else str(severity), 5)
    ...
    def _get_priority_from_level(self, level) -> int:
        ...
        return priority_map.get(level.value if hasattr(level, 'value') else str(level), 5)
    ...
    async def _collect_system_health(self) -> SystemHealth:
        ...
        return SystemHealth(
            ...
            repair_history=[task.to_dict() for task in self.repair_coordinator.repair_history[-10:]]
        )
    
    def _determine_overall_status(self, api_health, db_health, perf_health, 
                                security_health, log_health) -> SystemStatus:
        """全体ステータス判定"""
        # 重要なヘルス指標をチェック
        critical_indicators = [
    ...
    async def _update_system_status(self, health: SystemHealth):
        ...
        if old_status != self.system_status:
            logger.warning(f"System status changed: {old_status.value} -> {self.system_status.value}")
    ...
    async def _log_repair_results(self, completed_tasks: List[RepairTask]):
        ...
        for task in completed_tasks:
            ...
            log_file = Path("repair_logs") / f"repair_log_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    async def _update_statistics(self):
        ...
        stats = {
            ...
            "successful_repairs": len([t for t in self.repair_coordinator.repair_history if t.success]),
