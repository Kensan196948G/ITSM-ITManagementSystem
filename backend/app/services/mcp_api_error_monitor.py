    ...
    def save_error(self, error: APIError) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                error.timestamp, error.endpoint, error.method, error.status_code,
                error.error_message, error.response_time, error.category.value,
                error.severity.value, json.dumps(error.details), error.request_id
            ))
    ...
    def save_performance_metrics(self, metrics: PerformanceMetrics):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                metrics.endpoint, metrics.avg_response_time, metrics.max_response_time,
                metrics.min_response_time, metrics.error_rate, metrics.request_count,
                metrics.timestamp
            ))
...
class APIErrorMonitor:
    ...
    def categorize_error(self, status_code: int, error_message: str, response_time: float) -> tuple[ErrorCategory, ErrorSeverity]:
        """エラーのカテゴリと重要度を判定"""
        # カテゴリ判定
        ...
        elif status_code in [401, 403] or response_time > self.thresholds["response_time_critical"]:
            severity = ErrorSeverity.HIGH
        elif status_code in [400, 422, 429] or response_time > self.thresholds["response_time_medium"]:
        ...
        for endpoint, method in self.endpoints:
            try:
                ...
                if error:
                    ...
                    logger.warning(f"Error detected: {error.endpoint} - {error.error_message}")
    ...
    async def analyze_and_repair(self, errors: List[APIError]):
        ...
        for error in errors:
            ...
            if repair_action:
                ...
                logger.info(f"Repair action {repair_action} {'succeeded' if success else 'failed'} for {error.endpoint}")
    ...
    async def calculate_performance_metrics(self):
        ...
        for endpoint, _ in self.endpoints:
            ...
            if endpoint_errors:
                ...
                error_count = len([e for e in endpoint_errors if e['status_code'] >= 400])
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        for endpoint, _ in self.endpoints:
            ...
            endpoint_stats[endpoint] = {
                ...
                "avg_response_time": sum(e['response_time'] for e in endpoint_errors) / max(1, len(endpoint_errors))
            }
        ...
        return {
            ...
            "overall_health": "critical" if critical_errors > 5 else "healthy" if total_errors < 10 else "warning",
