...
class InfiniteLoopControlSystem:
        ...
        try:
            ...
            while self.master_active:
                try:
                    ...
                    self.logger.info(
                        f"Starting master control loop #{master_loop_count}"
                    )
                    ...
                    self.logger.error(
                        f"Master control loop #{master_loop_count} failed: {str(e)}"
                    )
                    ...
                    wait_time = min(
                        300,
                        self.master_loop_interval * (2 ** min(consecutive_failures, 5)),
                    )
    ...
    async def start_all_monitoring_systems(self):
        ...
        # 起動完了の確認
        running_systems = len(
            [name for name, active in self.systems_active.items() if active]
        )
        self.logger.info(
            f"Monitoring systems startup completed: {running_systems}/{len(self.monitoring_systems)} systems running"
        )
        ...
        try:
            ...
            if system_name == "mcp_playwright":
                ...
                task = asyncio.create_task(
                    system.start_infinite_performance_monitoring()
                )
    ...
    async def stop_monitoring_system(self, system_name: str):
        ...
        try:
            ...
            if system_name in self.system_statuses:
                ...
                self.system_statuses[system_name].last_activity = (
    ...
    async def perform_health_check(self):
        ...
        try:
            ...
            for system_name, system in self.monitoring_systems.items():
                if (
                    system_name not in self.systems_active
                    or not self.systems_active[system_name]
                ):
                    continue
                ...
                if hasattr(self, task_attr):
                    ...
                    if task.done() and not task.cancelled():
                        ...
                        self.logger.warning(
                            f"System {system_name} task has terminated unexpectedly"
                        )
                ...
                if system_name in self.system_statuses:
                    ...
                    try:
                        ...
                        status.memory_usage_mb = process.memory_info().rss / (
                            1024 * 1024
                        )
            ...
            for system_name, system in self.monitoring_systems.items():
                ...
                try:
                    if system_name == "mcp_playwright":
                        ...
                        health_score = max(
                            50.0, 100.0 - error_count * 3 + repair_count * 2
                        )
                        ...
                        error_entries = len(
                            [e for e in log_entries if e.level in ["ERROR", "CRITICAL"]]
                        )
                    ...
                    self.logger.warning(
                        f"Failed to collect status for {system_name}: {str(e)}"
                    )
    ...
    async def evaluate_overall_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for system_name, status in self.system_statuses.items():
                if self.systems_active.get(system_name, False):
                    ...
                    if (
                        ...
                        and status.health_score
                        >= self.performance_thresholds["min_health_score"]
                    ):
                        ...
                        overall_health["critical_issues"].append(
                            f"System {system_name} in error state"
                        )
                    ...
                    overall_health["system_details"][system_name] = {
                        ...
                        "uptime_hours": (
                            status.uptime_seconds / 3600 if status.uptime_seconds else 0
                        ),
                    }
                    ...
                    if (
                        ...
                        overall_health["warnings"].append(
                            f"Low health score for {system_name}: {status.health_score:.1f}"
                        )
                    ...
                    if (
                        ...
                        overall_health["warnings"].append(
                            f"High memory usage for {system_name}: {status.memory_usage_mb:.1f}MB"
                        )
                    ...
                    if (
                        ...
                        overall_health["warnings"].append(
                            f"High CPU usage for {system_name}: {status.cpu_usage_percent:.1f}%"
                        )

            # 全体健康スコアの計算
            if overall_health["systems_running"] > 0:
                overall_health["overall_health_score"] = (
                    total_health_score / overall_health["systems_running"]
                )
            ...
            self.logger.info(
                f"Overall health evaluation: {overall_health['overall_status']} ({overall_health['overall_health_score']:.1f})"
            )
    ...
    async def monitor_and_optimize_performance(self):
        ...
        try:
            ...
            self.logger.error(
                f"Performance monitoring and optimization failed: {str(e)}"
            )
        ...
        try:
            ...
            for system_name, system in self.monitoring_systems.items():
                ...
                if hasattr(system, "monitoring_interval"):
                    ...
                    system.monitoring_interval = min(
                        300, original_interval * 1.5
                    )  # 1.5倍に延長
                    self.logger.info(
                        f"Adjusted monitoring interval for {system_name}: {original_interval}s -> {system.monitoring_interval}s"
                    )
    ...
    async def _optimize_system_performance(self, system_name: str):
        ...
        try:
            ...
            self.logger.error(
                f"Performance optimization failed for {system_name}: {str(e)}"
            )
    ...
    async def detect_system_anomalies(self):
        ...
        try:
            ...
            for system_name, status in self.system_statuses.items():
                ...
                if status.uptime_seconds > 0:
                    ...
                    if error_rate > 10:  # 1時間に10エラー以上
                        anomalies.append(
                            f"High error rate for {system_name}: {error_rate:.1f} errors/hour"
                        )

                # ヘルススコアの急激な低下
                if status.health_score < 30:
                    anomalies.append(
                        f"Critical health score for {system_name}: {status.health_score:.1f}"
                    )

                # システム停止の検知
                if (
                    not self.systems_active.get(system_name, False)
                    and system_name in self.system_priority
                ):
            ...
            running_critical_systems = sum(
                1
                for name in ["mcp_playwright", "database_repair", "security_monitor"]
                if self.systems_active.get(name, False)
            )

            if running_critical_systems < 2:
                anomalies.append(
                    f"Too few critical systems running: {running_critical_systems}/3"
                )
                ...
                if not self.systems_active.get(system_name, False):
                    try:
                        ...
                        recovery_actions.append(
                            f"Failed to restart {system_name}: {str(e)}"
                        )

            # ヘルススコアが低いシステムの復旧
            for system_name, status in self.system_statuses.items():
                if status.health_score < 30 and self.systems_active.get(
                    system_name, False
                ):
                    try:
                        await self.restart_monitoring_system(system_name)
                        recovery_actions.append(
                            f"Recovered low-health system {system_name}"
                        )
                    except Exception as e:
                        recovery_actions.append(
                            f"Failed to recover {system_name}: {str(e)}"
                        )
        ...
        try:
            ...
            for system_name in list(self.systems_active.keys()):
                try:
                    ...
                    self.logger.error(
                        f"Error stopping {system_name} during emergency: {str(e)}"
                    )
            ...
            for system_name in critical_systems:
                try:
                    ...
                    self.logger.error(
                        f"Failed to restart {system_name} during emergency: {str(e)}"
                    )
    ...
    async def update_control_metrics(self, loop_time: float, success: bool):
        ...
        try:
            ...
            successful_loops = len(
                [m for m in self.loop_metrics if m.successful_loops > 0]
            ) + (1 if success else 0)
            failed_loops = total_loops - successful_loops

            # 平均ループ時間
            all_loop_times = [m.average_loop_time for m in self.loop_metrics] + [
                loop_time
            ]
            ...
            systems_running = len([s for s in self.systems_active.values() if s])
            systems_healthy = len(
                [s for s in self.system_statuses.values() if s.health_score >= 60]
            )

            # 全体健康スコア
            if self.system_statuses:
                overall_health_score = sum(
                    s.health_score for s in self.system_statuses.values()
                ) / len(self.system_statuses)
            else:
    ...
    async def save_control_state(self):
        ...
        try:
            state_data = {
                ...
                "loop_metrics": [
                    asdict(m) for m in self.loop_metrics[-10:]
                ],  # 最新10件
        ...
        try:
            if os.path.exists(self.control_state_file):
                ...
                if "control_settings" in state_data:
                    ...
                    self.master_loop_interval = settings.get(
                        "master_loop_interval", self.master_loop_interval
                    )
                    self.health_check_interval = settings.get(
                        "health_check_interval", self.health_check_interval
                    )
                    self.auto_recovery_enabled = settings.get(
                        "auto_recovery_enabled", self.auto_recovery_enabled
                    )
                ...
                if "performance_thresholds" in state_data:
                    self.performance_thresholds.update(
                        state_data["performance_thresholds"]
                    )
        ...
        try:
            report = {
                ...
                "system_summary": {
                    ...
                    "running_systems": len(
                        [s for s in self.systems_active.values() if s]
                    ),
                ...
                "performance_metrics": (
                    asdict(self.loop_metrics[-1]) if self.loop_metrics else None
                ),
                "resource_usage": {
            ...
            self.logger.info(
                f"Master report generated: {overall_health.get('overall_status', 'unknown')} health status"
            )
    ...
    def _generate_master_recommendations(
        self, overall_health: Dict[str, Any]
    ) -> List[str]:
        """マスター推奨事項生成"""
        recommendations = []
        ...
        try:
            ...
            if overall_health.get("systems_degraded", 0) > 1:
                recommendations.append(
                    "Multiple systems showing degraded performance - check resource allocation"
                )

            if overall_health.get("overall_health_score", 0) < 60:
                recommendations.append(
                    "Overall system health is low - immediate attention required"
                )
            ...
            if self.loop_metrics:
                ...
                if latest_metrics.error_rate > 0.1:
                    recommendations.append(
                        "High error rate in master control loop - investigate causes"
                    )

                if latest_metrics.average_loop_time > 120:
                    recommendations.append(
                        "Master control loop taking too long - optimize performance"
                    )
            ...
            if cpu_percent > self.performance_thresholds["max_cpu_usage_percent"]:
                recommendations.append(
                    "High CPU usage - consider reducing monitoring frequency"
                )
    ...
    async def graceful_shutdown(self):
        """グレースフルシャットダウン"""
        self.logger.info(
            "Initiating graceful shutdown of Infinite Loop Control System..."
        )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/infinite_loop_master.log"
            ),
