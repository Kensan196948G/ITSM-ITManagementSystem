...
class InfiniteLoopSystemTester:
    ...
    def test_url_accessibility(self) -> bool:
        ...
        details = (
            f"アクセス可能: {accessible_count}/{len(urls_to_test)} - "
            + " | ".join(results)
        )
    ...
    def test_python_imports(self) -> bool:
        ...
        required_modules = ["requests", "pydantic", "fastapi", "uvicorn", "sqlalchemy"]
        ...
        details = (
            f"インポート可能: {import_count}/{len(required_modules)} - "
            + " | ".join(import_results)
        )
        ...
        try:
            ...
            if Path("infinite_error_monitoring_orchestrator.py").exists():
                ...
                spec = importlib.util.spec_from_file_location(
                    "orchestrator", "infinite_error_monitoring_orchestrator.py"
                )
    ...
    async def run_all_tests(self):
        ...
        for test_func in tests:
            try:
                ...
                self.log_test_result(test_func.__name__, False, f"例外発生: {str(e)}")
