...
@router.get(
    ...
    responses={
        200: {
            "description": "ファイルを正常にダウンロードしました",
...
async def download_attachment(
    ...
    media_type = (
        ...
        or mimetypes.guess_type(attachment.file_name)[0]
...
async def delete_attachment(
    ...
    permanent: bool = Query(
        False, description="物理削除するか（論理削除がデフォルト）"
    ),
    db: Session = Depends(get_db),
    current_user_id: UUID = Depends(get_current_user_id),
) -> SuccessResponse:
...
@router.post(
    ...
    responses={
        ...
        400: {
            "model": APIError,
            "description": "削除されていないファイルは復元できません",
        },
        500: {"model": APIError, "description": "サーバーエラーが発生しました"},
    },
)
...
@router.get(
    ...
    responses={
        ...
        415: {
            "model": APIError,
            "description": "プレビューに対応していないファイル形式です",
        },
        500: {"model": APIError, "description": "サーバーエラーが発生しました"},
    },
)
async def preview_attachment(
    ...
    size: Optional[str] = Query(
        None, pattern="^(thumbnail|small|medium|large)$", description="プレビューサイズ"
    ),
    db: Session = Depends(get_db),
    current_user_id: UUID = Depends(get_current_user_id),
):
...
async def bulk_delete_attachments(
    ...
    """添付ファイルを一括削除する"""
    service = AttachmentService(db)
    deleted_count = service.bulk_delete_attachments(
        attachment_ids, current_user_id, permanent
    )
