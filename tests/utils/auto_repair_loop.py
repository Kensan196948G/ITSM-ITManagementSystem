...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/logs/auto_repair.log'),
...
# 設定定数
ERROR_INPUT_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/errors.json"
REPAIR_OUTPUT_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/repair_tasks.json"
REPAIR_STATUS_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/repair_status.json"
REPAIR_HISTORY_FILE = "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination/repair_history.json"
            ...
            for pattern in patterns:
                if pattern["error"].lower() in error_message.lower():
                    repair_suggestion.update({
                        ...
                        "estimated_effort": self._estimate_effort(pattern["priority"])
                    })
                    break
...
class AutoRepairLoop:
    ...
    def process_errors(self) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if error_id in existing_repairs:
                        ...
                        existing_repairs[error_id].update({
                            ...
                            "occurrence_count": existing_repairs[error_id].get("occurrence_count", 1) + 1
                        })
                        updated_repairs += 1
                    else:
                        # 新しい修復タスクを作成
            ...
            logger.info(f"修復タスク処理完了: 新規{len(new_repairs)}件、更新{updated_repairs}件")
    ...
    def _save_repair_tasks(self, repairs: Dict[str, Any]) -> None:
        ...
        try:
            ...
            repair_data = {
                ...
                "pending_tasks": len([r for r in sorted_repairs if r.get("status") == "pending"]),
                "in_progress_tasks": len([r for r in sorted_repairs if r.get("status") == "in_progress"]),
                "completed_tasks": len([r for r in sorted_repairs if r.get("status") == "completed"]),
    ...
    def _generate_agent_summary(self, repairs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        summary = {
            "ITSM-DevUI": {"pending": 0, "in_progress": 0, "completed": 0, "high_priority": 0},
            "ITSM-DevAPI": {"pending": 0, "in_progress": 0, "completed": 0, "high_priority": 0}
        }
        ...
        for repair in repairs:
            ...
            if agent not in summary:
                summary[agent] = {"pending": 0, "in_progress": 0, "completed": 0, "high_priority": 0}
        ...
        try:
            ...
            for agent, tasks in agent_tasks.items():
                ...
                agent_data = {
                    ...
                    "high_priority_tasks": len([t for t in sorted_tasks if t.get("priority") == "high"]),
                    "pending_tasks": len([t for t in sorted_tasks if t.get("status") == "pending"]),
                    "instructions": self._generate_agent_instructions(agent, sorted_tasks),
                    "tasks": sorted_tasks
                }
    ...
    def _generate_agent_instructions(self, agent: str, tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """エージェント向けの指示を生成
    ...
    def _calculate_total_effort(self, tasks: List[Dict[str, Any]]) -> str:
        ...
        total_points = sum(effort_points.get(t.get("estimated_effort", "medium"), 3) for t in tasks)
    ...
    def update_repair_status(self, error_id: str, status: str, 
        ...
        try:
            ...
            if status == "completed":
                repairs[error_id]["completed_at"] = datetime.now(timezone.utc).isoformat()
...
def main():
    ...
    parser.add_argument("--process", action="store_true", help="エラーを処理して修復タスクを生成")
    parser.add_argument("--status", nargs=3, metavar=("ERROR_ID", "STATUS", "NOTES"), help="修復ステータスを更新")
