...
class TestHealthChecker:
    ...
    def extract_test_metrics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get("coverage"):
            ...
            if "totals" in coverage_data:
                metrics["coverage_percent"] = coverage_data["totals"].get("percent_covered", 0)
    ...
    def _extract_suite_metrics(self, suite_name: str, suite_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def evaluate_quality_gates(self, metrics: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if success_rate < threshold:
            issues.append(f"Test success rate {success_rate:.1f}% below threshold {threshold}%")
            passed = False
        ...
        if failed_tests > threshold:
            issues.append(f"Failed tests count {failed_tests} exceeds threshold {threshold}")
            passed = False
        ...
        if security_issues > threshold:
            issues.append(f"Security issues count {security_issues} exceeds threshold {threshold}")
            passed = False
        ...
        if duration > threshold:
            issues.append(f"Test duration {duration:.1f}s exceeds threshold {threshold}s")
            passed = False
        ...
        for suite_name in critical_suites:
            ...
            if suite_metrics and suite_metrics.get("success_rate", 0) < 90:
                issues.append(f"Critical suite {suite_name} success rate {suite_metrics.get('success_rate', 0):.1f}% below 90%")
                passed = False
        
        return passed, issues
    
    def generate_health_report(self, metrics: Dict[str, Any], quality_passed: bool, issues: List[str]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, metrics: Dict[str, Any], issues: List[str]) -> List[str]:
        ...
        if metrics["success_rate"] < 95:
            recommendations.append("Review and fix failed tests to improve success rate")
        ...
        if metrics["total_duration"] > 300:  # 5 minutes
            recommendations.append("Optimize test execution time through parallelization or test selection")
        ...
        for suite_name, suite_metrics in metrics["suite_metrics"].items():
            ...
            if suite_metrics.get("duration", 0) > 180:  # 3 minutes per suite
                recommendations.append(f"Optimize {suite_name} test suite execution time")
    ...
    def _get_next_steps(self, quality_passed: bool, issues: List[str]) -> List[str]:
        ...
        if quality_passed:
            ...
            steps.extend([
                ...
                "Review test strategy if issues persist"
            ])
            return steps
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        logger.info(f"Extracted metrics: {metrics['total_tests']} tests, {metrics['success_rate']:.1f}% success rate")
        
        # Evaluate quality gates
        quality_passed, issues = self.evaluate_quality_gates(metrics)
        logger.info(f"Quality gates: {'PASSED' if quality_passed else 'FAILED'} ({len(issues)} issues)")
...
def main():
    parser = argparse.ArgumentParser(description="Test health checker and quality gate validator")
    parser.add_argument("--reports-dir", required=True, help="Directory containing test reports")
    parser.add_argument("--output", help="Output file for health report (JSON)")
    parser.add_argument("--quiet", action="store_true", help="Suppress output")
    parser.add_argument("--fail-on-quality-gate", action="store_true", help="Exit with error code if quality gates fail")
