...
class ApiErrorMonitor:
    ...
    def __init__(self, base_url: str = "http://192.168.3.135:8000"):
        ...
        self.security_patterns = {
            ...
            "path_traversal": [r"\.\./", r"\.\.\\", r"etc/passwd", r"windows/system32"],
            "dos_attack": [
    ...
    async def _parse_log_line(self, line: str, log_type: str) -> Optional[ApiError]:
        """ログ行を解析してエラー情報を抽出"""
        if not any(
            level in line.lower() for level in ["error", "exception", "traceback"]
        ):
            return None
    ...
    def _is_duplicate_error(self, error: ApiError) -> bool:
        ...
        for existing_error in self.errors:
            if (
                existing_error.error_type == error.error_type
                and existing_error.endpoint == error.endpoint
                and abs((existing_error.timestamp - error.timestamp).total_seconds())
                < 60
            ):
                return True
        return False
        ...
        for error in unfixed_errors:
            try:
                ...
                if fix_result["success"]:
                    logger.info(
                        f"✅ エラー修復成功: {error.error_type} - {fix_result['description']}"
                    )
                else:
                    logger.warning(
                        f"❌ エラー修復失敗: {error.error_type} - {fix_result['description']}"
                    )
    ...
    async def _fix_database_error(self, error: ApiError) -> Dict[str, Any]:
        ...
        try:
            ...
            if not db_path.exists():
                ...
                if init_script.exists():
                    ...
                    if result.returncode == 0:
                        ...
                        return {
                            "success": False,
                            "description": f"データベース初期化失敗: {result.stderr}",
                        }
            ...
            try:
                ...
                return {
                    "success": False,
                    "description": f"データベース接続エラー: {str(e)}",
                }

        except Exception as e:
            return {
                "success": False,
                "description": f"データベース修復エラー: {str(e)}",
            }
    ...
    async def _fix_validation_error(self, error: ApiError) -> Dict[str, Any]:
        ...
        try:
            return {
                "success": True,
                "description": "バリデーションスキーマを確認しました",
            }
        except Exception as e:
            return {
                "success": False,
                "description": f"バリデーション修復エラー: {str(e)}",
            }
    ...
    async def update_metrics(self):
        ...
        try:
            ...
            for error in self.errors:
                ...
                metrics["error_categories"][cat] = (
                    metrics["error_categories"].get(cat, 0) + 1
                )

                sev = error.severity.value
                metrics["error_severities"][sev] = (
                    metrics["error_severities"].get(sev, 0) + 1
                )
            ...
            if attempted_fixes:
                ...
                metrics["fix_success_rate"] = (
                    len(successful_fixes) / len(attempted_fixes) * 100
                )
            ...
            if self.health_history:
                ...
                metrics["health_status"] = (
                    "healthy" if latest_health.is_healthy else "unhealthy"
                )
        ...
        try:
            ...
            for error in recent_errors:
                ...
                if cat not in report["error_breakdown"]:
                    report["error_breakdown"][cat] = {"count": 0, "errors": []}
            ...
            if (
                report["summary"]["fixed_errors"]
                < report["summary"]["total_errors"] * 0.5
            ):
                report["recommendations"].append(
                    "⚠️ Low fix success rate - review error handling"
                )

            if not self.health_history or not self.health_history[-1].is_healthy:
                report["recommendations"].append(
                    "🔄 API health check failing - restart may be needed"
                )
    ...
    async def performance_monitoring(self):
        ...
        try:
                        ...
                        async with session.get(
                            ...
                            if (
                                ...
                                await self._create_performance_alert(
                                    endpoint, response_time, "slow_response"
                                )
            ...
            if memory_usage > self.performance_thresholds["max_memory_usage"]:
                await self._create_performance_alert(
                    "system", memory_usage, "high_memory"
                )

            logger.info(
                f"📊 パフォーマンス監視完了: CPU {cpu_usage}%, メモリ {memory_usage}%"
            )
                    ...
                    async with aiohttp.ClientSession(
                        ...
                        async with session.get(
                            ...
                            if response.status >= 400:
                                await self._create_documentation_error(
    ...
    async def _detect_injection_attempts(self):
        ...
        for error in self.errors:
            if any(
                re.search(pattern, error.message.lower())
                for pattern_list in self.security_patterns["sql_injection"]
    ...
    async def _detect_xss_attempts(self):
        ...
        for error in self.errors:
            if any(
                re.search(pattern, error.message.lower())
                for pattern_list in self.security_patterns["xss_attack"]
                for pattern in [pattern_list]
            ):
                await self._create_security_alert(
                    "xss_attempt",
    ...
    async def _create_performance_alert(
        self, endpoint: str, value: float, alert_type: str
    ):
        """パフォーマンスアラートの作成"""
    ...
    async def _create_documentation_error(
        self, endpoint: str, status_code: int, error_msg: str = ""
    ):
        """ドキュメントエラーの作成"""
        error = ApiError(
            timestamp=datetime.now(),
    ...
    async def update_comprehensive_metrics(self):
        ...
        try:
            ...
            for error in self.errors:
                cat = error.category.value
                metrics["error_metrics"]["error_categories"][cat] = (
                    metrics["error_metrics"]["error_categories"].get(cat, 0) + 1
                )

                sev = error.severity.value
                metrics["error_metrics"]["error_severities"][sev] = (
                    metrics["error_metrics"]["error_severities"].get(sev, 0) + 1
                )
            ...
            if attempted_fixes:
                successful_fixes = [e for e in attempted_fixes if e.fix_successful]
                metrics["error_metrics"]["fix_success_rate"] = (
                    len(successful_fixes) / len(attempted_fixes) * 100
                )
            ...
            for alert in self.security_alerts:
                ...
                metrics["security_metrics"]["alert_types"][alert_type] = (
                    metrics["security_metrics"]["alert_types"].get(alert_type, 0) + 1
                )

            # パフォーマンスメトリクス
            if self.performance_metrics:
                response_times = [m.response_time for m in self.performance_metrics]
                metrics["performance_metrics"]["avg_response_time"] = sum(
                    response_times
                ) / len(response_times)
    ...
    async def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_errors = [e for e in self.errors if e.timestamp > cutoff_time]
            recent_alerts = [
                a for a in self.security_alerts if a.timestamp > cutoff_time
            ]
            ...
            # 脅威レベル判定
            if (
                len([a for a in recent_alerts if a.severity == ErrorSeverity.CRITICAL])
                > 0
            ):
                report["security_analysis"]["threat_level"] = "critical"
            elif (
                len([a for a in recent_alerts if a.severity == ErrorSeverity.HIGH]) > 3
            ):
            ...
            if self.performance_metrics:
                ...
                if recent_perf:
                    ...
                    report["performance_analysis"]["avg_response_time"] = sum(
                        response_times
                    ) / len(response_times)
                    ...
                    report["performance_analysis"]["slow_endpoints"] = [
                        {"endpoint": m.endpoint, "response_time": m.response_time}
                        for m in slow_endpoints
                    ]

            # データベース分析
            if self.database_health_history:
                latest_db = self.database_health_history[-1]
                report["database_analysis"]["health_status"] = (
                    "healthy" if latest_db.is_healthy else "unhealthy"
                )
            ...
            if (
                critical_count > 5
                or report["security_analysis"]["threat_level"] == "critical"
            ):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocked_ips": len(self.blocked_ips),
            "database_health": (
                self.database_health_history[-1].is_healthy
                if self.database_health_history
                else None
            ),
            "last_health_check": (
                self.health_history[-1].timestamp.isoformat()
                if self.health_history
                else None
            ),
            "is_healthy": (
                self.health_history[-1].is_healthy if self.health_history else None
            ),
        }
