...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        response.headers["Content-Security-Policy"] = (
            "default-src 'self' data: blob:; script-src 'self' 'unsafe-inline' 'unsafe-eval' http://localhost:3000 http://127.0.0.1:3000 http://192.168.3.135:3000; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:; connect-src 'self' http://localhost:8000 http://127.0.0.1:8000 ws://localhost:* wss://localhost:*; frame-ancestors 'none';"
        )
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        response.headers["X-RateLimit-Remaining"] = str(
            self.max_requests - self.requests[key]
        )
