...
class CIRetryRequest(BaseModel):
    """CI/CDリトライリクエスト"""
    run_id: int
    retry_type: str = Field(default="failed_jobs", description="リトライタイプ: failed_jobs, all_jobs")
...
async def log_cicd_event(event_type: str, workflow_name: str, status: str, 
...
async def ci_health_check():
    ...
    try:
        ...
        for file_path in required_files:
            ...
            try:
                if os.path.exists(file_path):
                    health_status["components"][f"file_{file_name}"] = {"status": "healthy"}
                else:
                    health_status["components"][f"file_{file_name}"] = {"status": "missing"}
                    health_status["status"] = "degraded"
            except Exception as e:
...
async def get_ci_metrics():
    ...
    try:
        ...
        metrics.append(f"itsm_api_fix_success_rate {api_metrics.get('fix_success_rate', 0)}")
        
        # 無限ループ修復メトリクス
        metrics.append(f"itsm_repair_loop_count {loop_state.get('loop_count', 0)}")
        metrics.append(f"itsm_repair_total_errors_fixed {loop_state.get('total_errors_fixed', 0)}")
