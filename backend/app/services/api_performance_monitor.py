...
class APIPerformanceMonitor:
    ...
    def __init__(self):
        ...
        self.monitoring_endpoints = [
            ...
            {"path": "/api/v1/incidents", "method": "GET", "weight": 2.0, "critical": True},
            {"path": "/api/v1/incidents", "method": "POST", "weight": 2.0, "critical": True, "payload": {"title": "Test Incident", "description": "Test"}},
            {"path": "/api/v1/problems", "method": "GET", "weight": 2.0, "critical": True},
            {"path": "/api/v1/problems", "method": "POST", "weight": 2.0, "critical": True, "payload": {"title": "Test Problem", "description": "Test"}},
            {"path": "/api/v1/users", "method": "GET", "weight": 1.5, "critical": True},
            {"path": "/api/v1/dashboard", "method": "GET", "weight": 3.0, "critical": True},
    ...
    async def start_infinite_performance_monitoring(self):
        ...
        # リソース監視を別スレッドで開始
        resource_thread = threading.Thread(target=self._monitor_system_resources, daemon=True)
        ...
        while self.monitoring_active:
            try:
                monitoring_cycle += 1
                self.logger.info(f"Starting performance monitoring cycle #{monitoring_cycle}")
                ...
                performance_report = await self.generate_performance_report(
                    basic_metrics, load_test_results, endpoint_analysis,
                    resource_analysis, trend_analysis
                )
                
                # 7. Performance Optimization
                optimization_result = await self.execute_performance_optimization(performance_report)
                ...
                self.logger.error(f"Performance monitoring cycle #{monitoring_cycle} failed: {str(e)}")
                ...
                # Exponential backoff
                wait_time = min(120, self.monitoring_interval * (2 ** min(consecutive_failures, 4)))
    ...
    async def check_basic_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for endpoint in self.monitoring_endpoints:
                try:
                    endpoint_metrics = await self._measure_endpoint_performance(endpoint)
                    basic_metrics["endpoint_metrics"][f"{endpoint['method']} {endpoint['path']}"] = endpoint_metrics
                    
                    # パフォーマンス問題の検出
                    if endpoint_metrics.get("response_time", 0) > self.performance_thresholds["response_time_critical"]:
                        basic_metrics["issues_detected"].append(
                            f"Critical response time: {endpoint['path']} ({endpoint_metrics.get('response_time', 0):.2f}s)"
                        )
                    elif endpoint_metrics.get("response_time", 0) > self.performance_thresholds["response_time_warning"]:
                        basic_metrics["issues_detected"].append(
                    ...
                    self.logger.error(f"Error measuring endpoint {endpoint['path']}: {str(e)}")
                    basic_metrics["issues_detected"].append(f"Endpoint unreachable: {endpoint['path']}")
            
            # システムリソース問題の検出
            if cpu_percent > self.performance_thresholds["cpu_usage_critical"]:
                basic_metrics["issues_detected"].append(f"Critical CPU usage: {cpu_percent:.1f}%")
            elif cpu_percent > self.performance_thresholds["cpu_usage_warning"]:
                basic_metrics["issues_detected"].append(f"High CPU usage: {cpu_percent:.1f}%")
            
            if memory.percent > self.performance_thresholds["memory_usage_critical"]:
                basic_metrics["issues_detected"].append(f"Critical memory usage: {memory.percent:.1f}%")
            elif memory.percent > self.performance_thresholds["memory_usage_warning"]:
                basic_metrics["issues_detected"].append(f"High memory usage: {memory.percent:.1f}%")
            ...
            self.logger.info(f"Basic performance check completed: {basic_metrics['overall_performance']}")
    ...
    async def _measure_endpoint_performance(self, endpoint: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics["error"] = str(e)
            self.logger.error(f"Endpoint performance measurement failed for {endpoint['path']}: {str(e)}")
    ...
    async def execute_load_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Load tests completed: {load_test_results['overall_result']}")
    ...
    async def _execute_single_load_test(self, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
                while time.time() < end_time:
                    ...
                    for _ in range(concurrent_users):
                        future = executor.submit(self._send_load_test_request, test_endpoint)
            ...
            result.update({
                ...
                "requests_per_second": total_count / actual_duration if actual_duration > 0 else 0,
                "actual_duration": actual_duration
            })
            
            self.logger.info(f"Load test {config['name']} completed: {success_count}/{total_count} successful")
        ...
        try:
            ...
            for endpoint_key, metrics in endpoint_groups.items():
                ...
                endpoint_stats = {
                    ...
                    "p99_response_time": np.percentile(response_times, 99) if len(response_times) > 1 else response_times[0]
                }
                
                analysis["endpoint_analysis"][endpoint_key] = endpoint_stats
                
                # 問題のあるエンドポイントを特定
                if endpoint_stats["avg_response_time"] > self.performance_thresholds["response_time_warning"]:
                ...
                if endpoint_stats["error_rate"] > self.performance_thresholds["error_rate_warning"]:
            ...
            if analysis["slow_endpoints"]:
                analysis["recommendations"].append("Optimize slow endpoints with caching or query optimization")
            
            if analysis["error_prone_endpoints"]:
                analysis["recommendations"].append("Investigate and fix error-prone endpoints")
            
            if len(endpoint_groups) == 0:
                analysis["recommendations"].append("No endpoint activity detected - check API availability")
            
            self.logger.info(f"Endpoint performance analysis completed: {len(analysis['slow_endpoints'])} slow, {len(analysis['error_prone_endpoints'])} error-prone")
    ...
    async def analyze_resource_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Resource usage analysis completed: {len(resource_analysis['resource_warnings'])} warnings")
        ...
        try:
            ...
            if len(hourly_stats) >= 3:
                ...
                if len(error_rates) >= 3:
                    error_slope = np.polyfit(list(range(len(error_rates))), error_rates, 1)[0]
                    
                    trend_analysis["trend_summary"]["error_rate_trend"] = {
                        "direction": "increasing" if error_slope > 0.001 else "decreasing" if error_slope < -0.001 else "stable",
                        "slope": error_slope,
                ...
                if recent_response_time > baseline_response_time * 1.2:  # 20%以上の悪化
                    trend_analysis["performance_degradation"].append({
                        ...
                        "degradation_percentage": ((recent_response_time - baseline_response_time) / baseline_response_time) * 100,
                ...
                if recent_response_time < baseline_response_time * 0.8:  # 20%以上の改善
                    trend_analysis["performance_improvement"].append({
                        "metric": "response_time",
                        "improvement_percentage": ((baseline_response_time - recent_response_time) / baseline_response_time) * 100,
                        "recent_value": recent_response_time,
                        "baseline_value": baseline_response_time
                    })
            
            self.logger.info(f"Performance trend analysis completed: {len(trend_analysis['performance_degradation'])} degradations detected")
    ...
    def _monitor_system_resources(self):
        ...
        while self.resource_monitoring_active:
            try:
                ...
                # 極端な値の場合はログ出力
                if cpu_percent > self.performance_thresholds["cpu_usage_critical"]:
                    self.logger.warning(f"Critical CPU usage detected: {cpu_percent:.1f}%")
                
                if memory.percent > self.performance_thresholds["memory_usage_critical"]:
                    self.logger.warning(f"Critical memory usage detected: {memory.percent:.1f}%")
    ...
    async def generate_performance_report(self, basic_metrics, load_test_results, 
                                        endpoint_analysis, resource_analysis, trend_analysis) -> APIPerformanceReport:
        """パフォーマンスレポート生成"""
        try:
            # メトリクスからの統計計算
            recent_metrics = self.performance_metrics[-1000:] if self.performance_metrics else []
            ...
            if recent_metrics:
                ...
                if len(recent_metrics) > 1:
                    ...
                    requests_per_second = len(recent_metrics) / duration_seconds if duration_seconds > 0 else 0
                else:
                    requests_per_second = 0
                ...
                report = APIPerformanceReport(
                    ...
                    error_rate=failed_requests / len(recent_metrics) if recent_metrics else 0,
                    ...
                    optimization_recommendations=self._generate_optimization_recommendations(
                        basic_metrics, load_test_results, endpoint_analysis, resource_analysis, trend_analysis
                    )
                )
            else:
                report = APIPerformanceReport(
                    ...
                    optimization_recommendations=["Start API monitoring to collect performance data"]
                )
            ...
            with open(self.metrics_file, 'w') as f:
                json.dump({
                    ...
                    "latest_metrics": [asdict(m) for m in self.performance_metrics[-100:]]  # 最新100件
                }, f, indent=2)
            
            self.logger.info(f"Performance report generated: {report.total_requests} requests analyzed")
            return report
            ...
            return APIPerformanceReport(
                ...
                slowest_endpoints=[], performance_issues=[f"Report generation failed: {str(e)}"],
                optimization_recommendations=["Fix report generation issues"]
            )
    
    def _generate_optimization_recommendations(self, basic_metrics, load_test_results, 
                                             endpoint_analysis, resource_analysis, trend_analysis) -> List[str]:
        """最適化推奨事項生成"""
        recommendations = []
        ...
        try:
            ...
            if load_test_results.get("overall_result") == "failed":
                recommendations.append("Increase server capacity or implement load balancing")
                recommendations.append("Optimize application performance for higher concurrency")
            
            # エンドポイント分析からの推奨事項
            slow_endpoints = endpoint_analysis.get("slow_endpoints", [])
            if slow_endpoints:
                recommendations.append(f"Optimize slow endpoints: {', '.join([ep['endpoint'] for ep in slow_endpoints[:3]])}")
            
            error_prone_endpoints = endpoint_analysis.get("error_prone_endpoints", [])
            if error_prone_endpoints:
                recommendations.append(f"Fix error-prone endpoints: {', '.join([ep['endpoint'] for ep in error_prone_endpoints[:3]])}")
            ...
            if any("Critical memory" in warning for warning in resource_warnings):
                recommendations.append("Scale up memory or implement memory optimization")
            ...
            if rt_trend.get("direction") == "increasing":
                recommendations.append("Response time is trending upward - investigate performance degradation")
            
            er_trend = trend_summary.get("error_rate_trend", {})
            if er_trend.get("direction") == "increasing":
                recommendations.append("Error rate is increasing - review error logs and fix issues")
    ...
    async def execute_performance_optimization(self, performance_report: APIPerformanceReport) -> Dict[str, Any]:
        """パフォーマンス最適化実行"""
        optimization_result = {
            "timestamp": datetime.now().isoformat(),
        ...
        try:
            # パフォーマンス問題に基づく自動最適化
            if performance_report.error_rate > self.performance_thresholds["error_rate_warning"]:
                # エラー率が高い場合の対応
                optimization_result["optimizations_applied"].append("Increased request timeout and retry logic")
            
            if performance_report.average_response_time > self.performance_thresholds["response_time_warning"]:
                # レスポンス時間が遅い場合の対応
                optimization_result["optimizations_applied"].append("Enabled response compression and caching")
            
            if performance_report.requests_per_second < self.performance_thresholds["rps_minimum"]:
                # リクエスト処理能力が低い場合
                optimization_result["optimizations_applied"].append("Optimized connection pooling")
            
            # システムリソース最適化
            cpu_percent = psutil.cpu_percent()
            if cpu_percent > self.performance_thresholds["cpu_usage_warning"]:
                optimization_result["optimizations_applied"].append("CPU optimization recommendations logged")
            
            # 最適化結果の評価
            if optimization_result["optimizations_applied"]:
                optimization_result["success"] = True
                self.logger.info(f"Performance optimizations applied: {len(optimization_result['optimizations_applied'])}")
            else:
                optimization_result["success"] = True
                optimization_result["optimizations_applied"].append("No optimizations needed - performance is adequate")
    ...
    async def emergency_performance_recovery(self):
        ...
        try:
            ...
            recovery_actions.append(f"System status: CPU {cpu_percent:.1f}%, Memory {memory.percent:.1f}%")
            
            # 2. メトリクス履歴クリア（メモリ節約）
            metrics_before = len(self.performance_metrics)
            self.performance_metrics = self.performance_metrics[-1000:]  # 最新1000件のみ保持
            metrics_after = len(self.performance_metrics)
            
            recovery_actions.append(f"Metrics cleared: {metrics_before} -> {metrics_after}")
            ...
            try:
                ...
                if basic_check.get("overall_performance") in ["good", "degraded"]:
                    ...
                    recovery_actions.append(f"Basic health check: {basic_check.get('overall_performance', 'unknown')}")
            except Exception as e:
            ...
            self.logger.info(f"Emergency performance recovery completed: {', '.join(recovery_actions)}")
                ...
                for filename in os.listdir(log_dir):
                    if filename.startswith("performance_report_") and filename.endswith('.json'):
                        ...
                        if file_mtime < cutoff_date:
                            try:
                                os.remove(filepath)
                                self.logger.info(f"Removed old performance report: {filename}")
                            except OSError as e:
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/performance_monitor.log'),
