...
class APISpecAnalyzer:
    ...
    def _check_endpoint_compliance(
        self, endpoint: str, method: str, log_entry: Dict, openapi_spec: Dict
    ) -> List[APIError]:
        """エンドポイントの仕様準拠をチェック"""
        errors = []
...
class APIRepairer:
    ...
    async def _fix_missing_endpoint(self, error: APIError) -> Optional[APIFix]:
        ...
        endpoint_code = self._generate_endpoint_code(
            resource_name, method, endpoint_path
        )
        ...
        if api_file_path.exists():
            ...
            # 新しいファイルを作成
            full_file_content = self._generate_full_api_file(
                resource_name, method, endpoint_path
            )
        ...
        return APIFix(
            ...
            validation_tests=[
                f"python -c 'from app.api.v1.{resource_name} import router'"
            ],
        )
    ...
    def _add_error_handling(self, content: str, method: str) -> str:
        ...
        for line in lines:
            if re.match(r"^async def \w+\(", line.strip()):
                ...
                in_function
                and line.strip()
                and not line.startswith(" " * (function_indent + 4))
            ):
                # 関数終了
                in_function = False
                enhanced_lines.append(line)
            elif in_function and not any(
                keyword in line for keyword in ["try:", "except:", "finally:"]
            ):
            ...
            for test_command in api_fix.validation_tests:
                try:
                    ...
                    if result.returncode != 0:
                        self.logger.error(
                            f"バリデーションテスト失敗: {test_command}\n{result.stderr}"
                        )
                except subprocess.TimeoutExpired:
                    self.logger.error(
                        f"バリデーションテストタイムアウト: {test_command}"
                    )
