...
async def list_notifications(
    ...
    priority: Optional[str] = Query(None, pattern="^(low|medium|high|critical)$", description="優先度フィルター"),
...
async def bulk_delete_notifications(
    ...
    older_than_days: Optional[int] = Query(None, ge=1, description="指定日数より古い通知を削除"),
    db: Session = Depends(get_db),
    current_user_id: UUID = Depends(get_current_user_id)
) -> SuccessResponse:
    """通知を一括削除する"""
...
async def list_templates(
    template_type: Optional[str] = Query(None, description="テンプレートタイプフィルター"),
    db: Session = Depends(get_db),
    current_user_id: UUID = Depends(get_current_user_id),
