...
class ITSMAuditService:
    ...
    def __init__(self):
        self.audit_log_dir = (
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/audit"
        )
        self.metrics_dir = (
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/metrics"
        )
        self.compliance_dir = (
            "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/compliance"
        )
    ...
    async def log_audit_event(
        ...
        try:
            ...
            log_file = os.path.join(
                self.audit_log_dir, f"audit_{timestamp.strftime('%Y%m%d')}.log"
            )
    ...
    async def _log_high_severity_event(self, audit_entry: Dict[str, Any]) -> None:
        ...
        try:
            ...
            alert_entry = {
                ...
                "escalation_needed": audit_entry["severity"]
    ...
    async def _process_compliance_requirements(
    ...
    async def _handle_financial_audit(self, audit_entry: Dict[str, Any]) -> None:
        """財務監査の処理"""
        try:
            financial_log_file = os.path.join(
                self.compliance_dir, "financial_audit.log"
            )
    ...
    async def _handle_security_compliance(self, audit_entry: Dict[str, Any]) -> None:
        ...
        try:
            security_log_file = os.path.join(
                self.compliance_dir, "security_compliance.log"
            )
            ...
            security_entry = {
                ...
                "incident_response_triggered": audit_entry["severity"]
                in ["high", "critical"],
            }
    ...
    async def get_audit_logs(
        ...
        try:
            ...
            while current_date <= end_date.date():
                log_file = os.path.join(
                    self.audit_log_dir, f"audit_{current_date.strftime('%Y%m%d')}.log"
                )
                    ...
                    async with aiofiles.open(log_file, "r", encoding="utf-8") as f:
                        async for line in f:
                            try:
                                ...
                                if (
                                    event_type
                                    and entry.get("event_type") != event_type.value
                                ):
                                    continue
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            for log in all_logs:
                ...
                stats["events_by_user"][user_id] = (
                    stats["events_by_user"].get(user_id, 0) + 1
                )
    ...
    def _generate_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if total_events == 0:
                return [
                    "監査イベントがありません。システムの監査設定を確認してください。"
                ]

            high_severity_ratio = stats["high_severity_events"] / total_events
            if high_severity_ratio > 0.1:
                recommendations.append(
                    "高重要度イベントの発生率が高いです。セキュリティ対策の強化を検討してください。"
                )

            security_events = stats["security_events"]
            if security_events > 0:
                recommendations.append(
                    f"{security_events}件のセキュリティイベントが発生しています。詳細な調査を実施してください。"
                )
            ...
            if "system" in user_stats and user_stats["system"] / total_events > 0.8:
                recommendations.append(
                    "システム自動処理の割合が高いです。手動承認プロセスの導入を検討してください。"
                )

            if len(recommendations) == 0:
                recommendations.append(
                    "監査状況は良好です。現在のセキュリティポリシーを維持してください。"
                )
...
async def log_ci_workflow_action(
    action: str, workflow_id: int, user_id: str, result: str
) -> str:
...
async def log_automated_repair(
    repair_target: str, repair_result: str, repairs_count: int
) -> str:
    """自動修復をログに記録"""
    severity = AuditSeverity.MEDIUM
