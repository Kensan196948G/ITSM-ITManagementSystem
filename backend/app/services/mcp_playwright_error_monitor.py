...
class MCPPlaywrightErrorMonitor:
        ...
        while self.monitoring_active:
            try:
                ...
                if self._has_critical_errors(
                    health_metrics, db_status, perf_metrics, security_status
                ):
                ...
                if consecutive_errors >= max_consecutive_errors:
                    self.logger.critical(
                        f"Too many consecutive errors ({consecutive_errors}). Attempting system recovery..."
                    )
    ...
    async def check_api_health(self) -> APIHealthMetrics:
        ...
        for endpoint in self.api_endpoints:
            try:
                ...
                self.logger.error(
                    f"Error checking endpoint {endpoint['path']}: {str(e)}"
                )
                endpoint_results.append(
                    {
                        "endpoint": endpoint["path"],
        ...
        avg_response_time = (
            total_response_time / total_endpoints if total_endpoints > 0 else 0
        )
        error_rate = (
            (total_endpoints - healthy_count) / total_endpoints
            if total_endpoints > 0
            else 0
        )
        uptime_percentage = (
            (healthy_count / total_endpoints * 100) if total_endpoints > 0 else 0
        )
        ...
        self.logger.info(
            f"API Health Check completed: {healthy_count}/{total_endpoints} endpoints healthy"
        )
        return metrics
    ...
    async def check_database_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            db_status.update({"status": "error", "error": str(e)})
    ...
    async def monitor_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for endpoint in self.api_endpoints:
                try:
                    ...
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        ...
                        perf_metrics["api_response_times"][
                            endpoint["path"]
                        ] = response_time
                    ...
                    self.logger.error(
                        f"Performance check error for {endpoint['path']}: {str(e)}"
                    )
            ...
            self.logger.info(
                f"Performance monitoring completed: {perf_metrics['overall_status']}"
            )
    ...
    async def scan_security_issues(self) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_checks = [
                check
                for check, result in security_status["security_checks"].items()
            ...
            self.logger.info(
                f"Security scan completed: {security_status['overall_status']}"
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 401:
                    ...
                    result["details"] = (
                        f"Authentication check failed: got {response.status_code} instead of 401"
                    )
            ...
            async with httpx.AsyncClient() as client:
                for payload in payloads:
                    ...
                    response = await client.get(
                        f"{self.base_url}/api/v1/incidents", params={"search": payload}
                    )
    ...
    async def analyze_error_logs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log_analysis["recent_errors"] = [
                asdict(e) for e in recent_errors[-10:]
            ]  # Last 10

            # Generate recommendations
            if log_analysis["total_errors"] > 0:
                log_analysis["recommendations"] = self._generate_error_recommendations(
                    log_analysis
                )

            self.logger.info(
                f"Error log analysis completed: {log_analysis['total_errors']} total errors"
            )
    ...
    def _generate_error_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if "DATABASE_ERROR" in error_categories:
            recommendations.append(
                "Database connectivity issues detected - check database service and connection pool"
            )

        if "API_ERROR" in error_categories:
            recommendations.append(
                "API errors detected - review endpoint implementations and error handling"
            )

        if "CONNECTION_ERROR" in error_categories:
            recommendations.append(
                "Connection errors detected - check network connectivity and service availability"
            )

        if len(analysis.get("critical_errors", [])) > 5:
            recommendations.append(
                "High number of critical errors - immediate investigation required"
            )
    ...
    def _has_critical_errors(
        self, health_metrics, db_status, perf_metrics, security_status
    ) -> bool:
        """クリティカルエラーの検出"""
        # API health issues
        if (
    ...
    async def execute_auto_repair(self):
        ...
        self.logger.info(
            "Executing manual repair procedures with ClaudeCode integration..."
        )
        ...
        try:
            ...
            self.logger.info(
                f"Manual repair completed: {len(repair_actions)} actions performed"
            )
    ...
    async def _trigger_claudecode_manual_repair(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_report = {
                ...
                "critical_errors": [
                    e for e in self.error_history if e.severity == "critical"
                ],
            ...
            result["details"] = (
                f"ClaudeCode repair request created with {len(error_report['critical_errors'])} critical errors"
            )
    ...
    async def _perform_health_recheck(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                result["details"] = (
                    f"Health improved: {health_metrics.uptime_percentage:.1f}% uptime"
                )
            else:
                result["status"] = "needs_attention"
                result["details"] = (
                    f"Health issues persist: {health_metrics.error_rate:.2f} error rate"
                )
    ...
    async def _perform_security_repair(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["status"] = "success"
            result["details"] = (
                f"Security repair completed: {', '.join(security_actions)}"
            )
        ...
        try:
            ...
            if temp_dir.exists():
                for temp_file in temp_dir.glob("*"):
                    if (
                        temp_file.is_file()
                        and temp_file.stat().st_mtime < time.time() - 3600
                    ):  # 1 hour old
                        temp_file.unlink()
            ...
            result["status"] = "success"
            result["details"] = (
                f"Performance optimization completed: {', '.join(optimization_actions)}"
            )
    ...
    async def _verify_repair_success(self) -> Dict[str, Any]:
        ...
        try:
            ...
            passed_checks = sum(
                1 for check in verification_results if "PASSED" in check
            )
            ...
            if passed_checks == total_checks:
                ...
                result["status"] = "failed"
                result["details"] = (
                    f"Only {passed_checks}/{total_checks} verification checks passed"
                )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/api/v1/system/clear-cache"
                )
    ...
    async def _update_error_metrics(self):
        ...
        try:
            metrics = {
                ...
                "fix_success_rate": 0,
                "health_status": (
                    "healthy" if len(self.error_history) == 0 else "unhealthy"
                ),
            }
            ...
            attempted_fixes = sum(
                1 for error in self.error_history if error.fix_attempted
            )
            successful_fixes = sum(
                1 for error in self.error_history if error.fix_successful
            )
    ...
    async def emergency_system_recovery(self):
        ...
        try:
            ...
            recovery_actions.append(
                f"Health check performed: {health_result.uptime_percentage}% uptime"
            )

            # 4. Log recovery
            self.logger.info(
                f"Emergency recovery completed: {', '.join(recovery_actions)}"
            )
    ...
    async def generate_monitoring_report(
        self, health_metrics, db_status, perf_metrics, security_status, log_analysis
    ):
        """監視レポート生成"""
        try:
            report = {
            ...
            # Log summary
            self.logger.info(
                f"Monitoring report generated: {report['overall_status']} status"
            )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                "/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend/logs/mcp_playwright_monitor.log"
            ),
