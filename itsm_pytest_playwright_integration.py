...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/logs/itsm_pytest_playwright.log'),
...
class ITSMTestIntegration:
    ...
    def __init__(self):
        ...
        self.report_file = self.project_root / "reports" / f"itsm_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        self.markdown_report = self.project_root / "reports" / f"itsm_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"フロントエンド正常: {health_status['response_time']:.2f}秒")
                else:
    ...
    async def check_backend_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    health_status['status'] = 'healthy'
                    logger.info(f"バックエンド正常: {health_status['response_time']:.2f}秒")
                else:
                    health_status['status'] = 'unhealthy'
                    health_status['errors'].append(f"HTTP {response.status_code}")
    ...
    async def run_api_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"APIテスト完了: {api_test_results['passed']}/{api_test_results['total_tests']} 成功")
    ...
    async def run_ui_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"UIテスト完了: {ui_test_results['passed']}/{ui_test_results['total_tests']} 成功")
    ...
    async def _run_ui_test_case(self, page: Page, test_case_name: str) -> Dict[str, Any]:
        """個別UIテストケース実行"""
        try:
            test_result = {
    ...
    async def run_e2e_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"E2Eテスト完了: {e2e_test_results['passed']}/{e2e_test_results['total_tests']} 成功")
    ...
    async def run_load_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response_times:
                load_test_results['average_response_time'] = sum(response_times) / len(response_times)
    ...
    def run_pytest_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                pytest_results['total_tests'] = report_data.get('summary', {}).get('total', 0)
                pytest_results['passed'] = report_data.get('summary', {}).get('passed', 0)
                pytest_results['failed'] = report_data.get('summary', {}).get('failed', 0)
                pytest_results['skipped'] = report_data.get('summary', {}).get('skipped', 0)
    ...
    def assess_code_quality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_assessment['overall_score'] = coverage_score + linting_score + security_score + complexity_score
            
            # 推奨事項
            if quality_assessment['coverage'] < 80:
                quality_assessment['recommendations'].append('テストカバレッジを80%以上に向上')
            ...
            logger.info(f"コード品質評価完了: スコア {quality_assessment['overall_score']:.1f}/100")
    ...
    def evaluate_ci_compliance(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            # コード品質チェック
            if 'code_quality' in self.test_results and self.test_results['code_quality']:
                quality_score = self.test_results['code_quality'].get('overall_score', 0)
                if quality_score >= 70:  # 70点以上
                    compliance_checks.append(True)
                else:
                    compliance_checks.append(False)
                    issues.append(f'コード品質スコアが基準未満: {quality_score:.1f} < 70')
            else:
                compliance_checks.append(False)
            ...
            frontend_healthy = (
                'ui_tests' in self.test_results and 
                self.test_results['ui_tests'].get('passed', 0) > 0
            )
            backend_healthy = (
                'api_tests' in self.test_results and 
                self.test_results['api_tests'].get('passed', 0) > 0
            )
