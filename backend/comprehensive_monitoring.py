...
class ComprehensiveMonitoringSystem:
    ...
    def _print_scan_summary(self, report: dict):
        ...
        print(f"\n⚡ パフォーマンス分析:")
        print(f"  平均レスポンス時間: {performance_analysis.get('avg_response_time', 0):.3f}秒")
        print(f"  遅いエンドポイント: {len(performance_analysis.get('slow_endpoints', []))}個")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="監視間隔（秒）デフォルト: 60秒"
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="一回だけスキャンを実行して終了"
    )
