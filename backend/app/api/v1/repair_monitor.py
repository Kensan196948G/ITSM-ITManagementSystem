...
async def get_repair_status() -> Dict[str, Any]:
    ...
    try:
        ...
        coordination_status = {
            ...
            "last_repair": coordination_repair_service.repair_history[-1].timestamp if coordination_repair_service.repair_history else None
        }
        
        # エラーファイル状況
        backend_root = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/backend")
        coordination_dir = Path("/media/kensan/LinuxHDD/ITSM-ITmanagementSystem/coordination")
        ...
        if repair_file.exists():
            with open(repair_file, 'r') as f:
                ...
                file_status["realtime_repair"] = {
                    ...
                    "repairs_completed": repair_data.get("state", {}).get("repairs_completed", 0),
                    "status": repair_data.get("state", {}).get("status", "unknown")
                }
        
        return {
            "timestamp": datetime.now().isoformat(),
            "overall_status": "operational" if engine_status["is_running"] else "standby",
...
async def start_repair_system(background_tasks: BackgroundTasks) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start repair system: {str(e)}")
...
async def stop_repair_system() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to stop repair system: {str(e)}")
...
async def repair_coordination_errors() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Coordination repair failed: {str(e)}")
...
async def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        # 総合判定
        component_health = [comp["health"] if "health" in comp else comp["status"] 
        ...
        metrics = {
            ...
            "coordination_service": {
                ...
                "recent_repairs": [
                    ...
                    for r in coordination_repair_service.repair_history[-10:]  # 最新10件
                ]
            },
            "performance": {
