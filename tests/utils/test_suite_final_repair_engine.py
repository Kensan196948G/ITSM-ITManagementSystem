...
class TestSuiteFinalRepairEngine:
    ...
    async def check_test_suite_health(self) -> TestSuiteHealthStatus:
        ...
        next_repair_time = (
            datetime.now() + timedelta(seconds=self.repair_interval)
        ).isoformat()
    ...
    async def _count_failed_tests(self) -> int:
        ...
        try:
            ...
            report_patterns = ["*-report.json", "junit-*.xml", "test-results-*.json"]
                    ...
                    try:
                        if report_file.suffix == ".json":
                            async with aiofiles.open(report_file, "r") as f:
                                ...
                                if "summary" in report_data:
                                    failed_count += report_data["summary"].get(
                                        "failed", 0
                                    )
    ...
    def _determine_overall_health(self, *statuses) -> str:
        ...
        if (
            "down" in status_list
            or "missing_tests" in status_list
    ...
    async def execute_repair_action(self, action: TestSuiteRepairAction) -> Dict:
        ...
        for attempt in range(action.max_retries):
            try:
                ...
                logger.info(
                    f"🔧 Attempt {attempt + 1}/{action.max_retries}: {action.repair_command}"
                )
                ...
                if action.validation_command:
                    ...
                    val_stdout, val_stderr = await asyncio.wait_for(
                        validation_process.communicate(), timeout=30
                    )
                    ...
                    if validation_process.returncode == 0:
                        ...
                        logger.warning(
                            f"⚠️ Validation failed for {action.action_id}: {val_stderr.decode()}"
                        )
                else:
                ...
                repair_result["error"] = (
                    f"Timeout after {action.timeout_seconds} seconds"
                )
    ...
    async def run_repair_cycle(self) -> Dict:
        ...
        cycle_result["overall_success"] = (
            health_after.overall_health in ["healthy", "warning"]
            and cycle_result["repair_actions_failed"] == 0
        )
        ...
        logger.info(
            f"🏁 Repair cycle #{self.current_repair_cycle} completed in {cycle_result['duration_seconds']:.2f}s"
        )

        return cycle_result

    def _determine_needed_repairs(
        self, health_status: TestSuiteHealthStatus
    ) -> List[TestSuiteRepairAction]:
        ...
        repair_mapping = {
            ...
            "e2e_tests_status": [
                "playwright_environment_repair",
                "frontend_test_environment_repair",
            ],
        ...
        for component, action_ids in repair_mapping.items():
            ...
            if component_status in [
                ...
                for action_id in action_ids:
                    action = next(
                        (a for a in self.repair_actions if a.action_id == action_id),
                        None,
                    )
        ...
        if health_status.failed_test_count > 0 or health_status.error_count > 0:
            consolidation_action = next(
                (
                    ...
                    if a.action_id == "test_reports_consolidation_repair"
                ),
                None,
            )
    ...
    async def _save_repair_state(
        self, cycle_result: Dict, health_status: TestSuiteHealthStatus
    ):
        ...
        try:
            ...
            repair_state = {
                ...
                "last_successful_repair": (
                    self.last_successful_repair.isoformat()
                    if self.last_successful_repair
                    else None
                ),
    ...
    def _calculate_health_score(self, health_status: TestSuiteHealthStatus) -> float:
        ...
        try:
            ...
            final_score = max(
                0, base_score + coverage_bonus - failure_penalty - error_penalty
            )
        ...
        try:
            while True:
                ...
                if self.consecutive_failures >= self.max_consecutive_failures:
                    logger.error(
                        f"❌ Stopping repair loop after {self.consecutive_failures} consecutive failures"
                    )
                    break
                ...
                logger.info(
                    f"   Repairs: {cycle_result['repair_actions_successful']} successful, {cycle_result['repair_actions_failed']} failed"
                )
                logger.info(
                    f"   Overall Success: {'✅' if cycle_result['overall_success'] else '❌'}"
                )

                # Wait for next cycle
                logger.info(
                    f"⏰ Waiting {self.repair_interval} seconds for next repair cycle..."
                )
...
async def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--single":
        ...
        print(
            f"✅ Single cycle completed: {'Success' if result['overall_success'] else 'Failed'}"
        )
    else:
