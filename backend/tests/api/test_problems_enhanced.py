...
class TestProblemsEnhancedAPI:
    ...
    def test_rca_workflow(self, db_session: Session):
        ...
        response = client.post(
            f"/api/v1/problems/{problem.id}/rca/start", json=rca_start_data
        )
        ...
        response = client.post(
            f"/api/v1/problems/{problem.id}/rca/findings", json=finding_data
        )
    ...
    def test_bulk_operations(self, db_session: Session):
        ...
        bulk_update_data = {
            ...
            "updates": {"priority": "high", "business_impact": "medium"},
        }
        ...
        # 一括削除
        bulk_delete_data = {"problem_ids": problem_ids, "reason": "テストのため削除"}
    ...
    def test_export_functionality(self, db_session: Session):
        ...
        # フィルター付きエクスポート
        response = client.get("/api/v1/problems/export?format=csv&category=software")
        assert response.status_code == 200
...
def mock_security_check():
    """セキュリティチェックのモック"""
    with patch(
        "app.core.problem_security.ProblemSecurityManager.check_permission"
    ) as mock:
