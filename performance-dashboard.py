...
"""
    ...
    def _create_loop_progress_chart(self, metrics: Dict[str, Any]):
        ...
        # データ準備
        timestamps = [datetime.fromisoformat(repair['timestamp']) for repair in repair_history[-20:]]
        ...
        # ループ数推移
        ax1.plot(timestamps, loop_numbers, marker='o', linewidth=2, markersize=6, color=self.colors['primary'])
        ...
        plt.tight_layout()
        plt.savefig(self.reports_dir / 'loop_progress_chart.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_error_trend_chart(self, metrics: Dict[str, Any]):
        ...
        ax.plot(timestamps, total_errors, marker='o', linewidth=2, label='総エラー数', color=self.colors['danger'])
        ax.plot(timestamps, critical_errors, marker='s', linewidth=2, label='クリティカルエラー', color=self.colors['warning'])
        ...
        plt.tight_layout()
        plt.savefig(self.reports_dir / 'error_trend_chart.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_resource_chart(self, metrics: Dict[str, Any]):
        ...
        colors = [
            self.colors['danger'] if u > 80 else self.colors['warning'] if u > 60 else self.colors['success']
            for u in usage
        ]
        ...
        # 円グラフ
        ax2.pie(usage, labels=resources, autopct='%1.1f%%', colors=colors, startangle=90)
        ax2.set_title('リソース使用率分布', fontsize=14, fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(self.reports_dir / 'resource_chart.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_response_time_chart(self, metrics: Dict[str, Any]):
        ...
        colors = [self.colors['success'] if status == '正常' else self.colors['danger'] for status in statuses]
        ...
        for bar, value, status in zip(bars, response_times, statuses):
            if value > 0:
                ...
                ax.text(bar.get_x() + bar.get_width()/2, 0.05,
                       f'エラー', ha='center', va='bottom', fontweight='bold', color='white')
        
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(self.reports_dir / 'response_time_chart.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_repair_effectiveness_chart(self, metrics: Dict[str, Any]):
        ...
        ax1.set_ylabel('効率 (%)')
        ax1.set_ylim(0, 100)
        ax1.text(0, effectiveness + 2, f'{effectiveness:.1f}%', ha='center', fontweight='bold')
        
        # 修復実績
        ax2.bar(['総修復数', 'ループ数'], [total_fixes, loop_count], 
        ...
        # 修復対象別円グラフ
        if repair_targets:
            ax3.pie(repair_targets.values(), labels=repair_targets.keys(), autopct='%1.1f%%',
        ...
        if repair_history:
            ...
            timestamps = [datetime.fromisoformat(repair['timestamp']) for repair in recent_repairs]
            loop_nums = [repair['loop'] for repair in recent_repairs]
            
            ax4.plot(timestamps, loop_nums, marker='o', linewidth=2, color=self.colors['info'])
        ...
        plt.tight_layout()
        plt.savefig(self.reports_dir / 'repair_effectiveness_chart.png', dpi=300, bbox_inches='tight')
        plt.close()
