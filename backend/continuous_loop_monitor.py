...
class ContinuousLoopMonitor:
    ...
    def monitor_continuous_loop(self, max_duration_minutes=60):
        ...
        while self.monitoring_active and datetime.now() < end_time:
            ...
            print(
                f"[{elapsed_minutes:6.1f}min] Check #{check_count:3d}: "
            ...
            if current_errors == 0:
                zero_error_count += 1
                print(
                    f"  ✓ Zero errors detected ({zero_error_count}/{consecutive_zero_needed})"
                )
                ...
                if zero_error_count > 0:
                    print(
                        f"  ✗ Error count increased, resetting zero counter (was {zero_error_count})"
                    )
                zero_error_count = 0
            ...
            if current_errors > 0:
                ...
                print(
                    f"    API: {api_errors.get('total_errors', 0)}+{api_errors.get('penalty', 0)}, "
    ...
    def analyze_loop_efficiency(self):
        ...
        if len(self.loop_transitions) > 1:
            ...
            for i in range(1, len(self.loop_transitions)):
                prev_time = datetime.fromisoformat(
                    self.loop_transitions[i - 1]["timestamp"]
                )
                curr_time = datetime.fromisoformat(
                    self.loop_transitions[i]["timestamp"]
                )
            ...
            print(
                f"Loop interval range: {min(intervals):.1f} - {max(intervals):.1f} minutes"
            )
        ...
        if total_loops > 0:
            ...
            print(
                f"Loop progression: {first_loop} → {last_loop} ({loop_progression} loops)"
            )

        return {
            "total_transitions": len(self.loop_transitions),
            "average_interval_minutes": (
                avg_interval if len(self.loop_transitions) > 1 else None
            ),
