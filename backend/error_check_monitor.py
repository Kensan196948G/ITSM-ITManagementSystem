...
def check_system_errors():
    ...
    # 結果表示
    print(
        f"API Errors: {api_result.get('count', 'N/A')}, Health: {api_result.get('health', 'N/A')}"
    )
    print(f"Coordination Errors: {coord_result.get('count', 'N/A')}")
    print(
        f"Loop Status: Loop#{loop_result.get('loop_count', 'N/A')}, Fixed: {loop_result.get('total_fixed', 'N/A')}"
    )
    print(f"Basic Tests: {test_result.get('status', 'N/A')}")

    # エラースコア計算
    error_score = calculate_error_score(
        api_result, coord_result, loop_result, test_result
    )
...
def main():
    ...
    for i, result in enumerate(results, 1):
        print(
            f"Check #{i}: Score={result['error_score']}, Loop={'YES' if result['should_trigger_loop'] else 'NO'}"
        )
